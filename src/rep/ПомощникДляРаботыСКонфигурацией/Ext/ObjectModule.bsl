#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда

#Область ОписаниеПеременных

Перем РазрешенныеТипыКВыгрузке;

Перем ЗаполненныеРеквизиты;

Перем КэшИменМетаданныхПоТипу;

Перем ВыведенныеСообщения;

Перем ОсновныеРоли;

перем ТипыXDTO;

#КонецОбласти

#Область ПрограммныйИнтерфейс

#Область РаботаСМетаданными

// Получает подсистемы, в которые входит объект метаданных.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных - Метаданные объект
// 
// Возвращаемое значение:
//  Массив Из ОбъектМетаданныхПодсистема
//
Функция ПолучитьПодсистемыОбъекта(ОбъектМетаданных) Экспорт
	
	Подсистемы = Новый Массив;
	
	ПолучитьПодсистемыОбъектаРекурсивно(ОбъектМетаданных, Метаданные.Подсистемы, Подсистемы);
	
	Возврат Подсистемы;
	
КонецФункции

// Получает функциональные опции для объекта метаданных.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных - Метаданные объект
// 
// Возвращаемое значение:
//  Массив Из ОбъектМетаданныхФункциональнаяОпция
//
Функция ПолучитьФункциональныеОпцииОбъекта(ОбъектМетаданных) Экспорт
	
	ФункциональныеОпции = Новый Массив;
	
	Для Каждого ФункциональнаяОпция Из Метаданные.ФункциональныеОпции Цикл
		
		Если ФункциональнаяОпция.Состав.Содержит(ОбъектМетаданных) Тогда
			ФункциональныеОпции.Добавить(ФункциональнаяОпция);
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ФункциональныеОпции;
	
КонецФункции

// Получает статус регистрации для объекта метаданных.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных - Метаданные объекта.
// 
// Возвращаемое значение:
//  Булево
//
Функция ДоступнаРегистрацияОбъекта(ОбъектМетаданных) Экспорт
	
	ДоступнаРегистрацияОбъекта = Ложь;
	
	Для Каждого ПланОбъекта Из Метаданные.ПланыОбмена Цикл
		
		Если ПланОбъекта.Состав.Содержит(ОбъектМетаданных) Тогда
			ДоступнаРегистрацияОбъекта = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ДоступнаРегистрацияОбъекта;
	
КонецФункции

// Возвращает подчиненные метаданные в рамках раздела.
// 
// Параметры:
//  Раздел - Строка
//  АнализПрав - Булево - Режим получения метаданных для анализа прав.
// 
// Возвращаемое значение:
//  Массив Из Строка - Подчиненные метаданные раздела.
//
Функция ПодчиненныеМетаданныеРаздела(Раздел, АнализПрав = Ложь) Экспорт

	СписокМетаданных = Новый Массив;
	
	ДобавлятьКоманды = ИСТИНА;
	
	Если Раздел = "Подсистемы" Тогда
		СписокМетаданных.Добавить("Подсистемы");
		ДобавлятьКоманды = ЛОЖЬ;
	ИначеЕсли Раздел = "КритерииОтбора" Тогда
	ИначеЕсли Раздел = "WebСервисы" Тогда
		СписокМетаданных.Добавить("Операции");
		ДобавлятьКоманды = ЛОЖЬ;
	ИначеЕсли Раздел = "ЖурналыДокументов" Тогда
		СписокМетаданных.Добавить("СтандартныеРеквизиты");
	ИначеЕсли Раздел = "Отчеты" Тогда
		СписокМетаданных.Добавить("Реквизиты");
		СписокМетаданных.Добавить("ТабличныеЧасти");
	ИначеЕсли Раздел = "Обработки" Тогда
		СписокМетаданных.Добавить("Реквизиты");
		СписокМетаданных.Добавить("ТабличныеЧасти");
	ИначеЕсли Раздел = "Справочники" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
	ИначеЕсли Раздел = "Документы" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
	ИначеЕсли Раздел = "Перечисления" Тогда
	ИначеЕсли Раздел = "ПланыВидовХарактеристик" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
	ИначеЕсли Раздел = "ПланыОбмена" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
	ИначеЕсли Раздел = "ПланыСчетов" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
		СписокМетаданных.Вставить(1, "СтандартныеТабличныеЧасти");
		СписокМетаданных.Добавить("ПризнакиУчета");
		СписокМетаданных.Добавить("ПризнакиУчетаСубконто");
	ИначеЕсли Раздел = "ПланыВидовРасчета" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
		СписокМетаданных.Вставить(1, "СтандартныеТабличныеЧасти");
	ИначеЕсли Раздел = "РегистрыСведений" Тогда
		ПодчиненныеМетаданныеРегистра(СписокМетаданных);
	ИначеЕсли Раздел = "РегистрыНакопления" Тогда
		ПодчиненныеМетаданныеРегистра(СписокМетаданных);
	ИначеЕсли Раздел = "РегистрыБухгалтерии" Тогда
		ПодчиненныеМетаданныеРегистра(СписокМетаданных);
	ИначеЕсли Раздел = "РегистрыРасчета" Тогда
		ПодчиненныеМетаданныеРегистра(СписокМетаданных);
	ИначеЕсли Раздел = "БизнесПроцессы" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
	ИначеЕсли Раздел = "Задачи" Тогда
		ПодчиненныеМетаданныеОбъекта(СписокМетаданных);
		СписокМетаданных.Вставить(2, "РеквизитыАдресации");
	ИначеЕсли Раздел = "СтандартныеТабличныеЧасти" Тогда
		СписокМетаданных.Добавить("СтандартныеРеквизиты");
		ДобавлятьКоманды = ЛОЖЬ;
	ИначеЕсли Раздел = "ТабличныеЧасти" Тогда
		СписокМетаданных.Добавить("СтандартныеРеквизиты");
		СписокМетаданных.Добавить("Реквизиты");
		ДобавлятьКоманды = ЛОЖЬ;
	Иначе
		ДобавлятьКоманды = ЛОЖЬ;
	КонецЕсли;
	
	Если ДобавлятьКоманды Тогда
		СписокМетаданных.Добавить("Команды");
	КонецЕсли;
	
	Если Не АнализПрав Тогда
		ДополнительныеПодчиненныеМетаданныеРаздела(Раздел, СписокМетаданных);
	КонецЕсли;
	
	Возврат СписокМетаданных;

КонецФункции

// Получает разделы для инициализации таблицы метаданных
//
// Возвращаемое значение:
//  Массив Из См. ИнициализироватьРаздел
//
Функция ПолучитьРазделы() Экспорт
	
	Разделы = Новый Массив;
	// Общие
	Разделы.Добавить(ИнициализироватьРаздел("Подсистемы",              Истина, Истина));
	Разделы.Добавить(ИнициализироватьРаздел("ПараметрыСеанса",         Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ОбщиеРеквизиты",          Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ПланыОбмена",             Истина, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("КритерииОтбора",          Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("РегламентныеЗадания",     Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ФункциональныеОпции",     Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ХранилищаНастроек",       Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ОбщиеФормы",              Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ОбщиеКоманды",            Истина, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("WebСервисы",              Истина, Ложь));
	// Частные
	Разделы.Добавить(ИнициализироватьРаздел("Константы",               Ложь, Ложь, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("Справочники",             Ложь, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("Документы",               Ложь, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("Последовательности",      Ложь, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ЖурналыДокументов",       Ложь, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("Перечисления",            Ложь, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("Отчеты",                  Ложь, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("Обработки",               Ложь, Ложь));
	Разделы.Добавить(ИнициализироватьРаздел("ПланыВидовХарактеристик", Ложь, Ложь, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("ПланыСчетов",             Ложь, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("ПланыВидовРасчета",       Ложь, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("РегистрыСведений",        Ложь, Ложь, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("РегистрыНакопления",      Ложь, Ложь, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("РегистрыБухгалтерии",     Ложь, Ложь, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("РегистрыРасчета",         Ложь, Ложь, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("БизнесПроцессы",          Ложь, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("Задачи",                  Ложь, Ложь, ИСТИНА, ИСТИНА));
	Разделы.Добавить(ИнициализироватьРаздел("ВнешниеИсточникиДанных",  Ложь, Ложь));
	
	Возврат Разделы;
	
КонецФункции

// Определяет раздел по объекту метаданных.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных - Метаданные объекта.
// 
// Возвращаемое значение:
//  Неопределено, Строка - Имя раздела, которое соответствует объекту метаданных.
//
Функция ПолучитьРазделПоОбъекту(ОбъектМетаданных) Экспорт
	
	Если Метаданные.ПланыОбмена.Содержит(ОбъектМетаданных) Тогда
		Раздел = "ПланыОбмена";
	ИначеЕсли Метаданные.Константы.Содержит(ОбъектМетаданных) Тогда
		Раздел = "Константы";
	ИначеЕсли Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
		Раздел = "Справочники";
	ИначеЕсли Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
		Раздел = "Документы";
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМетаданных) Тогда
		Раздел = "ПланыВидовХарактеристик";
	ИначеЕсли Метаданные.ПланыСчетов.Содержит(ОбъектМетаданных) Тогда
		Раздел = "ПланыСчетов";
	ИначеЕсли Метаданные.ПланыВидовРасчета.Содержит(ОбъектМетаданных) Тогда
		Раздел = "ПланыВидовРасчета";
	ИначеЕсли Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных) Тогда
		Раздел = "РегистрыСведений";
	ИначеЕсли Метаданные.РегистрыНакопления.Содержит(ОбъектМетаданных) Тогда
		Раздел = "РегистрыНакопления";
	ИначеЕсли Метаданные.РегистрыБухгалтерии.Содержит(ОбъектМетаданных) Тогда
		Раздел = "РегистрыБухгалтерии";
	ИначеЕсли Метаданные.РегистрыРасчета.Содержит(ОбъектМетаданных) Тогда
		Раздел = "РегистрыРасчета";
	ИначеЕсли Метаданные.БизнесПроцессы.Содержит(ОбъектМетаданных) Тогда
		Раздел = "БизнесПроцессы";
	ИначеЕсли Метаданные.Задачи.Содержит(ОбъектМетаданных) Тогда
		Раздел = "Задачи";
	ИначеЕсли Метаданные.ВнешниеИсточникиДанных.Содержит(ОбъектМетаданных) Тогда
		Раздел = "ВнешниеИсточникиДанных";
	Иначе
		Раздел = Неопределено;
	КонецЕсли;
	
	Возврат Раздел;
	
КонецФункции

// Проверяет, что подчиненные метаданные описывают поля таблицы.
//
// Параметры:
//  ИмяПодраздела - Строка - Имя подраздела объекта метаданных.
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоПолеДанных(ИмяПодраздела) Экспорт
	
	Возврат
		ЭтоСтандартныйРеквизит(ИмяПодраздела)
		ИЛИ ИмяПодраздела = "Реквизиты"
		ИЛИ ИмяПодраздела = "ПризнакиУчета"
		ИЛИ ИмяПодраздела = "РеквизитыАдресации"
		ИЛИ ЭтоИзмерение(ИмяПодраздела)
		ИЛИ ИмяПодраздела = "Ресурсы";
	
КонецФункции

// Проверяет, что подчиненные метаданные описывают таблицу.
//
// Параметры:
//  ИмяПодраздела - Строка - Имя подраздела объекта метаданных.
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоТаблица(ИмяПодраздела) Экспорт
	
	Возврат ИмяПодраздела = "СтандартныеТабличныеЧасти"
		ИЛИ ИмяПодраздела = "ТабличныеЧасти";
	
КонецФункции

// Проверяет, что подчиненные метаданные описывают стандартный реквизит.
//
// Параметры:
//  ИмяПодраздела - Строка - Имя подраздела объекта метаданных.
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоСтандартныйРеквизит(ИмяПодраздела) Экспорт
	
	Возврат ИмяПодраздела = "СтандартныеРеквизиты";
	
КонецФункции

// Проверяет, что подчиненные метаданные описывают измерение.
//
// Параметры:
//  ИмяПодраздела - Строка - Имя подраздела объекта метаданных.
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоИзмерение(ИмяПодраздела) Экспорт
	
	Возврат ИмяПодраздела = "Измерения";
	
КонецФункции

// Проверяет, полное имя метаданных описывает таблицу для хранения данных.
// 
// Параметры:
//  ПолноеИмя - Строка - Полное имя метаданных
// 
// Возвращаемое значение:
//  Булево
//
Функция ЭтоТаблицаСДанными(ПолноеИмя) Экспорт
	
	Возврат
		ЭтоТаблицаСсылочныхДанных(ПолноеИмя)
	ИЛИ ЭтоТаблицаНабораДанных(ПолноеИмя);
	
КонецФункции

// По пути к метаданным возвращает полное имя метаданных.
// 
// Параметры:
//  ПутьКМетаданным - Строка - Путь к метаданным
// 
// Возвращаемое значение:
//  Строка - Полное имя объекта метаданных
//
Функция ПолучитьПолноеИмяОбъектаМетаданных(ПутьКМетаданным) Экспорт
	
	ЧастиПутиМетаданных = СтрРазделить(ПутьКМетаданным, ".");
	Попытка
		ОбъектМетаданных = Метаданные;
		Для Каждого ЧастьПути Из ЧастиПутиМетаданных Цикл
			ОбъектМетаданных = ОбъектМетаданных[ЧастьПути];
		КонецЦикла;
		ПолноеИмяОбъектаМетаданных = ОбъектМетаданных.ПолноеИмя();
	Исключение
		ПолноеИмяОбъектаМетаданных = "";
	КонецПопытки;
	
	Возврат ПолноеИмяОбъектаМетаданных;
	
КонецФункции

// Проверяет, что поле содержит реальные данные, к которым можно обратиться.
// 
// Параметры:
//  ИмяПоля - Строка
//  ИмяТаблицы - Строка
// 
// Возвращаемое значение:
//  Число - Тип получаемых данных: 1 - данные можно получить, 0 - данные нельзя получить, 2 - получить значения субконто
//
Функция ПроверитьТипДанных(ИмяПоля, ИмяТаблицы) Экспорт
	
	Если ИмяПоля = "ТипЗначения" И ЭтоПланВидовХарактеристик(ИмяТаблицы) Тогда
		Возврат 0;
	КонецЕсли;
	
	Если (Лев(ИмяПоля, 8) = "Субконто" ИЛИ Лев(ИмяПоля, 11) = "ВидСубконто") И ЭтоРегистрБухгалтерии(ИмяТаблицы) Тогда
		Возврат 2;
	КонецЕсли;
	
	Возврат 1;
	
КонецФункции

// Получает метаданные в разрезе разделов.
//
// Возвращаемое значение:
//  Массив Из Структура:
//   * Раздел - Строка
//   * ИмяМетаданных - Строка
//   * ИмяТаблицы - Строка
//
Функция ПолучитьМетаданныеПоРазделам() Экспорт
	
	МетаданныеПоРазделам = Новый Массив;
	
	СписокНеОбъектныхМетаданных = ПолучитьСписокНеОбъектныхМетаданных();
	
	Для Каждого ЭлементСоответствия Из СписокНеОбъектныхМетаданных Цикл
		
		Раздел = ЭлементСоответствия.Ключ;
		
		Для Каждого ЭлементМетаданных Из ЭлементСоответствия.Значение Цикл
			
			ИмяТаблицыМетаданных = ЭлементМетаданных.ПолноеИмя();
			
			СтруктураМетаданных = Новый Структура("Раздел, ИмяМетаданных, ИмяТаблицы");
			СтруктураМетаданных.ИмяМетаданных = ЭлементМетаданных.Имя;
			СтруктураМетаданных.ИмяТаблицы    = ИмяТаблицыМетаданных;
			СтруктураМетаданных.Раздел        = Раздел;
			
			МетаданныеПоРазделам.Добавить(СтруктураМетаданных);
			
		КонецЦикла;
		
	КонецЦикла;
	
	Возврат МетаданныеПоРазделам;
	
КонецФункции

// Получает поля метаданные в разрезе разделов и мени метаданных.
// 
// Параметры:
//  Раздел - Строка
//  ИмяМетаданных - Строка
//  ТолькоСсылочные - Булево
// 
// Возвращаемое значение:
//  Массив Из Структура:
//   * ПолеТаблицы - Строка
//   * Тип - ОписаниеТипов
//   * ПустойТип - Булево
//   * КоличествоСсылочныхТипов - Число
//   * ГруппаМетаданных - Строка
//
Функция ПолучитьПоляМетаданныхПоРазделу(Раздел, ИмяМетаданных, ТолькоСсылочные = Ложь) Экспорт
	
	РеквизитыМетаданныеПоРазделу = Новый Массив;
	
	СписокНеОбъектныхМетаданных = ПолучитьСписокНеОбъектныхМетаданных();
	
	РазделМетаданных = СписокНеОбъектныхМетаданных.Получить(Раздел);
	ЭлементМетаданных = РазделМетаданных.Найти(ИмяМетаданных);
	
	СписокПолейТаблицы = ПолучитьСписокПолейТаблицы(ЭлементМетаданных, Раздел);
	
	Для Каждого ЭлементСоответствия Из СписокПолейТаблицы Цикл
		
		Если ТолькоСсылочные Тогда
			СодержитСсылочныйТип = СодержитСсылочныйТип(ЭлементСоответствия.Значение.Тип);
			Если НЕ СодержитСсылочныйТип Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		СтруктураРеквизита = Новый Структура;
		СтруктураРеквизита.Вставить("ПолеТаблицы");
		СтруктураРеквизита.Вставить("Тип");
		СтруктураРеквизита.Вставить("ПустойТип");
		СтруктураРеквизита.Вставить("КоличествоСсылочныхТипов");
		СтруктураРеквизита.Вставить("ГруппаМетаданных");
		
		ЗаполнитьЗначенияСвойств(СтруктураРеквизита, ЭлементСоответствия.Значение);
		СтруктураРеквизита.ПолеТаблицы = ЭлементСоответствия.Ключ;
		
		РеквизитыМетаданныеПоРазделу.Добавить(СтруктураРеквизита);
		
	КонецЦикла;
	
	Возврат РеквизитыМетаданныеПоРазделу;
	
КонецФункции

// Добавляет ресурсы к реквизитам оборотов остатков.
// 
// Параметры:
//  РеквизитыОборотовОстатков - Массив Из Структура
//  Реквизит - ОбъектМетаданных
//  ВидДвижения - Строка
//
Процедура ДобавитьРесурсыКРеквизитамОборотовОстатков(РеквизитыОборотовОстатков, Реквизит, ВидДвижения) Экспорт
	
	ДобавляемыйРеквизит = Новый Структура();
	Если ВидДвижения = "Обороты" Тогда
		
		ДобавляемыйРеквизит.Вставить("Имя",       Реквизит.Имя + "Оборот");
		ДобавляемыйРеквизит.Вставить("Тип",       Реквизит.Тип);
		ДобавляемыйРеквизит.Вставить("Подраздел", Реквизит.Подраздел);
		ДобавляемыйРеквизит.Вставить("Пароль",    Ложь);
		
		РеквизитыОборотовОстатков.Добавить(ДобавляемыйРеквизит);
		
	ИначеЕсли ВидДвижения = "Итоги" Тогда
		
		ДобавляемыйРеквизит.Вставить("Имя",       Реквизит.Имя + "Остаток");
		ДобавляемыйРеквизит.Вставить("Тип",       Реквизит.Тип);
		ДобавляемыйРеквизит.Вставить("Подраздел", Реквизит.Подраздел);
		ДобавляемыйРеквизит.Вставить("Пароль",    Ложь);
		
		РеквизитыОборотовОстатков.Добавить(ДобавляемыйРеквизит);
		
	КонецЕсли;
	
КонецПроцедуры

// Получает описание объекта объекта метаданных.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных
//  ИмяРаздела - Строка
//  ИнтерфейсныеОбъекты - См. ПолучитьИнтерфейсныеОбъекты
// 
// Возвращаемое значение:
//  Структура: См. ИнициализироватьОписаниеОбъекта
//
Функция ПолучитьОписаниеОбъекта(ОбъектМетаданных, ИмяРаздела, ИнтерфейсныеОбъекты) Экспорт
	
	Если ИнтерфейсныеОбъекты = Неопределено Тогда
		ИнтерфейсныеОбъекты = ПолучитьИнтерфейсныеОбъекты();
	КонецЕсли;
	
	ОписаниеОбъекта = ИнициализироватьОписаниеОбъекта();
	ОписаниеОбъекта.ОбъектМетаданных    = ОбъектМетаданных;
	ОписаниеОбъекта.ПолноеИмя           = ОбъектМетаданных.ПолноеИмя();
	ОписаниеОбъекта.Имя                 = ОбъектМетаданных.Имя;
	ОписаниеОбъекта.Путь                = ИмяРаздела + "." + ОбъектМетаданных.Имя;
	ОписаниеОбъекта.ПутьВЗапросе        = ОписаниеОбъекта.ПолноеИмя;
	ОписаниеОбъекта.Раздел              = ИмяРаздела;
	ОписаниеОбъекта.ЭтоСсылка           = ЭтоСсылочныйОбъект(ОбъектМетаданных);
	ОписаниеОбъекта.ЭтоНаборЗаписей     = ЭтоНаборЗаписей(ОбъектМетаданных);
	ОписаниеОбъекта.ЭтоОсновнаяТаблица  = ЭтоОсновнаяТаблица(ОписаниеОбъекта.ПолноеИмя);
//	ОписаниеОбъекта.ИдентификаторСтроки = ЭлементМетаданных.ПолучитьИдентификатор();
	
	Если ИмяРаздела = "Подсистемы" Тогда
		
		ОписаниеОбъекта.ВключенВИнтерфейс = ОбъектМетаданных.ВключатьВКомандныйИнтерфейс;
		ОписаниеОбъекта.ЭтоПодсистема     = Истина;
		
		Для Каждого ОбъектСостава Из ОбъектМетаданных.Состав Цикл
			ОписаниеОбъектаСостава = ИнициализироватьОписаниеОбъекта();
			ОписаниеОбъектаСостава.ОбъектМетаданных = ОбъектСостава;
			ОписаниеОбъектаСостава.ПолноеИмя        = ОбъектСостава.ПолноеИмя();
			ОписаниеОбъектаСостава.Имя              = ОбъектСостава.Имя;
			ОписаниеОбъекта.Объекты.Добавить(ОписаниеОбъектаСостава);
		КонецЦикла;
		
	ИначеЕсли ИмяРаздела = "ФункциональныеОпции" Тогда
		
		ОписаниеОбъекта.ЭтоФункциональнаяОпция = Истина;
		
		Для Каждого ОбъектСостава Из ОбъектМетаданных.Состав Цикл
			ОписаниеОбъектаСостава = ИнициализироватьОписаниеОбъекта();
			ОписаниеОбъектаСостава.ОбъектМетаданных = ОбъектСостава.Объект;
			ОписаниеОбъектаСостава.ПолноеИмя        = ОбъектСостава.Объект.ПолноеИмя();
			ОписаниеОбъектаСостава.Имя              = ОбъектСостава.Объект.Имя;
			ОписаниеОбъекта.Объекты.Добавить(ОписаниеОбъектаСостава);
		КонецЦикла;
		
	ИначеЕсли ИмяРаздела = "Константы" Тогда
		
		ОписаниеПодчиненногоОбъекта = ИнициализироватьОписаниеОбъекта();
		ОписаниеПодчиненногоОбъекта.Имя    = "Значение";
		ОписаниеПодчиненногоОбъекта.Раздел = "Константы";
		ОписаниеПодчиненногоОбъекта.Путь   =
			ИмяРаздела
			+ "." + ОписаниеОбъекта.Имя
			+ ".Значение";
		ОписаниеПодчиненногоОбъекта.ВключенВИнтерфейс = ОписаниеОбъекта.ВключенВИнтерфейс;
		ОписаниеПодчиненногоОбъекта.ЭтоНаборЗаписей   = Ложь;
		ОписаниеПодчиненногоОбъекта.ПутьВЗапросе      = "Значение";
		ОписаниеПодчиненногоОбъекта.ПолноеИмя         = ОписаниеОбъекта.ПолноеИмя + ".Значение";
		ОписаниеПодчиненногоОбъекта.ЭтоСсылка         = СодержитСсылочныйТип(ОбъектМетаданных.Тип);
		
		ОписаниеОбъекта.Объекты.Добавить(ОписаниеПодчиненногоОбъекта);
		
	Иначе
		
		ОписаниеОбъекта.ВключенВИнтерфейс = Не ИнтерфейсныеОбъекты.Получить(ОписаниеОбъекта.ПолноеИмя) = Неопределено;
		
		ПодчиненныеМетаданныеРаздела = ПодчиненныеМетаданныеРаздела(ИмяРаздела, Истина);
		
		Для Каждого ИмяПодраздела Из ПодчиненныеМетаданныеРаздела Цикл
			
			Для Каждого ПодчиненныеМетаданныеОбъекта Из ОбъектМетаданных[ИмяПодраздела] Цикл
				
				Если ИмяПодраздела = "СтандартныеТабличныеЧасти"
					И ПодчиненныеМетаданныеОбъекта.Имя = "ВидыСубконто"
					И НЕ (
						ЗначениеЗаполнено(ОбъектМетаданных.ВидыСубконто)
						И ОбъектМетаданных.МаксКоличествоСубконто > 0
					) Тогда
					Продолжить;
				КонецЕсли;
				
				ОписаниеПодчиненногоОбъекта = ИнициализироватьОписаниеОбъекта();
				ОписаниеПодчиненногоОбъекта.Имя    = ПодчиненныеМетаданныеОбъекта.Имя;
				ОписаниеПодчиненногоОбъекта.Раздел = ИмяПодраздела;
				ОписаниеПодчиненногоОбъекта.Путь   =
					ИмяРаздела
					+ "." + ОписаниеОбъекта.Имя
					+ "." + ИмяПодраздела
					+ "." + ПодчиненныеМетаданныеОбъекта.Имя;
				ОписаниеПодчиненногоОбъекта.ВключенВИнтерфейс = ОписаниеОбъекта.ВключенВИнтерфейс;
				ОписаниеПодчиненногоОбъекта.ЭтоПолеДанных     = ЭтоПолеДанных(ОписаниеПодчиненногоОбъекта.Раздел);
				ОписаниеПодчиненногоОбъекта.ЭтоТаблица        = ЭтоТаблица(ИмяПодраздела);
				ОписаниеПодчиненногоОбъекта.ЭтоСтандартныйРеквизит = ЭтоСтандартныйРеквизит(ИмяПодраздела);
				ОписаниеПодчиненногоОбъекта.ЭтоИзмерение      = ЭтоИзмерение(ИмяПодраздела);
				Если ОписаниеПодчиненногоОбъекта.ЭтоТаблица Тогда
					ОписаниеПодчиненногоОбъекта.ПутьВЗапросе = ОписаниеОбъекта.ПолноеИмя + "." + ОписаниеПодчиненногоОбъекта.Имя;
				Иначе
					ОписаниеПодчиненногоОбъекта.ПутьВЗапросе = ОписаниеПодчиненногоОбъекта.Имя;
				КонецЕсли;
				
				Если Лев(ИмяПодраздела, 11) = "Стандартные" Тогда
					ОписаниеПодчиненногоОбъекта.ПутьКСтандартномуРеквизиту = ПодчиненныеМетаданныеОбъекта.Имя;
					ОписаниеПодчиненногоОбъекта.ПолноеИмя                  = ОписаниеОбъекта.ПолноеИмя + "." + ОписаниеПодчиненногоОбъекта.ПутьКСтандартномуРеквизиту;
				Иначе
					ОписаниеПодчиненногоОбъекта.ПутьКСтандартномуРеквизиту = Неопределено;
					ОписаниеПодчиненногоОбъекта.ПолноеИмя                  = ПодчиненныеМетаданныеОбъекта.ПолноеИмя();
				КонецЕсли;
				ОписаниеПодчиненногоОбъекта.ОбъектМетаданных         = ПодчиненныеМетаданныеОбъекта;
				ОписаниеПодчиненногоОбъекта.ОбъектМетаданныхРодитель = ОбъектМетаданных;
				
				Если ОписаниеОбъекта.ЭтоОсновнаяТаблица
					 И Не ИмяПодраздела = "Команды"
					 И НЕ ОписаниеПодчиненногоОбъекта.ЭтоТаблица Тогда
					ОписаниеПодчиненногоОбъекта.ЭтоСсылка           = СодержитСсылочныйТип(ПодчиненныеМетаданныеОбъекта.Тип);
					Если ОписаниеПодчиненногоОбъекта.ЭтоПолеДанных Тогда
						ОписаниеПодчиненногоОбъекта.ЭтоРедактируемыйТип = ОписаниеПодчиненногоОбъекта.ЭтоСсылка
							ИЛИ СодержитТипОтображения(ПодчиненныеМетаданныеОбъекта.Тип) = 2;
						ОписаниеПодчиненногоОбъекта.ЭтоТипДляПросмотра = ОписаниеПодчиненногоОбъекта.ЭтоРедактируемыйТип
							ИЛИ СодержитТипОтображения(ПодчиненныеМетаданныеОбъекта.Тип) = 1;
					КонецЕсли;
				КонецЕсли;
				
				ОписаниеОбъекта.Объекты.Добавить(ОписаниеПодчиненногоОбъекта);
				
				ПодчиненныеМетаданныеТабличныхЧастей = ПодчиненныеМетаданныеРаздела(ИмяПодраздела, Истина);
				
				Для Каждого ПодчиненныеМетаданныеТабличнойЧасти Из ПодчиненныеМетаданныеТабличныхЧастей Цикл
					
					Для Каждого РеквизитТабличнойЧасти Из ПодчиненныеМетаданныеОбъекта[ПодчиненныеМетаданныеТабличнойЧасти] Цикл
						
						ОписаниеПодчиненногоОбъектаТЧ = ИнициализироватьОписаниеОбъекта();
						ОписаниеПодчиненногоОбъектаТЧ.Имя    = РеквизитТабличнойЧасти.Имя;
						ОписаниеПодчиненногоОбъектаТЧ.Раздел = ПодчиненныеМетаданныеТабличнойЧасти;
						ОписаниеПодчиненногоОбъектаТЧ.Путь   =
							ИмяРаздела
							+ "." + ОписаниеОбъекта.Имя
							+ "." + ИмяПодраздела
							+ "." + ПодчиненныеМетаданныеОбъекта.Имя
							+ "." + ПодчиненныеМетаданныеТабличнойЧасти
							+ "." + РеквизитТабличнойЧасти.Имя;
						ОписаниеПодчиненногоОбъектаТЧ.ПутьВЗапросе      = ОписаниеПодчиненногоОбъектаТЧ.Имя;
						ОписаниеПодчиненногоОбъектаТЧ.ВключенВИнтерфейс = ОписаниеОбъекта.ВключенВИнтерфейс;
						ОписаниеПодчиненногоОбъектаТЧ.ЭтоПолеДанных     = ЭтоПолеДанных(ОписаниеПодчиненногоОбъектаТЧ.Раздел);
						
						Если Лев(ПодчиненныеМетаданныеТабличнойЧасти, 11) = "Стандартные" Тогда
							Если Лев(ИмяПодраздела, 11) = "Стандартные" Тогда
								ОписаниеПодчиненногоОбъектаТЧ.ПутьКСтандартномуРеквизиту = ПодчиненныеМетаданныеОбъекта.Имя + "." + РеквизитТабличнойЧасти.Имя;
								ОписаниеПодчиненногоОбъектаТЧ.ПолноеИмя                  = ОписаниеОбъекта.ПолноеИмя + "." + ОписаниеПодчиненногоОбъектаТЧ.ПутьКСтандартномуРеквизиту;
								ОписаниеПодчиненногоОбъектаТЧ.ОбъектМетаданныхРодитель   = ОбъектМетаданных;
							Иначе
								ОписаниеПодчиненногоОбъектаТЧ.ПутьКСтандартномуРеквизиту = РеквизитТабличнойЧасти.Имя;
								ОписаниеПодчиненногоОбъектаТЧ.ПолноеИмя                  = ПодчиненныеМетаданныеОбъекта.ПолноеИмя() + "." + ОписаниеПодчиненногоОбъектаТЧ.ПутьКСтандартномуРеквизиту;
								ОписаниеПодчиненногоОбъектаТЧ.ОбъектМетаданныхРодитель   = ПодчиненныеМетаданныеОбъекта;
							КонецЕсли;
						Иначе
							ОписаниеПодчиненногоОбъектаТЧ.ПутьКСтандартномуРеквизиту = Неопределено;
							ОписаниеПодчиненногоОбъектаТЧ.ПолноеИмя                  = РеквизитТабличнойЧасти.ПолноеИмя();
							ОписаниеПодчиненногоОбъектаТЧ.ОбъектМетаданныхРодитель   = ПодчиненныеМетаданныеОбъекта;
						КонецЕсли;
						
						Если Не ИмяПодраздела = "Команды" Тогда
							ОписаниеПодчиненногоОбъектаТЧ.ЭтоСсылка           = СодержитСсылочныйТип(РеквизитТабличнойЧасти.Тип);
							Если ОписаниеПодчиненногоОбъектаТЧ.ЭтоПолеДанных Тогда
								ОписаниеПодчиненногоОбъектаТЧ.ЭтоРедактируемыйТип = ОписаниеПодчиненногоОбъектаТЧ.ЭтоСсылка ИЛИ СодержитТипОтображения(РеквизитТабличнойЧасти.Тип) = 2;
								ОписаниеПодчиненногоОбъектаТЧ.ЭтоТипДляПросмотра  = ОписаниеПодчиненногоОбъектаТЧ.ЭтоРедактируемыйТип ИЛИ СодержитТипОтображения(РеквизитТабличнойЧасти.Тип) = 1;
							КонецЕсли;
						КонецЕсли;
						
						ОписаниеПодчиненногоОбъектаТЧ.ОбъектМетаданных = РеквизитТабличнойЧасти;
						
						ОписаниеПодчиненногоОбъекта.Объекты.Добавить(ОписаниеПодчиненногоОбъектаТЧ);
						
					КонецЦикла;
					
				КонецЦикла;
				
			КонецЦикла;
			
		КонецЦикла;
		
	КонецЕсли;
	
	Возврат ОписаниеОбъекта;
	
КонецФункции

// Получает список свойств характеристик.
// 
// Возвращаемое значение:
//  Массив Из Строка
//
Функция ПолучитьСписокСвойствХарактеристик() Экспорт
	
	СписокСвойствХарактеристик = Новый Массив();
	
	СписокСвойствХарактеристик.Добавить("ВидыХарактеристик");
	СписокСвойствХарактеристик.Добавить("ПолеКлюча");
	СписокСвойствХарактеристик.Добавить("ПолеОтбораВидов");
	СписокСвойствХарактеристик.Добавить("ЗначениеОтбораВидов");
	СписокСвойствХарактеристик.Добавить("ЗначенияХарактеристик");
	СписокСвойствХарактеристик.Добавить("ПолеОбъекта");
	СписокСвойствХарактеристик.Добавить("ПолеВида");
	СписокСвойствХарактеристик.Добавить("ПолеЗначения");
	
	Возврат СписокСвойствХарактеристик;
	
КонецФункции

// Получает раздел по полному имени.
// 
// Параметры:
//  ПолноеИмя - Строка - полное имя объекта метаданных.
// 
// Возвращаемое значение:
//  Строка - Раздел.
//
Функция ПолучитьРазделПоПолномуИмени(ПолноеИмя) Экспорт
	
	Если Лев(ПолноеИмя, 9)       = "Константа" Тогда
		Возврат "Константы";
	ИначеЕсли Лев(ПолноеИмя, 10) = "ПланОбмена" Тогда
		Возврат "ПланыОбмена";
	ИначеЕсли Лев(ПолноеИмя, 10) = "Справочник" Тогда
		Возврат "Справочники";
	ИначеЕсли Лев(ПолноеИмя, 8)  = "Документ" Тогда
		Возврат "Документы";
	ИначеЕсли Лев(ПолноеИмя, 12) = "Перечисление" Тогда
		Возврат "Перечисления";
	ИначеЕсли ЭтоПланВидовХарактеристик(ПолноеИмя) Тогда
		Возврат "ПланыВидовХарактеристик";
	ИначеЕсли Лев(ПолноеИмя, 10) = "ПланСчетов" Тогда
		Возврат "ПланыСчетов";
	ИначеЕсли Лев(ПолноеИмя, 16) = "ПланВидовРасчета" Тогда
		Возврат "ПланыВидовРасчета";
	ИначеЕсли Лев(ПолноеИмя, 15) = "РегистрСведений" Тогда
		Возврат "РегистрыСведений";
	ИначеЕсли Лев(ПолноеИмя, 17) = "РегистрНакопления" Тогда
		Возврат "РегистрыНакопления";
	ИначеЕсли Лев(ПолноеИмя, 18) = "РегистрБухгалтерии" Тогда
		Возврат "РегистрыБухгалтерии";
	ИначеЕсли Лев(ПолноеИмя, 14) = "РегистрРасчета" Тогда
		Возврат "РегистрыРасчета";
	ИначеЕсли Лев(ПолноеИмя, 13) = "БизнесПроцесс" Тогда
		Возврат "БизнесПроцессы";
	ИначеЕсли Лев(ПолноеИмя, 6) =  "Задача" Тогда
		Возврат "Задачи";
	Иначе
		Возврат "";
	КонецЕсли;
	
КонецФункции

// Проверяет, что тип ссылочны
// 
// Параметры:
//  Тип - Тип
//  ПолучитьУИД - Булево - Проверяет что тип ссылочный, и для него можно получить УИД
// 
// Возвращаемое значение:
//  Булево
//
Функция ЭтоСсылка(Тип, ПолучитьУИД = Ложь) Экспорт
	
	Возврат
		Не ЭтоПростойТип(Тип)
		И (Справочники.ТипВсеСсылки().СодержитТип(Тип)
		ИЛИ Документы.ТипВсеСсылки().СодержитТип(Тип)
		Или Не ПолучитьУИД И Перечисления.ТипВсеСсылки().СодержитТип(Тип)
		ИЛИ ПланыВидовХарактеристик.ТипВсеСсылки().СодержитТип(Тип)
		ИЛИ ПланыСчетов.ТипВсеСсылки().СодержитТип(Тип)
		ИЛИ ПланыВидовРасчета.ТипВсеСсылки().СодержитТип(Тип)
		ИЛИ ПланыОбмена.ТипВсеСсылки().СодержитТип(Тип)
		ИЛИ БизнесПроцессы.ТипВсеСсылки().СодержитТип(Тип)
		ИЛИ Не ПолучитьУИД И БизнесПроцессы.ТипВсеСсылкиТочекМаршрутаБизнесПроцессов().СодержитТип(Тип)
		ИЛИ Задачи.ТипВсеСсылки().СодержитТип(Тип));
	
КонецФункции

// Получает имя элементы объекта метаданных для запроса.
//
// Параметры:
//  ЭлементОбъектаМетаданных - Структура - Элемент объекта метаданных
//  Приставка - Строка - Приставка к имени элемента объекта метаданных
//
// Возвращаемое значение:
//  Строка
//
Функция ПолучитьИмяЭлементаОбъектаМетаданных(ЭлементОбъектаМетаданных, Приставка = "") Экспорт
	
	Если ПроверитьСуществованиеСвойства("Балансовый", ЭлементОбъектаМетаданных) И НЕ ЭлементОбъектаМетаданных.Балансовый Тогда
		ИмяПоля = ЭлементОбъектаМетаданных.Имя + Приставка;
	ИначеЕсли Лев(ЭлементОбъектаМетаданных.Имя, 8) = "Субконто" Тогда
		ИмяПоля = "Субконто" + Приставка + Сред(ЭлементОбъектаМетаданных.Имя, 9);
	ИначеЕсли Лев(ЭлементОбъектаМетаданных.Имя, 11) = "ВидСубконто" Тогда
		ИмяПоля = "ВидСубконто" + Приставка + Сред(ЭлементОбъектаМетаданных.Имя, 12);
	Иначе
		ИмяПоля = ЭлементОбъектаМетаданных.Имя;
	КонецЕсли;
	
	Возврат ИмяПоля;
	
КонецФункции

// Получает форму списка по имени объекта метаданных.
//
// Параметры:
//  ПолноеИмя - Строка - Полное имя объекта метаданных.
//  ТипФормы - Строка  - Тип открываемой формы.
//  ЭтоГруппа - Булево - Признак того, что открываемый элемент группа справочника.
//
// Возвращаемое значение:
//  Строка - Форма списка
//
Функция ПолучитьФормуПоОбъектуМетаданных(ПолноеИмя, ТипФормы, ЭтоГруппа = Ложь) Экспорт
	
	Если СтрНачинаетсяС(ПолноеИмя, "ФункциональныеОпции") Тогда
		ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ПолноеИмя);
		ЧастиОбъектаМетаданных = СтрРазделить(ОбъектМетаданных.Хранение.ПолноеИмя(), ".");
	Иначе
		ЧастиОбъектаМетаданных = СтрРазделить(ПолноеИмя, ".");
	КонецЕсли;
	
	ЭтоСправочник = ПолучитьРазделПоПолномуИмени(ПолноеИмя) = "Справочники";
	
	Если ТипФормы = "ФормаСписка" Тогда
		ИмяФормы = 
			ЧастиОбъектаМетаданных[0] + "."
			+ ЧастиОбъектаМетаданных[1] + "."
			+ ?(ЧастиОбъектаМетаданных[0] = "Константа", "ФормаКонстант", "ФормаСписка");
	ИначеЕсли ТипФормы = "ФормаРедактирования" Тогда
		ИмяФормы = 
			ЧастиОбъектаМетаданных[0] + "."
			+ ЧастиОбъектаМетаданных[1] + "."
			+ ?(
				ЧастиОбъектаМетаданных[0] = "Константа",
				"ФормаКонстант", 
				?(
					ЭтоСправочник и ЭтоГруппа,
					"ФормаГруппы",
					?(
						ЭтоТаблицаСсылочныхДанных(ПолноеИмя),
						"ФормаОбъекта",
						"ФормаЗаписи"
					)
				)
			);
	Иначе
		ИмяФормы = "";
	КонецЕсли;
	
	Возврат ИмяФормы;
	
КонецФункции

// Получает полное имя метаданных по типу.
//
// Параметры:
//  Тип - Тип - Тип данных.
//
// Возвращаемое значение:
//  Строка - Полное имя объекта метаданных по типу
//
Функция ПолучитьПолноеИмяМетаданныхПоТипу(Тип) Экспорт
	
	Если КэшИменМетаданныхПоТипу = Неопределено Тогда
		КэшИменМетаданныхПоТипу = Новый Соответствие;
	КонецЕсли;
	
	ПолноеИмя = КэшИменМетаданныхПоТипу.Получить(Тип);
	Если ПолноеИмя = Неопределено Тогда
		ОбъектМетаданных = Метаданные.НайтиПоТипу(Тип);
		Если Не ОбъектМетаданных = Неопределено Тогда
			ПолноеИмя = ОбъектМетаданных.ПолноеИмя();
			Если Прав(Тип, 16) = "(точка маршрута)" Тогда
				ПолноеИмя = ПолноеИмя + ".Точки";
			КонецЕсли;
		КонецЕсли;
		КэшИменМетаданныхПоТипу.Вставить(Тип, ПолноеИмя);
	КонецЕсли;
	
	Возврат ПолноеИмя;
	
КонецФункции

// Получает Менеджер объекта по полному имени.
// 
// Параметры:
//  ПолноеИмя - Строка - Полное имя объекта метаданных.
// 
// Возвращаемое значение:
//  - ПланыОбменаМенеджер
//  - КонстантыМенеджер
//  - СправочникиМенеджер
//  - ДокументыМенеджер
//  - ПеречисленияМенеджер
//  - ПланыВидовХарактеристикМенеджер
//  - ПланыСчетовМенеджер
//  - ПланыВидовРасчетаМенеджер
//  - БизнесПроцессыМенеджер
//  - ЗадачиМенеджер
//  - РегистрыСведенийМенеджер
//  - РегистрыНакопленияМенеджер
//  - РегистрыБухгалтерииМенеджер
//  - РегистрыРасчетаМенеджер
//  - Неопределено
//
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя) Экспорт
	
	ЧастиИмени = РазложитьСтрокуВМассивПодстрок(ПолноеИмя);
	
	Если ЧастиИмени.Количество() >= 2 Тогда
		КлассОМ = ЧастиИмени[0];
		ИмяОМ = ЧастиИмени[1];
	КонецЕсли;
	
	Если КлассОМ = "ПланОбмена" Тогда
		Менеджер = ПланыОбмена;
	ИначеЕсли КлассОМ = "Константа" Тогда
		Менеджер = Константы;
	ИначеЕсли КлассОМ = "Справочник" Тогда
		Менеджер = Справочники;
	ИначеЕсли КлассОМ = "Документ" Тогда
		Менеджер = Документы;
	ИначеЕсли КлассОМ = "Перечисление" Тогда
		Менеджер = Перечисления;
	ИначеЕсли КлассОМ = "ПланВидовХарактеристик" Тогда
		Менеджер = ПланыВидовХарактеристик;
	ИначеЕсли КлассОМ = "ПланСчетов" Тогда
		Менеджер = ПланыСчетов;
	ИначеЕсли КлассОМ = "ПланВидовРасчета" Тогда
		Менеджер = ПланыВидовРасчета;
	ИначеЕсли КлассОМ = "БизнесПроцесс" Тогда
		Менеджер = БизнесПроцессы;
	ИначеЕсли КлассОМ = "Задача" Тогда
		Менеджер = Задачи;
	ИначеЕсли КлассОМ = "РегистрСведений" Тогда
		Менеджер = РегистрыСведений;
	ИначеЕсли КлассОМ = "РегистрНакопления" Тогда
		Менеджер = РегистрыНакопления;
	ИначеЕсли КлассОМ = "РегистрБухгалтерии" Тогда
		Менеджер = РегистрыБухгалтерии;
	ИначеЕсли КлассОМ = "РегистрРасчета" Тогда
		Менеджер = РегистрыРасчета;
	Иначе
		Менеджер = Неопределено;
	КонецЕсли;
	
	Если Менеджер = Неопределено Тогда
		Возврат Менеджер;
	Иначе
		Возврат Менеджер[ИмяОМ];
	КонецЕсли;
	
КонецФункции

#КонецОбласти

#Область РаботаСПравамиДоступа

// Проверяет права доступа к объекту метаданных.
//
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданных
//  ПутьКОбъектуМетаданных - Строка
//  ПутьКСтандартномуРеквизиту - Строка
//  РольПользователь - Строка
//  ПраваРаздела - Массив Из Строка - См. ПраваРаздела - Список прав, на который проверяется объект метаданных.
//  ПраваОбъектаМетаданных - ТаблицаЗначений
//
Процедура ПроверитьПраваДоступа(
		ОбъектМетаданных, ПутьКОбъектуМетаданных, ПутьКСтандартномуРеквизиту,
		РольПользователь, ПраваРаздела, ПраваОбъектаМетаданных
	) Экспорт
	
	Для каждого Право Из ПраваРаздела Цикл
		
		Строка                  = ПраваОбъектаМетаданных.Добавить();
		Строка.Реквизит         = ПутьКОбъектуМетаданных;
		Строка.РольПользователь = РольПользователь.Имя;
		Строка.Право            = Право;
		Строка.Использование    = ПравоДоступа(Право, ОбъектМетаданных, РольПользователь, ПутьКСтандартномуРеквизиту);
		
		Если ПутьКСтандартномуРеквизиту <> Неопределено Тогда
			Строка.Объект = ПутьКСтандартномуРеквизиту;
		Иначе
			Строка.Объект = ОбъектМетаданных.Имя;
		КонецЕсли;
		
	КонецЦикла;
	
КонецПроцедуры

// Добавляет строку прав реквизитов объектов метаданных.
// 
// Параметры:
//  ТаблицаПрав - ТаблицаЗначений
//  ОбъектыМетаданныхРолей - ТаблицаЗначений
//  ОбъектыМетаданныхПользователей - ТаблицаЗначений
//  РежимРаботы - Строка - Режим анализа прав: Роли - по ролям, Пользователи - по пользователям.
//  АнализироватьТолькоИнтерфейсныеОбъекты - Булево
//
Процедура ДобавитьСтрокуПравРеквизитовОбъектовМетаданных(ТаблицаПрав, ОбъектыМетаданныхРолей, ОбъектыМетаданныхПользователей, РежимРаботы, АнализироватьТолькоИнтерфейсныеОбъекты = Ложь) Экспорт
	
	Для Каждого Строка Из ТаблицаПрав Цикл
		
		Если АнализироватьТолькоИнтерфейсныеОбъекты И
			НЕ Строка.ВключенВИнтерфейс Тогда
			Продолжить;
		КонецЕсли;
			
		Если РежимРаботы = "Роли" Тогда
			НоваяСтрока = ОбъектыМетаданныхРолей.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
			НоваяСтрока.АнализироватьТолькоИнтерфейсныеОбъекты = АнализироватьТолькоИнтерфейсныеОбъекты;
		Иначе
			НоваяСтрока =  ОбъектыМетаданныхПользователей.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
		КонецЕсли;
		
	КонецЦикла;
	
КонецПроцедуры

// Проверяет права доступа по ролям пользователям.
// 
// Параметры:
//  РольПользователь - ОбъектМетаданныхРоль
//  ПраваРаздела - Массив Из Строка
//  ОписаниеОбъекта - См. ИнициализироватьОписаниеОбъекта
//  ПраваОбъектовМетаданных- ТаблицаЗначений
//  РежимРаботы - Строка - Режим анализа прав: Роли - по ролям, Пользователи - по пользователям.
//
Процедура ПроверитьПраваДоступаПоРолямПользователям(РольПользователь, ПраваРаздела, ОписаниеОбъекта, ПраваОбъектовМетаданных, РежимРаботы) Экспорт
	
	Если Не ЗначениеЗаполнено(ОписаниеОбъекта.ПутьКСтандартномуРеквизиту) Тогда
		СтандартныйРеквизит = Неопределено;
		ОбъектМетаданных    = ОписаниеОбъекта.ОбъектМетаданных;
	Иначе
		СтандартныйРеквизит = ОписаниеОбъекта.ПутьКСтандартномуРеквизиту;
		ОбъектМетаданных    = ОписаниеОбъекта.ОбъектМетаданныхРодитель;
	КонецЕсли;
	
	Для Каждого Право Из ПраваРаздела Цикл
		
		Строка = ПраваОбъектовМетаданных.Добавить();
		Строка.ИмяОбъекта        = ОписаниеОбъекта.Путь;
		Строка.Право             = Право;
		Строка.Использование     = ПравоДоступа(Право, ОбъектМетаданных, РольПользователь, СтандартныйРеквизит);
		Строка.ВключенВИнтерфейс = ОписаниеОбъекта.ВключенВИнтерфейс;
		
		Если РежимРаботы = "Роли" Тогда
			Строка.Роль         = РольПользователь.Имя;
		ИначеЕсли РежимРаботы = "Пользователи" Тогда
			Строка.Пользователь = РольПользователь.Имя;
		КонецЕсли;
		
	КонецЦикла;
	
КонецПроцедуры

// Сравнивает права доступа объекта метаданных и его реквизитов.
// 
// Параметры:
//  ПраваОбъектовМетаданных - ТаблицаЗначений
//  ПраваРеквизитовОбъектовМетаданных- ТаблицаЗначений
// 
// Возвращаемое значение:
//  Булево - Права доступа совпадают
//
Функция СравнитьПраваДоступа(ПраваОбъектовМетаданных, ПраваРеквизитовОбъектовМетаданных) Экспорт
	
	Для Каждого ПраваРеквизитаОбъектаМетаданных Из ПраваРеквизитовОбъектовМетаданных Цикл
		Если ПраваОбъектовМетаданных.НайтиСтроки(
				Новый Структура(
					"Право, Использование",
					ПраваРеквизитаОбъектаМетаданных.Право,
					ПраваРеквизитаОбъектаМетаданных.Использование
				)
			).Количество() = 0 Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
	
	Возврат Истина;
	
КонецФункции

// Возвращает список прав, доступных для раздела.
// 
// Параметры:
//  Раздел - Строка
// 
// Возвращаемое значение:
//  Массив Из Строка
//
Функция ПраваРаздела(Раздел) Экспорт
	
	СписокПравРаздела = Новый Массив;
	
	Если Раздел = "Подсистемы" Тогда
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "ПараметрыСеанса" Тогда
		СписокПравРаздела.Добавить("Получение");
		СписокПравРаздела.Добавить("Установка");
	ИначеЕсли Раздел = "ОбщиеРеквизиты" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "ПланыОбмена" Тогда
		ПраваСправочника(СписокПравРаздела);
	ИначеЕсли Раздел = "КритерииОтбора" Тогда
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "ОбщиеФормы" Тогда
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "ОбщиеКоманды" Тогда
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "Интерфейсы" Тогда
		СписокПравРаздела.Добавить("Использование");
	ИначеЕсли Раздел = "WebСервисы" Тогда
		СписокПравРаздела.Добавить("Использование");
	ИначеЕсли Раздел = "Константы" Тогда
		ПраваРегистра(СписокПравРаздела);
	ИначеЕсли Раздел = "Справочники" Тогда
		ПраваСправочника(СписокПравРаздела);
	ИначеЕсли Раздел = "Документы" Тогда
		ПраваСправочника(СписокПравРаздела);
		ПраваДокумента(СписокПравРаздела);
	ИначеЕсли Раздел = "Последовательности" Тогда
		СписокПравРаздела.Добавить("Чтение");
		СписокПравРаздела.Добавить("Изменение");
	ИначеЕсли Раздел = "ЖурналыДокументов" Тогда
		СписокПравРаздела.Добавить("Чтение");
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "Отчеты" Тогда
		СписокПравРаздела.Добавить("Использование");
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "Обработки" Тогда
		СписокПравРаздела.Добавить("Использование");
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "ПланыВидовХарактеристик" Тогда
		ПраваСправочника(СписокПравРаздела);
	ИначеЕсли Раздел = "ПланыСчетов" Тогда
		ПраваСправочника(СписокПравРаздела);
	ИначеЕсли Раздел = "ПланыВидовРасчета" Тогда
		ПраваСправочника(СписокПравРаздела);
	ИначеЕсли Раздел = "РегистрыСведений" Тогда
		ПраваРегистра(СписокПравРаздела);
	ИначеЕсли Раздел = "РегистрыНакопления" Тогда
		ПраваРегистра(СписокПравРаздела);
		СписокПравРаздела.Добавить("УправлениеИтогами");
	ИначеЕсли Раздел = "РегистрыБухгалтерии" Тогда
		ПраваРегистра(СписокПравРаздела);
		СписокПравРаздела.Добавить("УправлениеИтогами");
	ИначеЕсли Раздел = "РегистрыРасчета" Тогда
		ПраваРегистра(СписокПравРаздела);
	ИначеЕсли Раздел = "БизнесПроцессы" Тогда
		ПраваСправочника(СписокПравРаздела);
		СписокПравРаздела.Добавить("ИнтерактивнаяАктивация");
		СписокПравРаздела.Добавить("Старт");
		СписокПравРаздела.Добавить("ИнтерактивныйСтарт");
	ИначеЕсли Раздел = "Задачи" Тогда
		ПраваСправочника(СписокПравРаздела);
		СписокПравРаздела.Добавить("ИнтерактивнаяАктивация");
		СписокПравРаздела.Добавить("Выполнение");
		СписокПравРаздела.Добавить("ИнтерактивноеВыполнение");
	ИначеЕсли Раздел = "ВнешниеИсточникиДанных" Тогда
		СписокПравРаздела.Добавить("Использование");
		СписокПравРаздела.Добавить("Администрирование");
		СписокПравРаздела.Добавить("ИзменениеСтандартнойАутентификации");
		СписокПравРаздела.Добавить("ИзменениеСтандартнойАутентификацииСеанса");
		СписокПравРаздела.Добавить("ИзменениеАутентификацииОССеанса");
	ИначеЕсли Раздел = "СтандартныеРеквизиты" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "СтандартныеТабличныеЧасти" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "Реквизиты" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "РеквизитыНаПросмотр" Тогда
		СписокПравРаздела.Добавить("Просмотр");
	ИначеЕсли Раздел = "ТабличныеЧасти" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "РеквизитыАдресации" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "ПризнакиУчета" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "ПризнакиУчетаСубконто" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "Измерения" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "Ресурсы" Тогда
		СписокПравРаздела.Добавить("Просмотр");
		СписокПравРаздела.Добавить("Редактирование");
	ИначеЕсли Раздел = "Перерасчеты" Тогда
		СписокПравРаздела.Добавить("Чтение");
		СписокПравРаздела.Добавить("Изменение");
	ИначеЕсли Раздел = "Операции" Тогда
		СписокПравРаздела.Добавить("Использование");
	ИначеЕсли Раздел = "Команды" Тогда
		СписокПравРаздела.Добавить("Просмотр");
	КонецЕсли;
	
	Возврат СписокПравРаздела;

КонецФункции

#КонецОбласти

#Область РаботаСоСтатистикойДанных

// Получает статистику таблицы.
// 
// Параметры:
//  ИмяТаблицы - Строка
// 
// Возвращаемое значение:
//  Структура - Получить статистику таблицы:
//   * КоличествоЭлементов - Число
//   * КоличествоПредопределенных - Число
//   * Выключен - Булево - Поле таблицы выключено через функциональную опцию.
//
Функция ПолучитьСтатистикуТаблицы(ИмяТаблицы) Экспорт
	
	Результат = Новый Структура("КоличествоЭлементов, КоличествоПредопределенных, Выключен", 0, 0, Ложь);
	
	Если ПустаяСтрока(ИмяТаблицы) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ИмеютсяПредопределенные = ЭтоТаблицаПредопределенныхДанных(ИмяТаблицы);
	ЭтоКонстанта            = ЭтоКонстанта(ИмяТаблицы);
	ЭтоТаблицаСДанными      = ЭтоТаблицаСДанными(ИмяТаблицы);
	
	Если НЕ ЭтоТаблицаСДанными И НЕ ЭтоКонстанта Тогда
		Возврат Результат;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если ЭтоКонстанта Тогда
		ШаблонТекстаЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаДанных.Значение КАК Значение,
		|	0 КАК КоличествоПредопределенных
		|ИЗ
		|	&ИмяТаблицы КАК ТаблицаДанных";
	Иначе
		ШаблонТекстаЗапроса = 
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(*) КАК КоличествоЭлементов,
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|		КОГДА &Предопределенный
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ), 0) КАК КоличествоПредопределенных
		|ИЗ
		|	&ИмяТаблицы КАК ТаблицаДанных";
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&ИмяТаблицы", ИмяТаблицы);
	Если ИмеютсяПредопределенные Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Предопределенный", "ТаблицаДанных.Предопределенный");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Предопределенный", "ЛОЖЬ");
	КонецЕсли;
	
	Запрос  = Новый Запрос(ТекстЗапроса);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() Тогда
		Если ЭтоКонстанта Тогда
			Если ТипЗнч(Выборка.Значение) = Тип("Булево") Тогда
				Результат.КоличествоЭлементов = 1;
			ИначеЕсли ТипЗнч(Выборка.Значение) = Тип("ХранилищеЗначения") Тогда
				Результат.КоличествоЭлементов = ЗначениеЗаполнено(Выборка.Значение.Получить());
			Иначе
				Результат.КоличествоЭлементов = ЗначениеЗаполнено(Выборка.Значение);
			КонецЕсли;
		Иначе
			ЗаполнитьЗначенияСвойств(Результат, Выборка);
		КонецЕсли;
	КонецЕсли;
	
	ЭлементОбъектовМетаданных = Новый Структура("ПолноеИмя, Раздел", ИмяТаблицы, "");
	СхемаКомпоновкиДанных = ПодготовитьСхемуКомпоновкиДанных(ЭлементОбъектовМетаданных, "ПЕРВЫЕ 1 *");
	НастройкиКомпоновкиДанных = Новый КомпоновщикНастроекКомпоновкиДанных;
	ЗагрузитьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, Неопределено, Новый УникальныйИдентификатор);
	Результат.Выключен = Результат.КоличествоЭлементов > 0 И Не ПроверитьДоступностьДанныхВТаблице(ИмяТаблицы);
	
	Возврат Результат;
	
КонецФункции

// Проверяет доступность данных в таблице через функциональные опции.
// 
// Параметры:
//  ИмяТаблицы - Строка
// 
// Возвращаемое значение:
//  Булево
//
Функция ПроверитьДоступностьДанныхВТаблице(ИмяТаблицы) Экспорт
	
	ЭлементОбъектовМетаданных = Новый Структура("ПолноеИмя, Раздел", ИмяТаблицы, "");
	СхемаКомпоновкиДанных = ПодготовитьСхемуКомпоновкиДанных(ЭлементОбъектовМетаданных, "ПЕРВЫЕ 1 *");
	НастройкиКомпоновкиДанных = Новый КомпоновщикНастроекКомпоновкиДанных;
	ЗагрузитьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, Неопределено, Новый УникальныйИдентификатор);
	Данные = ВывестиСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, Истина);
	ДанныеДоступны = ЗначениеЗаполнено(Данные);
	Данные.Очистить();
	
	Возврат ДанныеДоступны;
	
КонецФункции

// Получает статистику заполненности полей ВВТаблице.
// 
// Параметры:
//  ИмяТаблицы - Строка
//  РеквизитыОбъекта - Массив Из ОбъектМетаданных
// 
// Возвращаемое значение:
//  ВыборкаИзРезультатаЗапроса
//
Функция ПолучитьСтатистикуЗаполненностиПолейВВТаблице(Знач ИмяТаблицы, РеквизитыОбъекта) Экспорт
	
	ДанныеПоСтатистике = Неопределено;
	
	Если ПустаяСтрока(ИмяТаблицы) Тогда
		Возврат ДанныеПоСтатистике;
	КонецЕсли;
	
	Если НЕ ЭтоТаблицаСДанными(ИмяТаблицы) Тогда
		Возврат ДанныеПоСтатистике;
	КонецЕсли;
	
	Если РеквизитыОбъекта.Количество() = 0 Тогда
		Возврат ДанныеПоСтатистике;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(ИСТИНА);
	
	ИмяТаблицы = СтрЗаменить(ИмяТаблицы, ".ТабличнаяЧасть.", ".");
	КэшСсылочныеТипыТаблицы = Новый Соответствие;
	ДобавитьЗначенияСубконто = Ложь;
	ШаблонСубконтоПроверкиНаПустоеЗначение = Неопределено;
	
	//@skip-check bsl-ql-hub
	ТекстЗапроса = "ВЫБРАТЬ";
	ТекстЗапросаСоединение = "";
	ШаблонПустаяСсылкаСоединение =
	"	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПустыеСсылки КАК %1ПустыеСсылки
	|	ПО ТаблицаДанных.%1 = %1ПустыеСсылки.ПустаяСсылка";
	
	Для Каждого Реквизит Из РеквизитыОбъекта Цикл
		
		ТипДанных = ПроверитьТипДанных(Реквизит.Имя, ИмяТаблицы);
		Если ТипДанных = 0 Тогда
			Продолжить;
		ИначеЕсли ТипДанных = 2 Тогда
			ДобавитьЗначенияСубконто = Истина;
		КонецЕсли;
		
		ШаблонПроверкиНаПустоеЗначение = Неопределено;
		
		Если Реквизит.Тип.СодержитТип(Тип("ХранилищеЗначения"))
			ИЛИ Реквизит.Пароль
			ИЛИ Реквизит.Тип.СодержитТип(Тип("Строка")) И Реквизит.Тип.КвалификаторыСтроки.Длина = 0 Тогда
			ШаблонСтрокиЗапроса = "	0 КАК %ИмяПоля%";
		ИначеЕсли ТипДанных = 2 Тогда
			Если Лев(Реквизит.Имя, 10) = "СубконтоДт" Тогда
				ШаблонСтрокиЗапроса = "	МАКСИМУМ(ВТ_ТаблицаДанныхСубконтоДт.КоличествоСубконто) КАК %ИмяПоля%";
			ИначеЕсли Лев(Реквизит.Имя, 13) = "ВидСубконтоДт" Тогда
				ШаблонСтрокиЗапроса = "	МАКСИМУМ(ВТ_ТаблицаДанныхСубконтоДт.КоличествоВидовСубконто) КАК %ИмяПоля%";
			ИначеЕсли Лев(Реквизит.Имя, 10) = "СубконтоКт" Тогда
				ШаблонСтрокиЗапроса = "	МАКСИМУМ(ВТ_ТаблицаДанныхСубконтоКт.КоличествоСубконто) КАК %ИмяПоля%";
			ИначеЕсли Лев(Реквизит.Имя, 13) = "ВидСубконтоКт" Тогда
				ШаблонСтрокиЗапроса = "	МАКСИМУМ(ВТ_ТаблицаДанныхСубконтоКт.КоличествоВидовСубконто) КАК %ИмяПоля%";
			КонецЕсли;
			Если ШаблонСубконтоПроверкиНаПустоеЗначение = Неопределено Тогда
				ШаблонСубконтоПроверкиНаПустоеЗначение = ШаблонПроверкиНаПустоеЗначение(ИмяТаблицы, Реквизит, КэшСсылочныеТипыТаблицы);
				ТекстЗапросаСоединение = ТекстЗапросаСоединение
					+ Символы.ПС
					+ "	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаДанныхСубконтоДт КАК ВТ_ТаблицаДанныхСубконтоДт
					|	ПО ИСТИНА"
					+ Символы.ПС
					+ "	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТаблицаДанныхСубконтоКт КАК ВТ_ТаблицаДанныхСубконтоКт
					|	ПО ИСТИНА";
			КонецЕсли;
		Иначе
			ШаблонСтрокиЗапроса = "	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.%ИмяПоля%) -
			|	МАКСИМУМ(&ТекстШаблонаПроверкиНаПустоеЗначение) КАК %ИмяПоля%";
			ШаблонПроверкиНаПустоеЗначение = ШаблонПроверкиНаПустоеЗначение(ИмяТаблицы, Реквизит, КэшСсылочныеТипыТаблицы);
		КонецЕсли;
		
		Если Не ШаблонПроверкиНаПустоеЗначение = Неопределено Тогда
			ШаблонСтрокиЗапроса = СтрЗаменить(
				ШаблонСтрокиЗапроса,
				"&ТекстШаблонаПроверкиНаПустоеЗначение",
				ШаблонПроверкиНаПустоеЗначение.УсловиеПроверки
			);
			Если ШаблонПроверкиНаПустоеЗначение.ДобавленоУсловиеПустойСсылки Тогда
				ТекстЗапросаСоединение = ТекстЗапросаСоединение + Символы.ПС + СтрШаблон(ШаблонПустаяСсылкаСоединение, Реквизит.Имя);
			КонецЕсли;
		КонецЕсли;
		
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + СтрЗаменить(ШаблонСтрокиЗапроса, "%ИмяПоля%", Реквизит.Имя) + ",";
		
	КонецЦикла;
	
	ТекстЗапроса = ТекстЗапроса + Символы.ПС + "	КОЛИЧЕСТВО(*) КАК КоличествоСтрокПоТаблице";
	ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ИЗ" + Символы.ПС + ИмяТаблицы + " КАК ТаблицаДанных";
	Если ЗначениеЗаполнено(ТекстЗапросаСоединение) Тогда
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + ТекстЗапросаСоединение;
	КонецЕсли;
	
	Если ДобавитьЗначенияСубконто Тогда
		ТекстЗапросаЗначенияСубконто = 
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Вид) КАК КоличествоВидовСубконто,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Значение) - МАКСИМУМ(&ТекстШаблонаПроверкиСубконтоНаПустоеЗначение) КАК КоличествоСубконто
		|ПОМЕСТИТЬ ВТ_ТаблицаДанныхСубконтоДт
		|ИЗ
		|	" + ИмяТаблицы + ".Субконто КАК ТаблицаДанных
		|"
		+
		?(ШаблонСубконтоПроверкиНаПустоеЗначение.ДобавленоУсловиеПустойСсылки,
			"	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПустыеСсылки КАК ЗначениеПустыеСсылки
			|	ПО ТаблицаДанных.Значение = ЗначениеПустыеСсылки.ПустаяСсылка
			|",
			""
		)
		+
		"ГДЕ
		|	ТаблицаДанных.ВидДвижения = &ВидДвиженияСубконтоДт
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Вид) КАК КоличествоВидовСубконто,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Значение) - МАКСИМУМ(&ТекстШаблонаПроверкиСубконтоНаПустоеЗначение) КАК КоличествоСубконто
		|ПОМЕСТИТЬ ВТ_ТаблицаДанныхСубконтоКт
		|ИЗ
		|	" + ИмяТаблицы + ".Субконто КАК ТаблицаДанных
		|"
		+
		?(ШаблонСубконтоПроверкиНаПустоеЗначение.ДобавленоУсловиеПустойСсылки,
			"	ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПустыеСсылки КАК ЗначениеПустыеСсылки
			|	ПО ТаблицаДанных.Значение = ЗначениеПустыеСсылки.ПустаяСсылка
			|",
			""
		)
		+
		"ГДЕ
		|	ТаблицаДанных.ВидДвижения = &ВидДвиженияСубконтоКт
		|;
		|";
		ТекстЗапросаЗначенияСубконто = СтрЗаменить(
			ТекстЗапросаЗначенияСубконто,
			"&ТекстШаблонаПроверкиСубконтоНаПустоеЗначение",
			СтрЗаменить(ШаблонСубконтоПроверкиНаПустоеЗначение.УсловиеПроверки, "%ИмяПоля%", "Значение")
		);
	Иначе
		ТекстЗапросаЗначенияСубконто = "";
	КонецЕсли;
	
	Если КэшСсылочныеТипыТаблицы.Количество() > 0 Тогда
		СсылочныеТипыТаблицы = Новый Массив;
		Для Каждого КлючИЗначение Из КэшСсылочныеТипыТаблицы Цикл
			СсылочныеТипыТаблицы.Добавить(КлючИЗначение.Ключ);
		КонецЦикла;
		ТекстЗапросаПустыеСсылки = ПолучитьТекстЗапросаПустыеСсылки(СсылочныеТипыТаблицы);
	Иначе
		ТекстЗапросаПустыеСсылки = "";
	КонецЕсли;
	
	ТекстЗапроса = ТекстЗапросаПустыеСсылки + Символы.ПС + ТекстЗапросаЗначенияСубконто + Символы.ПС + ТекстЗапроса;
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ПустойУникальныйИдентификатор", Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"));
	Запрос.УстановитьПараметр("ТипУникальногоИдентификатора",  Тип("УникальныйИдентификатор"));
	Запрос.УстановитьПараметр("ВидДвиженияСубконтоДт",         ВидДвиженияБухгалтерии.Дебет);
	Запрос.УстановитьПараметр("ВидДвиженияСубконтоКт",         ВидДвиженияБухгалтерии.Кредит);
	
	Попытка
		РезультатЗапроса = Запрос.Выполнить();
		Выборка = РезультатЗапроса.Выбрать();
		Если Выборка.Следующий() Тогда
			ДанныеПоСтатистике = Выборка;
		КонецЕсли;
	Исключение
		СообщитьПользователю(ОписаниеОшибки());
	КонецПопытки;
	
	Возврат ДанныеПоСтатистике;
	
КонецФункции

// Получает статистику заполненности полей в системной таблице.
// 
// Параметры:
//  ИмяТаблицы - Строка
//  СистемнаяТаблица - Строка
// 
// Возвращаемое значение:
//  ВыборкаИзРезультатаЗапроса
//
Функция ПолучитьСтатистикуЗаполненностиПолейВСистемнойТаблице(Знач ИмяТаблицы, СистемнаяТаблица) Экспорт
	
	Если ПустаяСтрока(ИмяТаблицы) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если НЕ ЭтоТаблицаСДанными(ИмяТаблицы) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(ИСТИНА);
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(*) КАК КоличествоСтрокПоТаблице";
	
	Если СистемнаяТаблица = "ПредопределенныеДанные" Тогда
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ИЗ" + Символы.ПС + "	" + ИмяТаблицы + " КАК ТаблицаДанных";
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ГДЕ" + Символы.ПС + "	ТаблицаДанных.Предопределенный = Истина";
	ИначеЕсли СтрНачинаетсяС(СистемнаяТаблица, "Регистратор ССЫЛКА") Тогда
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ИЗ" + Символы.ПС + "	" + ИмяТаблицы + " КАК ТаблицаДанных";
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ГДЕ" + Символы.ПС + "	ТаблицаДанных." + СистемнаяТаблица;
	Иначе
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ИЗ" + Символы.ПС + "	" + ИмяТаблицы + " КАК ТаблицаДанных";
	КонецЕсли;
	
	Запрос = Новый Запрос(ТекстЗапроса);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	
	Если Выборка.Следующий() Тогда
		Возврат Выборка;
	КонецЕсли;
	
КонецФункции

// Получает запрос по полям объекта.
// 
// Параметры:
//  ИмяТаблицы - Строка
//  РеквизитыТаблицы - Массив Из Строка
//  Сгруппировать - Булево 
//  ЗначенияОтбора - Массив Из Произвольный, ТаблицаЗначений
//
// Возвращаемое значение:
//  Запрос
//
Функция ПолучитьЗапросПоПолямОбъекта(ИмяТаблицы, РеквизитыТаблицы, Сгруппировать = Истина, ЗначенияОтбора = Неопределено) Экспорт
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	&СписокПолейЗапроса
	|ИЗ
	|	&ИмяТаблицы КАК ДанныеТаблицы";
	
	СписокПолейОтбора  = "";
	СписокПолейЗапроса = "";
	
	Для Каждого Реквизит Из РеквизитыТаблицы Цикл
		СписокПолейЗапроса = СписокПолейЗапроса + ?(ПустаяСтрока(СписокПолейЗапроса), "", ",") + "ДанныеТаблицы."
			+ Реквизит;
	КонецЦикла;
	
	СписокПолейГруппировки = СписокПолейЗапроса;
	Если РеквизитыТаблицы.Количество() = 1 Тогда
		СписокПолейЗапроса = СписокПолейЗапроса + ",КОЛИЧЕСТВО(" + "ДанныеТаблицы." + Реквизит + ") КАК "
			+ Реквизит + "_Количество";
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЗначенияОтбора) И ТипЗнч(ЗначенияОтбора) = Тип("Массив") Тогда
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ГДЕ" + Символы.ПС + "	"
			+ "ДанныеТаблицы.Ссылка В (&ЗначенияОтбора)";
	ИначеЕсли ЗначениеЗаполнено(ЗначенияОтбора) И ТипЗнч(ЗначенияОтбора) = Тип("ТаблицаЗначений") Тогда
		//@skip-check bsl-ql-hub
		ТекстЗапроса =
			"ВЫБРАТЬ
			|	&СписокПолейОтбора
			|ПОМЕСТИТЬ ВТ_ЗначенияОтбора
			|ИЗ
			|	&ЗначенияОтбора КАК ЗначенияОтбора
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	&СписокПолейОтбора
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|"
			+ ТекстЗапроса + Символы.ПС
			+ "	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ЗначенияОтбора КАК ЗначенияОтбора";
		ПоляОтбора = Новый Массив;
		Для Каждого Колонка Из ЗначенияОтбора.Колонки Цикл
			ТекстЗапроса = ТекстЗапроса + Символы.ПС +
				?(ПоляОтбора.Количество() = 0, "	ПО", "		И") + " ДанныеТаблицы." + Колонка.Имя + " = ЗначенияОтбора." + Колонка.Имя;
				ПоляОтбора.Добавить("ЗначенияОтбора." + Колонка.Имя);
		КонецЦикла;
		СписокПолейОтбора = СтрСоединить(ПоляОтбора, ",");
	КонецЕсли;
	
	Если Сгруппировать Тогда
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "СГРУППИРОВАТЬ ПО" + Символы.ПС + "	"
			+ "&СписокПолейГруппировки";
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СписокПолейОтбора",      СписокПолейОтбора);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СписокПолейЗапроса",     СписокПолейЗапроса);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СписокПолейГруппировки", СписокПолейГруппировки);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяТаблицы",             ИмяТаблицы);
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ЗначенияОтбора", ЗначенияОтбора);
	
	Возврат Запрос;
	
КонецФункции

// Находит ссылки, которые содержат текущее значение реквизита.
// 
// Параметры:
//  ЗначениеРеквизита - Произвольный
//  ИмяТаблицы - Строка
//  ИмяРеквизита - Строка
// 
// Возвращаемое значение:
//  Массив Из ЛюбаяСсылка
//
Функция НайтиСсылкиПоЗначениюРеквизита(ЗначениеРеквизита, ИмяТаблицы, ИмяРеквизита) Экспорт
	
	Если ЭтоТаблицаСсылочныхДанных(ИмяТаблицы) Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ДанныеТаблицы.Ссылка
		|ИЗ
		|	&ИмяТаблицы КАК ДанныеТаблицы
		|ГДЕ
		|	ДанныеТаблицы.ИмяРеквизита = &Значение";
		
		Если ТипЗнч(ЗначениеРеквизита) = Тип("Строка") Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДанныеТаблицы.ИмяРеквизита = &Значение",
				"ВЫРАЗИТЬ(ДанныеТаблицы.ИмяРеквизита КАК Строка(500)) = &Значение");
		КонецЕсли;
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицы",  ИмяТаблицы);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИмяРеквизита", ИмяРеквизита);
		
		Запрос.УстановитьПараметр("Значение", ЗначениеРеквизита);
		
		СсылкиТаблицы = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
		
	Иначе
		СсылкиТаблицы = Новый Массив;
	КонецЕсли;
	
	Возврат СсылкиТаблицы;
	
КонецФункции

// Получает статистику по ссылающимся типам.
// 
// Параметры:
//  ЭлементОбъектовМетаданных - Структура
//  ТипПоиска - Число - Тип поиска ссылок: 1 - Поиск ссылок на объект, 2 - Поиск ссылок из объекта на другие объекты.
// 
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение - Статистика по ссылающимся метаданным относительно указанного.
//
Функция ПолучитьСтатистикуПоСсылающимсяТипам(ЭлементОбъектовМетаданных, ТипПоиска) Экспорт
	
	ПолноеИмя = ЭлементОбъектовМетаданных.ПолноеИмя;
	Раздел    = ЭлементОбъектовМетаданных.Раздел;
	
	ЭтоТаблицаСсылочныхДанных = ЭтоТаблицаСсылочныхДанных(ПолноеИмя);
	ЭтоТаблицаНабораДанных    = ЭтоТаблицаНабораДанных(ПолноеИмя);
	ЭтоКонстанта              = ЭтоКонстанта(ПолноеИмя);
	
	СтатистикаПоСсылающимсяТипам = Новый Соответствие;
	
	Если Не ЭтоТаблицаСсылочныхДанных
		И Не ЭтоТаблицаНабораДанных
		И Не ЭтоКонстанта Тогда
		Возврат СтатистикаПоСсылающимсяТипам;
	КонецЕсли;
	
	СхемаКомпоновкиДанных = ПодготовитьСхемуКомпоновкиДанных(ЭлементОбъектовМетаданных);
	НастройкиКомпоновкиДанных = Новый КомпоновщикНастроекКомпоновкиДанных;
	ЗагрузитьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, Неопределено, Новый УникальныйИдентификатор);
	Данные = ВывестиСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных);
	
	Если Не ЗначениеЗаполнено(Данные) Тогда
		Возврат СтатистикаПоСсылающимсяТипам;
	КонецЕсли;
	
	Если ЭтоТаблицаСсылочныхДанных ИЛИ ЭтоКонстанта Тогда
		ЗначенияОбъекта = Новый Массив;
	ИначеЕсли ЭтоТаблицаНабораДанных Тогда
		НаборЗаписей   = МенеджерОбъектаПоПолномуИмени(ПолноеИмя).СоздатьНаборЗаписей();
		Отбор          = НаборЗаписей.Отбор;
		ЗначенияОбъекта   = Новый ТаблицаЗначений;
		Для Каждого ЭлементОтбора Из Отбор Цикл
			ЗначенияОбъекта.Колонки.Добавить(ЭлементОтбора.Имя, ЭлементОтбора.ТипЗначения);
		КонецЦикла;
	КонецЕсли;
	
	Для Каждого Строка Из Данные Цикл
		
		Если ЭтоТаблицаСсылочныхДанных Тогда
			ЗначенияОбъекта.Добавить(Строка.Ссылка);
		ИначеЕсли ЭтоТаблицаНабораДанных И НаборЗаписей.Отбор.Количество() > 0 Тогда
			НоваяСтрока = ЗначенияОбъекта.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
		КонецЕсли;
		
	КонецЦикла;
	
	Данные.Очистить();
	
	ТипыНайденныхСсылок = Новый Соответствие;
	
	Если ТипПоиска = 1 Тогда
		
		ДанныеПоиска = НайтиПоСсылкам(ЗначенияОбъекта);
		
		Для Каждого Строка Из ДанныеПоиска Цикл
			Если Не ЗначениеЗаполнено(Строка.Данные) Тогда
				Продолжить;
			КонецЕсли;
			ТипЗначения = ТипЗнч(Строка.Данные);
			ТипСтрокой = ТипыНайденныхСсылок.Получить(ТипЗначения);
			Если ТипСтрокой = Неопределено Тогда
				ТипСтрокой = ПолучитьТипСтрокой(ТипЗначения);
				ТипыНайденныхСсылок.Вставить(ТипЗначения, ТипСтрокой);
			КонецЕсли;
			Количество = СтатистикаПоСсылающимсяТипам.Получить(ТипСтрокой);
			Если Количество = Неопределено Тогда
				Количество = 0;
			КонецЕсли;
			Количество = Количество + 1;
			СтатистикаПоСсылающимсяТипам.Вставить(ТипСтрокой, Количество);
		КонецЦикла;
		
	ИначеЕсли ТипПоиска = 2 Тогда
		
		ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ПолноеИмя);
		ОписаниеОбъекта  = ПолучитьОписаниеОбъекта(ОбъектМетаданных, Раздел, Новый Соответствие);
		КлючиЗаписей     = ИнициализироватьКлючиЗаписей();
		НайтиСсылкиПоОбъектуМетаданных(ОписаниеОбъекта, ЗначенияОбъекта, КлючиЗаписей, Новый Соответствие);
		
		Для Каждого КлючЗаписи Из КлючиЗаписей Цикл
			Если Не ЗначениеЗаполнено(КлючЗаписи.Значение) Тогда
				Продолжить;
			КонецЕсли;
			СтатистикаПоСсылающимсяТипам.Вставить(КлючЗаписи.Ключ, КлючЗаписи.Значение.Количество() - 1);
		КонецЦикла;
		
	Иначе
		ДанныеПоиска = Новый Массив;
	КонецЕсли;
	
	Возврат СтатистикаПоСсылающимсяТипам;
	
КонецФункции

// Получает статистику реквизита по типам.
// 
// Параметры:
//  ИмяПоля - Строка
//  ИмяТаблицы - Строка
// 
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение
//
Функция ПолучитьСтатистикуРеквизитаПоТипам(ИмяПоля, ИмяТаблицы) Экспорт
	
	СтатистикаРеквизитаПоТипам = Новый Соответствие;
	
	Если Не ЗначениеЗаполнено(ИмяПоля)
		Или Не ЗначениеЗаполнено(ИмяТаблицы) Тогда
		Возврат СтатистикаРеквизитаПоТипам;
	КонецЕсли;
	
	ЭтоСубконто = Лев(ИмяТаблицы, 18) = "РегистрБухгалтерии" И (Лев(ИмяПоля, 8) = "Субконто" ИЛИ Лев(ИмяПоля, 11) = "ВидСубконто");
	ВидДвиженияСубконто = ?(Сред(ИмяПоля, 9, 2) = "Дт" ИЛИ Сред(ИмяПоля, 12, 2) = "Дт", ВидДвиженияБухгалтерии.Дебет, ВидДвиженияБухгалтерии.Кредит);
	
	Если Не ЭтоСубконто Тогда
		//@skip-check bsl-ql-hub
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТИПЗНАЧЕНИЯ(ВложенныйЗапрос.Значение) КАК ТипЗначения,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВложенныйЗапрос.Значение) КАК Количество,
		|	СУММА(ВложенныйЗапрос.Размер) КАК Размер
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицаДанных." + ИмяПоля + " КАК Значение,
		|		РАЗМЕРХРАНИМЫХДАННЫХ(ТаблицаДанных." + ИмяПоля + ") КАК Размер
		|	ИЗ
		|		" + ИмяТаблицы + " КАК ТаблицаДанных
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ТаблицаДанных." + ИмяПоля + ") КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ТИПЗНАЧЕНИЯ(ВложенныйЗапрос.Значение)";
	Иначе
		//@skip-check bsl-ql-hub
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТИПЗНАЧЕНИЯ(ТаблицаДанных.Значение) КАК ТипЗначения,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.Значение) КАК Количество,
		|	СУММА(РАЗМЕРХРАНИМЫХДАННЫХ(ТаблицаДанных.Значение)) КАК Размер
		|ИЗ
		|	" + ИмяТаблицы + ".Субконто КАК ТаблицаДанных
		|ГДЕ
		|	ТаблицаДанных.ВидДвижения = &ВидДвиженияСубконто
		|
		|СГРУППИРОВАТЬ ПО
		|	ТИПЗНАЧЕНИЯ(ТаблицаДанных.Значение)";
	КонецЕсли;
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ВидДвиженияСубконто", ВидДвиженияСубконто);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ТипСтрокой = ПолучитьТипСтрокой(Выборка.ТипЗначения);
		СтатистикаРеквизитаПоТипам.Вставить(ТипСтрокой, Новый Структура("Количество, Размер", Выборка.Количество, Выборка.Размер));
	КонецЦикла;
	
	Возврат СтатистикаРеквизитаПоТипам;
	
КонецФункции

#КонецОбласти

#Область РаботаСТипами

// Определяет тип объекта метаданных.
// 
// Параметры:
//  ОписаниеТипа - ОписаниеТипов
//  ОбъектМетаданных - СтрокаТаблицыЗначений
// 
// Возвращаемое значение:
//  Строка - Представление типа.
//
Функция ОпределитьТип(ОписаниеТипа, ОбъектМетаданных) Экспорт
	
	Типы              = ОписаниеТипа.Типы();
	КоличествоТипов   = Типы.Количество();
	ПредставлениеТипа = "Неопределено";
	
	Для Каждого Тип Из Типы Цикл
		ПредставлениеТипа = ПолучитьТипСтрокой(Тип);
		Если ПредставлениеТипа = "Строка"
			ИЛИ ПредставлениеТипа = "Число"
			ИЛИ ПредставлениеТипа = "Дата" Тогда
			ПредставлениеТипа = ПолучитьТипСКвалификаторами(ОписаниеТипа, ПредставлениеТипа);
		КонецЕсли;
		ОбъектМетаданных.ЗначениеТипа = ОбъектМетаданных.ЗначениеТипа + ПредставлениеТипа + Символы.ПС;
	КонецЦикла;
	
	Если КоличествоТипов > 1 Тогда
		ОбъектМетаданных.ЗначениеТипа = ОбъектМетаданных.ЗначениеТипа + "Неопределено";
		ПредставлениеТипа      = НСтр("ru = '<Несколько типов>'");
	КонецЕсли;
	
	Возврат ПредставлениеТипа;
	
КонецФункции

#КонецОбласти

#Область РаботаСБД

// Инициализирует структуру хранения.
// 
// Параметры:
//  СтруктураХранения - Структура - Структура хранения для инициализации.
//  АдресСтруктурыХранения - Строка - Адрес хранилища для помещения структуры хранения.
//  УникальныйИдентификатор - УникальныйИдентификатор - Уникальный идентификатор для формирования адреса хранилища.
//  ЗаполнитьСтруктуруХранения - Булево - Признак отключения заполнения структуры хранения.
//
Процедура ИнициализироватьСтруктуруХранения(СтруктураХранения, АдресСтруктурыХранения, УникальныйИдентификатор, ЗаполнитьСтруктуруХранения = Истина) Экспорт
	
	СтруктураХраненияСистемныхТаблиц      = Новый ТаблицаЗначений;
	СтруктураХраненияСистемныхТаблиц.Колонки.Добавить("Назначение");
	СтруктураХраненияСистемныхТаблиц.Колонки.Добавить("Метаданные");
	СтруктураХраненияСистемныхТаблиц.Колонки.Добавить("ИмяТаблицы");
	СтруктураХраненияСистемныхТаблиц.Колонки.Добавить("ИмяТаблицыХранения");
	СтруктураХраненияСистемныхТаблиц.Колонки.Добавить("Прочие");
	
	СтруктураХранения = Новый Структура;
	СтруктураХранения.Вставить("СтруктураХраненияТаблиц",                Новый Соответствие);
	СтруктураХранения.Вставить("СтруктураХраненияИндексов",              Новый Соответствие);
	СтруктураХранения.Вставить("СтруктураХраненияОбщихСистемныхТаблиц",  Новый Соответствие);
	СтруктураХранения.Вставить("СтруктураХраненияСистемныхТаблиц",       СтруктураХраненияСистемныхТаблиц);
	
	Если Не ЗаполнитьСтруктуруХранения Тогда
		АдресСтруктурыХранения = ПоместитьВоВременноеХранилище(СтруктураХранения, УникальныйИдентификатор);
		Возврат;
	КонецЕсли;
	
	ОбщаяСтруктураХранения = ПолучитьСтруктуруХраненияБазыДанных();
	
	Для Каждого Таблица Из ОбщаяСтруктураХранения Цикл
		
		Если ПустаяСтрока(Таблица.Метаданные) Тогда
			
			СтруктураХранения.СтруктураХраненияОбщихСистемныхТаблиц.Вставить(Таблица.Назначение, Таблица.ИмяТаблицыХранения);
			
		ИначеЕсли НЕ (Таблица.Назначение = "Константа")
			И НЕ (Таблица.Назначение = "Основная")
			И НЕ (Таблица.Назначение = "ТабличнаяЧасть") Тогда
			
			НоваяСистемнаяТаблица = СтруктураХранения.СтруктураХраненияСистемныхТаблиц.Добавить();
			НоваяСистемнаяТаблица.Назначение = Таблица.Назначение;
			НоваяСистемнаяТаблица.Метаданные = Таблица.Метаданные;
			НоваяСистемнаяТаблица.ИмяТаблицы = Таблица.ИмяТаблицы;
			НоваяСистемнаяТаблица.ИмяТаблицыХранения = Таблица.ИмяТаблицыХранения;
			
			Если Таблица.ИмяТаблицы <> "" 
				И Таблица.Назначение <> "РегистрацияИзменений"
				И Таблица.Назначение <> "ТочкиМаршрута"
				И Таблица.Назначение <> "ЗначенияСубконто" Тогда
				НоваяСистемнаяТаблица.Прочие = Истина;
			Иначе
				НоваяСистемнаяТаблица.Прочие = Ложь;
			КонецЕсли;
		
		ИначеЕсли НЕ ПустаяСтрока(Таблица.ИмяТаблицы) Тогда
			
			СтруктураХранения.СтруктураХраненияТаблиц.Вставить(Таблица.Метаданные, Таблица.ИмяТаблицыХранения);
			
		КонецЕсли;
		
		Для Каждого Поле Из Таблица.Поля Цикл
			
			ИмяПоля = Поле.ИмяПоля;
			
			Если ПустаяСтрока(ИмяПоля) Тогда
				ИмяПоля = ПолучитьИмяПоляПоИмениПоляХранения(Таблица.Метаданные, Поле.ИмяПоляХранения);
				Если ИмяПоля = "" Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			
			Если Таблица.Метаданные = Поле.Метаданные ИЛИ ПустаяСтрока(Поле.Метаданные) Тогда
				СтруктураХранения.СтруктураХраненияТаблиц.Вставить(Таблица.Метаданные + "." + ИмяПоля, Поле.ИмяПоляХранения);
			ИначеЕсли Прав(ИмяПоля ,2) = "Дт" ИЛИ Прав(ИмяПоля ,2) = "Кт" Тогда
				СтруктураХранения.СтруктураХраненияТаблиц.Вставить(Поле.Метаданные + Прав(ИмяПоля ,2), Поле.ИмяПоляХранения);
			Иначе
				СтруктураХранения.СтруктураХраненияТаблиц.Вставить(Поле.Метаданные, Поле.ИмяПоляХранения);
			КонецЕсли;
			
		КонецЦикла;
		
		ИндексыТаблицы = Новый Массив;
		
		Для Каждого Индекс Из Таблица.Индексы Цикл
			
			Для Каждого ПолеИндекса Из Индекс.Поля Цикл
				
				ИмяПоля = ПолеИндекса.ИмяПоля;
				
				Если ПустаяСтрока(ИмяПоля) Тогда
					ИмяПоля = ПолучитьИмяПоляПоИмениПоляХранения(Таблица.Метаданные, ПолеИндекса.ИмяПоляХранения);
					Если ИмяПоля = "" Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
				
				ИндексТаблицы = Новый Структура;
				ИндексТаблицы.Вставить("ПолноеИмя",  Таблица.ИмяТаблицы);
				ИндексТаблицы.Вставить("ИндексСУБД", Индекс.ИмяИндексаХранения);
				ИндексТаблицы.Вставить("ПолеСУБД",   ПолеИндекса.ИмяПоляХранения);
				ИндексТаблицы.Вставить("ИмяПоля",    ИмяПоля);
				
				Если Таблица.Метаданные = ПолеИндекса.Метаданные ИЛИ ПустаяСтрока(ПолеИндекса.Метаданные) Тогда
					ПолноеИмяПоля = Таблица.Метаданные + "." + ИмяПоля;
				Иначе
					ПолноеИмяПоля = ПолеИндекса.Метаданные;
				КонецЕсли;
				
				ИндексТаблицы.Вставить("ПолноеИмяПоля", ПолноеИмяПоля);
				
				СтруктураХранения.СтруктураХраненияИндексов.Вставить(ПолноеИмяПоля, Таблица.ИмяТаблицы);
				
				ИндексыТаблицы.Добавить(ИндексТаблицы);
				
			КонецЦикла;
			
		КонецЦикла;
		
		Если Таблица.ИмяТаблицы <> "" И ИндексыТаблицы.Количество() > 0 Тогда
			СтруктураХранения.СтруктураХраненияИндексов.Вставить(Таблица.ИмяТаблицы, ИндексыТаблицы);
		КонецЕсли;
		
	КонецЦикла;
	
	АдресСтруктурыХранения = ПоместитьВоВременноеХранилище(СтруктураХранения, УникальныйИдентификатор);
	
КонецПроцедуры

// Получает объект метаданных ИБ по таблице БД.
// 
// Параметры:
//  ИмяСущностиБД - Строка -  Имя таблицы или поля таблицы БД
//
Процедура ПолучитьОбъектПоТаблицеБД(ИмяСущностиБД) Экспорт
	
	ШаблонОписанияМетаданных = НСтр("ru = 'Метаданные: %1, назначение: %2, реквизит'");
	ШаблонОписанияМетаданныхРеквизита = НСтр("ru = 'Метаданные: %1, назначение: %2, реквизит: %3'");
	
	ВывестиОписание = Ложь;
	ИмяРеквизита = "";
	ИмяСущностиБД = ВРег(ИмяСущностиБД);
	ОбщаяСтруктураХранения = ПолучитьСтруктуруХраненияБазыДанных();
	Для Каждого Таблица Из ОбщаяСтруктураХранения Цикл
		
		ИмяТаблицыХранения = ВРег(Таблица.ИмяТаблицыХранения);
		Если СтрНайти(ИмяТаблицыХранения, ИмяСущностиБД) > 0 Тогда
			ВывестиОписание = Истина;
		КонецЕсли;
		
		Для Каждого Индекс Из Таблица.Индексы Цикл
			Если ВывестиОписание Тогда
				Прервать;
			КонецЕсли;
			ИмяИндексаХранения = ВРег(Индекс.ИмяИндексаХранения);
			Если СтрНайти(ИмяИндексаХранения, ИмяСущностиБД) > 0 Тогда
				ВывестиОписание = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Для Каждого Поле Из Таблица.Поля Цикл
			Если ВывестиОписание Тогда
				Прервать;
			КонецЕсли;
			ИмяПоляХранения = ВРег(Поле.ИмяПоляХранения);
			Если СтрНайти(ИмяПоляХранения, ИмяСущностиБД) > 0 Тогда
				ВывестиОписание = Истина;
				ИмяРеквизита = Поле.Метаданные
			КонецЕсли;
		КонецЦикла;
		
		Если ВывестиОписание Тогда
			Если ЗначениеЗаполнено(ИмяРеквизита) Тогда
				ОписанияМетаданных = СтрШаблон(ШаблонОписанияМетаданныхРеквизита, Таблица.Метаданные, Таблица.Назначение, ИмяРеквизита);
			Иначе
				ОписанияМетаданных = СтрШаблон(ШаблонОписанияМетаданных, Таблица.Метаданные, Таблица.Назначение);
			КонецЕсли;
			СообщитьПользователю(ОписанияМетаданных);
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
КонецПроцедуры

#КонецОбласти

#Область РаботаСДанными

// Помечает на удаление ссылку.
// 
// Параметры:
//  СсылкаНаОбъект - ЛюбаяСсылка
//  ЭлементОбъектовМетаданных - Неопределено, Структура - Элемент объектов метаданных:
//   * ПолноеИм - Строка
//   * Раздел -   Строка
//  РежимОбменаДанными - Булево - Устанавливает жим Обмен данными - Загрузка.
//  ПометкаУдаления    - Булево - Если параметр задан, то устанавливается в пометку удаления это значение.
//
Процедура ПометитьНаУдалениеСсылку(СсылкаНаОбъект, ЭлементОбъектовМетаданных, РежимОбменаДанными = Ложь, ПометкаУдаления = Неопределено) Экспорт
	
	Если ПроверитьОсновныеРоли() И НЕ ПривилегированныйРежим() Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
	
	ОбъектДанных = СсылкаНаОбъект.ПолучитьОбъект();
	Если ОбъектДанных = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если РежимОбменаДанными Тогда
		ОбъектДанных.ОбменДанными.Загрузка = Истина;
	КонецЕсли;
	
	Если Не ОбъектДанных.ПометкаУдаления И ЭтоТаблицаПредопределенныхДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
		ОбъектДанных.ИмяПредопределенныхДанных = "";
		Если ЭлементОбъектовМетаданных.Раздел = "ПланыСчетов" Тогда
			ОбъектДанных.ВидыСубконто.Очистить();
		КонецЕсли;
	ИначеЕсли ЭлементОбъектовМетаданных.Раздел = "ПланыОбмена" Тогда
		ОбъектДанных.ЭтотУзел = Ложь;
	КонецЕсли;
	
	ОбъектДанных.ПометкаУдаления = ?(Не ПометкаУдаления = Неопределено, ПометкаУдаления, Не ОбъектДанных.ПометкаУдаления);
	ОбъектДанных.Записать();
	
КонецПроцедуры

// Удаляет все помеченные на удаление объекты без контроля на ссылочную целостность.
// 
// Параметры:
//  УдаляемыеСсылки - Неопределено - Все помеченные на удаление.
//                  - Массив Из ЛюбаяСсылка - Удаляемые ссылки.
//                  - Строка - Полное имя объекта метаданных, по которому нужно запустить удаление.
//  КонтрольСсылочнойЦелостности - Булево
//  РежимОбменаДанными - Булево
//
// Возвращаемое значение:
//  ТаблицаЗначений
//
Функция ЗапуститьУдалениеПомеченных(УдаляемыеСсылки = Неопределено, КонтрольСсылочнойЦелостности = Истина, РежимОбменаДанными = Ложь) Экспорт
	
	Если ПроверитьОсновныеРоли() И НЕ ПривилегированныйРежим() Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
	
	НайденныеДанные = Новый ТаблицаЗначений;
	
	Если ТипЗнч(УдаляемыеСсылки) = Тип("Строка") Или УдаляемыеСсылки = Неопределено Тогда
		ВключитьОбъекты = Неопределено;
		Если ЗначениеЗаполнено(УдаляемыеСсылки) Тогда
			ВключитьОбъекты = Новый Массив;
			ВключитьОбъекты.Добавить(УдаляемыеСсылки);
		КонецЕсли;
		ПомеченныеНаУдаление = НайтиПомеченныеНаУдаление(, ВключитьОбъекты);
	Иначе
		ПомеченныеНаУдаление = УдаляемыеСсылки;
	КонецЕсли;
	
	Если КонтрольСсылочнойЦелостности И Не РежимОбменаДанными Тогда
		ТекущийМонопольныйРежим = МонопольныйРежим();
		Если Не ТекущийМонопольныйРежим Тогда
			УстановитьМонопольныйРежим(Истина);
		КонецЕсли;
		Попытка
			УдалитьОбъекты(ПомеченныеНаУдаление, Истина, НайденныеДанные);
		Исключение
			ТекстОшибки = НСтр("ru = 'Не удалось провести удаление объектов:'") + Символы.ПС + ОписаниеОшибки();
			ТекстОшибки = ТекстОшибки + Символы.ПС +
				НСтр("ru = 'Необходимо найти виновника ошибки и удалить его вручную через форму списка элементов:
					| При запуске метода УдалитьОбъекты запускаются события ПередУдалением, в которых может быть код прерывающий удаление,
					| поэтому можно удалить объекты в режиме обмена с контролем ссылочной целостности или удалить непосредственно.'");
			СообщитьПользователю(ТекстОшибки);
		КонецПопытки;
		Если Не ТекущийМонопольныйРежим Тогда
			УстановитьМонопольныйРежим(Ложь);
		КонецЕсли;
	Иначе
		Для Каждого СсылкаНаОбъект Из ПомеченныеНаУдаление Цикл
			УдалитьСсылкуВРежимеОбменДанными(СсылкаНаОбъект, КонтрольСсылочнойЦелостности);
		КонецЦикла;
	КонецЕсли;
	
	Возврат НайденныеДанные;
	
КонецФункции

// Очищает данные по объекту метаданных.
// 
// Параметры:
//  ЭлементОбъектовМетаданных - Структура
//
Процедура ОчиститьОбъект(ЭлементОбъектовМетаданных) Экспорт
	
	ПараметрыВыгрузки = ИнициализироватьПараметрыВыгрузки(Истина);
	ТранспортДанных = ИнициализироватьТранспортДанных("Очистка", ПараметрыВыгрузки);
	Если ЭлементОбъектовМетаданных.Раздел = "РегистрыСведений" И НЕ ЗначениеЗаполнено(ЭлементОбъектовМетаданных.Отбор) Тогда
		ОчиститьРегистрПоОтбору(ЭлементОбъектовМетаданных.ПолноеИмя);
	Иначе
		ПолучитьДанныеОбъектаМетаданных(ЭлементОбъектовМетаданных, ТранспортДанных);
		Пока ТранспортДанных.КлючиЗаписей.Количество() > 0 Цикл
			УдалитьКлючиЗаписей(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей);
			ТранспортДанных.КлючиЗаписей.Очистить();
			ПолучитьДанныеОбъектаМетаданных(ЭлементОбъектовМетаданных, ТранспортДанных);
		КонецЦикла;
		Если ЭтоТаблицаСсылочныхДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
			ЗапуститьУдалениеПомеченных(ТранспортДанных.КлючиЗаписей);
		КонецЕсли;
	КонецЕсли;
	ОчиститьКлючиЗаписейТранспортаДанных(ТранспортДанных);
	
КонецПроцедуры

// Производит поиск объекта по ссылке.
// 
// Параметры:
//  ПолноеИмя - Строка - полное имя объекта метаданных.
//  СсылкаНаОбъектСтрокой - Строка - Ссылка на объект строкой: ГУИД или навигационная ссылка
// 
// Возвращаемое значение:
//  ЛюбаяСсылка
//
Функция НайтиОбъектПоСсылке(ПолноеИмя, СсылкаНаОбъектСтрокой) Экспорт
	
	Если СтрНайти(СсылкаНаОбъектСтрокой, "?ref=") Тогда
		УИДНавигационнойСсылки = СтрРазделить(СсылкаНаОбъектСтрокой, "=")[1];
		УИДНавигационнойСсылки =
			Сред(УИДНавигационнойСсылки, 25, 8) + "-" +
			Сред(УИДНавигационнойСсылки, 21, 4) + "-" +
			Сред(УИДНавигационнойСсылки, 17, 4) + "-" +
			Сред(УИДНавигационнойСсылки, 1, 4) + "-" +
			Сред(УИДНавигационнойСсылки, 5, 12);
	Иначе
		УИДНавигационнойСсылки = СсылкаНаОбъектСтрокой;
	КонецЕсли;
	
	УИДОбъекта      = Новый УникальныйИдентификатор(УИДНавигационнойСсылки);
	МенеджерОбъекта = МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
	СсылкаНаОбъект   = МенеджерОбъекта.ПолучитьСсылку(УИДОбъекта);
	
	Возврат СсылкаНаОбъект;
	
КонецФункции

#КонецОбласти

#Область РаботаСБитымиСсылками

// Получает таблицу битых ссылок в ИБ.
//
// Параметры:
//   Отбор - Неопределено
//         - Структура:
//          * Раздел - Строка
//          * ИмяТаблицы - Строка
//          * ИмяПоля - Строка
//
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * Раздел - СТрока
//   * ИмяТаблицы - СТрока
//   * ПолноеИмяТаблицы - СТрока
//   * Ключ - Структура - Ключ записи регистра
//   * ИмяПоля - СТрока
//   * БитаяСсылка - ЛюбаяСсылка
//   * ВремяПоиска - Число - Время затраченное на поиск битой ссылки
//
Функция ПолучитьТаблицуБитыхСсылок(Отбор = Неопределено) Экспорт
	
	Раздел     = ?(ЗначениеЗаполнено(Отбор) И Отбор.Свойство("Раздел"),     Отбор.Раздел,     "");
	ИмяТаблицы = ?(ЗначениеЗаполнено(Отбор) И Отбор.Свойство("ИмяТаблицы"), Отбор.ИмяТаблицы, "");
	ИмяПоля    = ?(ЗначениеЗаполнено(Отбор) И Отбор.Свойство("ИмяПоля"),    Отбор.ИмяПоля,    "");
	Порция     = 0;
	Очистить   = Ложь;
	
	ТаблицаБитыхСсылок = Новый ТаблицаЗначений;
	ТаблицаБитыхСсылок.Колонки.Добавить("Раздел");
	ТаблицаБитыхСсылок.Колонки.Добавить("ИмяТаблицы");
	ТаблицаБитыхСсылок.Колонки.Добавить("ПолноеИмяТаблицы");
	ТаблицаБитыхСсылок.Колонки.Добавить("Ключ");
	ТаблицаБитыхСсылок.Колонки.Добавить("ИмяПоля");
	ТаблицаБитыхСсылок.Колонки.Добавить("БитаяСсылка");
	ТаблицаБитыхСсылок.Колонки.Добавить("ВремяПоиска");
	
	СписокОбъектныхМетаданных = ПолучитьСписокОбъектныхМетаданных();
	
	Для Каждого РазделМетаданных Из СписокОбъектныхМетаданных Цикл
		
		Если ЗначениеЗаполнено(Раздел) И РазделМетаданных.Ключ <> Раздел Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого ЭлементМетаданных Из РазделМетаданных.Значение Цикл
		
			Если ЗначениеЗаполнено(ИмяТаблицы) И ЭлементМетаданных.Имя <> ИмяТаблицы Тогда
				Продолжить;
			КонецЕсли;
			
			//@skip-check query-in-loop
			ЗаполнитьТаблицуБитыхСсылокПоОбъекту(ТаблицаБитыхСсылок, ЭлементМетаданных, ЭлементМетаданных, ИмяПоля, РазделМетаданных.Ключ, Порция, Очистить);
			
			Если ЗначениеЗаполнено(ИмяПоля) Тогда
				Продолжить;
			КонецЕсли;
			
			СписокГруппМетаданных = ПодчиненныеМетаданныеРаздела(РазделМетаданных.Ключ);
			
			Для Каждого ГруппаМетаданныхРаздела Из СписокГруппМетаданных Цикл
				
				Если НЕ ЭтоТаблица(ГруппаМетаданныхРаздела) Тогда
					Продолжить;
				КонецЕсли;
				
				Для Каждого Таблица Из ЭлементМетаданных[ГруппаМетаданныхРаздела] Цикл
					
					//@skip-check query-in-loop
					ЗаполнитьТаблицуБитыхСсылокПоОбъекту(ТаблицаБитыхСсылок, ЭлементМетаданных, Таблица, "", ГруппаМетаданныхРаздела, Порция, Очистить);
					
				КонецЦикла;
				
			КонецЦикла;
			
		КонецЦикла;
		
	КонецЦикла;
	
	СписокНеОбъектныхМетаданных = ПолучитьСписокНеОбъектныхМетаданных();
	
	Для Каждого РазделМетаданных Из СписокНеОбъектныхМетаданных Цикл
		
		Если ЗначениеЗаполнено(Раздел) И РазделМетаданных.Ключ <> Раздел Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого ЭлементМетаданных Из РазделМетаданных.Значение Цикл
		
			Если ЗначениеЗаполнено(ИмяТаблицы) И ЭлементМетаданных.ПолноеИмя() <> ИмяТаблицы Тогда
				Продолжить;
			КонецЕсли;
			
			//@skip-check query-in-loop
			ЗаполнитьТаблицуБитыхСсылокПоОбъекту(ТаблицаБитыхСсылок, ЭлементМетаданных, ЭлементМетаданных, ИмяПоля, РазделМетаданных.Ключ, Порция, Очистить);
			Если ЭтоРегистрБухгалтерии(РазделМетаданных.Ключ) Тогда
				ЭлементСубконто = Новый Структура("Имя", "Субконто");
				//@skip-check query-in-loop
				ЗаполнитьТаблицуБитыхСсылокПоОбъекту(ТаблицаБитыхСсылок, ЭлементМетаданных, ЭлементСубконто, ИмяПоля, РазделМетаданных.Ключ, Порция, Очистить);
			КонецЕсли;
			
		КонецЦикла;
		
	КонецЦикла;
	
	Возврат ТаблицаБитыхСсылок;
	
КонецФункции

// Исправляет битую ссылку в ссылочном объекте.
// 
// Параметры:
//  Ссылка - ЛюбаяСсылка - Ссылка на объект.
//  БитаяСсылка - ЛюбаяСсылка
//  ИмяПоля - Строка
//  ИмяТаблицы - Строка
//
Процедура ИсправитьБитуюСсылкуВОбъекте(Ссылка, БитаяСсылка, ИмяПоля, ИмяТаблицы = "") Экспорт
	
	ЗаписатьОбъект = Ложь;
	
	Попытка
		ОбъектПоСсылке = Ссылка.ПолучитьОбъект();
		Если ЗначениеЗаполнено(ИмяТаблицы) Тогда
			Для Каждого Строка Из ОбъектПоСсылке[ИмяТаблицы] Цикл
				Если Строка[ИмяПоля] = БитаяСсылка Тогда
					Строка[ИмяПоля] = Неопределено;
					ЗаписатьОбъект = Истина;
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли ОбъектПоСсылке[ИмяПоля] = БитаяСсылка Тогда
			ОбъектПоСсылке[ИмяПоля] = Неопределено;
			ЗаписатьОбъект = Истина;
		Иначе
			// Не записываем объект
		КонецЕсли;
		
		Если ЗаписатьОбъект Тогда
			ОбъектПоСсылке.ОбменДанными.Загрузка = Истина;
			ОбъектПоСсылке.Записать();
		КонецЕсли;
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось изменить объект по ссылке:'") + Символы.ПС + ОписаниеОшибки();
		СообщитьПользователю(ТекстОшибки);
	КонецПопытки;
	
КонецПроцедуры

// Исправляет битую ссылку в записи регистра.
// 
// Параметры:
//  КлючЗаписи - Структура - Ключ записи регистра.
//  Раздел - Строка
//  ИмяТаблицы - Строка
//  ИмяПоля - Строка
//
Процедура ИсправитьБитуюСсылкуВЗаписи(КлючЗаписи, Раздел, ИмяТаблицы, ИмяПоля) Экспорт
	
	КлючОтбора = Новый Структура;
	Для Каждого Элемент Из КлючЗаписи Цикл
		КлючОтбора.Вставить(Элемент.Представление, Элемент.Значение);
	КонецЦикла;
	
	Попытка
		
		НаборЗаписей = Неопределено;
		НомерСтроки  = Неопределено;
		МенеджерРегистра = ПолучитьМенеджерИлиМодуль(Раздел);
		НаборЗаписей = МенеджерРегистра[ИмяТаблицы].СоздатьНаборЗаписей();
		
		КлючОтбора.Свойство("НомерСтроки", НомерСтроки);
		
		БитаяСсылкаВИзмерении = КлючОтбора.Свойство(ИмяПоля);
		
		Если КлючОтбора.Свойство("Регистратор") Тогда
			НаборЗаписей.Отбор["Регистратор"].Установить(КлючОтбора.Регистратор);
			УдалитьЗапись = ИмяПоля = "Регистратор";
		Иначе
			Для Каждого КлючИЗначение Из КлючОтбора Цикл
				НаборЗаписей.Отбор[КлючИЗначение.Ключ].Установить(КлючИЗначение.Значение);
			КонецЦикла;
			УдалитьЗапись = БитаяСсылкаВИзмерении;
		КонецЕсли;
		
		НаборЗаписей.Прочитать();
		
		Если УдалитьЗапись Тогда
			НаборЗаписей.Очистить();
			НаборЗаписей.Записать(ИСТИНА);
		Иначе
			
			ЗаписиКУдалению = Новый Массив;
			
			Для Каждого Запись Из НаборЗаписей Цикл
				
				Если НомерСтроки <> Неопределено И Запись.НомерСтроки <> НомерСтроки Тогда
					Продолжить;
				КонецЕсли;
				
				Если БитаяСсылкаВИзмерении Тогда
					ЗаписиКУдалению.Добавить(Запись);
				Иначе
					Запись[ИмяПоля] = Неопределено;
				КонецЕсли;
				
			КонецЦикла;
			
			Для Каждого Запись Из ЗаписиКУдалению Цикл
				НаборЗаписей.Удалить(Запись);
			КонецЦикла;
			
			НаборЗаписей.ОбменДанными.Загрузка = Истина;
			НаборЗаписей.Записать(Истина);
			
		КонецЕсли;
		
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось изменить запись регистра:'") + Символы.ПС + ОписаниеОшибки();
		СообщитьПользователю(ТекстОшибки);
	КонецПопытки;
	
КонецПроцедуры

#КонецОбласти

#Область Выгрузка

// Инициализирует параметры выгрузки.
//
// Параметры:
//  Инициализировать - Булево - Инициализировать параметры по умолчанию;
// 
// Возвращаемое значение:
//  Структура - Параметры выгрузки:
//   * РежимВыгрузкиПоСсылке                   - Неопределено, Булево - Выгрузка всех данных связанных с отмеченными выгружаемыми данными.
//   * ОчищатьНеВыгружаемыеСсылки              - Неопределено, Булево - Очищает все ссылки, которые не были в выгрузке, но  были в информационной базе.
//   * СтрогийФорматВыгрузки                   - Неопределено, Булево - Выгрузка данных с контролем корректности метаданных.
//   * ВыгружатьПомеченныеНаУдаление           - Неопределено, Булево - Выгружает помеченные на удаление ссылки.
//   * ОчищатьНеСовпадающиеЭлементыПриЗагрузке - Неопределено, Булево - Помещает в выгрузку соответствующий статус для загрузки.
//  
функция ИнициализироватьПараметрыВыгрузки(Инициализировать = Ложь) Экспорт
	
	ПараметрыВыгрузки = Новый Структура;
	ПараметрыВыгрузки.Вставить("РежимВыгрузкиПоСсылке",                   ?(Инициализировать, Ложь, Ложь));
	ПараметрыВыгрузки.Вставить("ОчищатьНеВыгружаемыеСсылки",              ?(Инициализировать, Истина, Истина));
	ПараметрыВыгрузки.Вставить("СтрогийФорматВыгрузки",                   ?(Инициализировать, Ложь, Неопределено));
	ПараметрыВыгрузки.Вставить("ВыгружатьПомеченныеНаУдаление",           ?(Инициализировать, Ложь, Ложь));
	ПараметрыВыгрузки.Вставить("ОчищатьНеСовпадающиеЭлементыПриЗагрузке", ?(Инициализировать, Ложь, Неопределено));
	
	Возврат ПараметрыВыгрузки;
	
КонецФункции

// Выгружает данные объектов из ИБ.
// 
// Параметры:
//  ОбъектыМетаданных - ТаблицаЗначений - Объекты метаданных для выгрузки.
//  УникальныйИдентификатор - УникальныйИдентификатор - Уникальный идентификатор формы для формирования хранилища файла выгрузки.
//  ПараметрыВыгрузки - См. ИнициализироватьПараметрыВыгрузки
// 
// Возвращаемое значение:
//  Строка - Адрес хранилища файла выгрузки.
//
Функция ВыгрузитьДанныеОбъектов(ОбъектыМетаданных, УникальныйИдентификатор, ПараметрыВыгрузки) Экспорт
	
	ШаблонНачалоВыгрузки = НСтр("ru = 'Начало выгрузки: %Дата'");
	СообщениеНачалоВыгрузки = СтрЗаменить(ШаблонНачалоВыгрузки, "%Дата", ТекущаяДатаСеанса());
	СообщитьПользователю(СообщениеНачалоВыгрузки);
	
	Если ПараметрыВыгрузки.РежимВыгрузкиПоСсылке Тогда
		ТранспортДанных = ИнициализироватьТранспортДанных("ВыгрузкаПоСсылке", ПараметрыВыгрузки);
	Иначе
		ТранспортДанных = ИнициализироватьТранспортДанных("Выгрузка", ПараметрыВыгрузки);
	КонецЕсли;
	
	ДанныеУспешноПолучены = ПолучитьДанные(ОбъектыМетаданных, ТранспортДанных);
	ДвоичныеДанные        = ЗафиксироватьТранспортДанных(ТранспортДанных, ДанныеУспешноПолучены);
	ОчиститьКлючиЗаписейТранспортаДанных(ТранспортДанных);
	
	ШаблонОкончание = НСтр("ru = 'Окончание выгрузки: %Дата'");
	СообщениеОкончание = СтрЗаменить(ШаблонОкончание, "%Дата", ТекущаяДатаСеанса());
	СообщитьПользователю(СообщениеОкончание);
	
	Если ДанныеУспешноПолучены Тогда
		СообщитьПользователю(НСтр("ru = 'Выгрузка данных успешно завершена'"));
	Иначе
		СообщитьПользователю(НСтр("ru = 'Ошибка выгрузки данных'"));
	КонецЕсли;
	
	АдресФайлаВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные, УникальныйИдентификатор);
	
	ДанныеВыгрузки = Новый Структура;
	ДанныеВыгрузки.Вставить("Адрес",    АдресФайлаВоВременномХранилище);
	ДанныеВыгрузки.Вставить("Протокол", ТранспортДанных.Протокол);
	
	Возврат ДанныеВыгрузки;
	
КонецФункции

// Загружает данные объектов в ИБ.
// 
// Параметры:
//  Адрес - Строка - Адрес хранилища файла загрузки.
//  ПараметрыВыгрузки - См. ИнициализироватьПараметрыВыгрузки.
//
// Возвращаемое значение:
//  Структура:
//   * Протокол - См. ИнициализироватьПротокол
//
Функция ЗагрузитьДанныеОбъектов(Адрес, ПараметрыВыгрузки) Экспорт
	
	Если ПроверитьОсновныеРоли() И НЕ ПривилегированныйРежим() Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
	
	ТранспортДанных = ИнициализироватьТранспортДанных("Загрузка", ПараметрыВыгрузки, Адрес);
	
	Если ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке Тогда
		УстановитьМонопольныйРежим(Истина);
	КонецЕсли;
	
	ТекущийТип                = Неопределено;
	ЭлементОбъектовМетаданных = Неопределено;
	ЧислоЗаписанныхЭлементов  = 0;
	ШаблонСообщенияОшибки     = НСтр("ru = 'Ошибка загрузки элементов объекта метаданных %1:
											|%2	
											|%3'");
	ШаблонСообщенияУспеха     = НСтр("ru = 'Загружено элементов объекта метаданных %1:
											|%2'");
	
	НачатьТранзакцию();
	
	ОбъектДанных = ПрочитатьОбъектИзТранспортДанных(ТранспортДанных);
	
	Пока Не ОбъектДанных = Неопределено Цикл
		
		Если ТекущийТип <> ТипЗнч(ОбъектДанных) Тогда
			Если ТекущийТип <> Неопределено Тогда
				Если ЧислоЗаписанныхЭлементов > 0 Тогда
					ТекстСообщения = СтрШаблон(
						ШаблонСообщенияУспеха,
						ЭлементОбъектовМетаданных.ПолноеИмя,
						Формат(ЧислоЗаписанныхЭлементов, "ЧГ=0")
					);
					СообщитьПользователю(ТекстСообщения);
				КонецЕсли;
				Если ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке Тогда
					УдалитьКлючиЗаписей(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей);
				КонецЕсли;
			КонецЕсли;
			ЗафиксироватьТранзакцию();
			НачатьТранзакцию();
			ЧислоЗаписанныхЭлементов = 0;
			ОбъектМетаданных = ОбъектДанных.Метаданные();
			Раздел           = ПолучитьРазделПоОбъекту(ОбъектМетаданных);
			ЭлементОбъектовМетаданных = Новый Структура; 
			ЭлементОбъектовМетаданных.Вставить("Имя",              ОбъектМетаданных.Имя);
			ЭлементОбъектовМетаданных.Вставить("ПолноеИмя",        ОбъектМетаданных.ПолноеИмя());
			ЭлементОбъектовМетаданных.Вставить("Раздел",           Раздел);
			ЭлементОбъектовМетаданных.Вставить("ПараметрыВыгрузки", "");
			КлючиЗаписейПоТипу = ПолучитьКлючиЗаписейПоТипу(ТранспортДанных.КлючиЗаписей, ЭлементОбъектовМетаданных);
			Если КлючиЗаписейПоТипу.Количество() = 0 Тогда
				ПолучитьДанныеОбъектаМетаданных(ЭлементОбъектовМетаданных, ТранспортДанных);
			КонецЕсли;
			ТекущийТип = ТипЗнч(ОбъектДанных);
			БлокировкаДанных = Новый БлокировкаДанных;
			БлокировкаДанных.Добавить(ЭлементОбъектовМетаданных.ПолноеИмя);
			БлокировкаДанных.Заблокировать();
		КонецЕсли;
		
		Если НЕ СравнитьОбъектДанныхСДаннымиВБазе(ОбъектДанных, ЭлементОбъектовМетаданных, ТранспортДанных) Тогда
			Попытка
				//@skip-check query-in-loop
				ОбработкаОбъектаДанныхПередЗаписью(ОбъектДанных, ЭлементОбъектовМетаданных, ТранспортДанных);
				ОбъектДанных.ОбменДанными.Загрузка = Истина;
				ОбъектДанных.ЗаписьИсторииДанных.Отказ = Истина;
				ОбъектДанных.Записать();
				ЧислоЗаписанныхЭлементов = ЧислоЗаписанныхЭлементов + 1;
			Исключение
				ОтменитьТранзакцию();
				ТекстОшибки = ОписаниеОшибки();
				ТекстСообщения = СтрШаблон(
					ШаблонСообщенияОшибки,
					ЭлементОбъектовМетаданных.ПолноеИмя,
					Формат(ЧислоЗаписанныхЭлементов, "ЧГ=0"),
					ТекстОшибки
				);
				СообщитьПользователю(ТекстСообщения);
				ЧислоЗаписанныхЭлементов = 0;
				НачатьТранзакцию();
			КонецПопытки;
		КонецЕсли;
		
		Если ЭтоТаблицаНабораДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
			КлючЗаписиОбъекта = Новый Структура;
			Для Каждого ЭлементОтбора Из ОбъектДанных.Отбор Цикл
				КлючЗаписиОбъекта.Вставить(ЭлементОтбора.Имя, ЭлементОтбора.Значение);
			КонецЦикла;
		ИначеЕсли ЭтоТаблицаСсылочныхДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
			КлючЗаписиОбъекта = Новый Структура("Ссылка, ПометкаУдаления", ОбъектДанных.Ссылка, ОбъектДанных.ПометкаУдаления);
		ИначеЕсли ЭтоКонстанта(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
			КлючЗаписиОбъекта = ЭлементОбъектовМетаданных.ПолноеИмя;
		Иначе
			Продолжить;
		КонецЕсли;
		
		ОчиститьКлючЗаписи(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей, КлючЗаписиОбъекта);
		
		ОбъектДанных = ПрочитатьОбъектИзТранспортДанных(ТранспортДанных);
		
	КонецЦикла;
	
	ЗафиксироватьТранзакцию();
	
	Если ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке Тогда
		УдалитьКлючиЗаписей(Неопределено, ТранспортДанных.КлючиЗаписей);
		ЗапуститьУдалениеПомеченных();
	КонецЕсли;
	
	СообщитьПользователю(НСтр("ru = 'Загрузка данных успешно завершена'"));
	
	ТранспортДанных.ЧтениеДанных.Закрыть();
	ОчиститьКлючиЗаписейТранспортаДанных(ТранспортДанных);
	
	Если ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке Тогда
		УстановитьМонопольныйРежим(Ложь);
	КонецЕсли;
	
	ДанныеЗагрузки = Новый Структура;
	ДанныеЗагрузки.Вставить("Протокол", ТранспортДанных.Протокол);
	
	Возврат ДанныеЗагрузки;
	
КонецФункции

// Преобразовать строку в параметры выгрузки.
// 
// Параметры:
//  ТекстНастроек - Строка - Текст настроек
// 
// Возвращаемое значение:
//  Структура:
// * НастройкиВыгрузки - КомпоновщикНастроекКомпоновкиДанных
//                     - Неопределено
// * НеВыгружаемыеПоля - Соответствие Из КлючИЗначение
//
Функция ПреобразоватьСтрокуВПараметрыВыгрузки(ТекстНастроек) Экспорт
	
	ПараметрыВыгрузки = Новый Структура("НастройкиВыгрузки, НеВыгружаемыеПоля", Неопределено, Новый Соответствие);
	
	Если Не ЗначениеЗаполнено(ТекстНастроек) Тогда
		Возврат ПараметрыВыгрузки;
	КонецЕсли;
	
	Сериализатор = Новый СериализаторXDTO(ФабрикаXDTO);
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.УстановитьСтроку(ТекстНастроек);
	ПараметрыВыгрузки = Сериализатор.ПрочитатьXML(ЧтениеXML);
	ЧтениеXML.Закрыть();
	
	Сериализатор = Неопределено;
	ЧтениеXML    = Неопределено;
	
	Возврат ПараметрыВыгрузки;
	
КонецФункции

// Преобразовать параметры выгрузки в строку.
// 
// Параметры:
//  НастройкиВыгрузки - КомпоновщикНастроекКомпоновкиДанных
//  НеВыгружаемыеПоля - Соответствие Из КлючИЗначение
// 
// Возвращаемое значение:
//  Строка - Преобразовать параметры выгрузки в строку
//
Функция ПреобразоватьПараметрыВыгрузкиВСтроку(НастройкиВыгрузки, НеВыгружаемыеПоля) Экспорт
	
	ТекстНастроек = "";
	НастройкиКомпоновкиДанных = НастройкиВыгрузки.ПолучитьНастройки();
	
	Если Не ИспользуетсяОтборСКД(НастройкиКомпоновкиДанных)
		И Не ЗначениеЗаполнено(НеВыгружаемыеПоля) Тогда
		Возврат ТекстНастроек;
	КонецЕсли;
	
	ПараметрыВыгрузки = Новый Структура("НастройкиВыгрузки, НеВыгружаемыеПоля");
	ПараметрыВыгрузки.НастройкиВыгрузки = НастройкиКомпоновкиДанных;
	ПараметрыВыгрузки.НеВыгружаемыеПоля = НеВыгружаемыеПоля;
	
	Сериализатор = Новый СериализаторXDTO(ФабрикаXDTO);
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
	ЗаписьXML.ЗаписатьОбъявлениеXML();
	Сериализатор.ЗаписатьXML(ЗаписьXML, ПараметрыВыгрузки);
	ТекстНастроек = ЗаписьXML.Закрыть();
	
	Возврат ТекстНастроек;
	
КонецФункции

// Загружает параметры выгрузки из строки.
// 
// Параметры:
//  НастройкиВыгрузки - КомпоновщикНастроекКомпоновкиДанных
//  ЭлементОбъектовМетаданных - Структура
//  УникальныйИдентификатор - УникальныйИдентификатор - Уникальный идентификатор формы для инициализации СКД.
//
Процедура ЗагрузитьПараметрыВыгрузкиИзСтроки(НастройкиВыгрузки, ЭлементОбъектовМетаданных, УникальныйИдентификатор) Экспорт
	
	СхемаКомпоновкиДанных = ПодготовитьСхемуКомпоновкиДанных(ЭлементОбъектовМетаданных);
	ЗагрузитьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиВыгрузки, ЭлементОбъектовМетаданных.НастройкиВыгрузки, УникальныйИдентификатор);
	
КонецПроцедуры

#КонецОбласти

#Область Прочее

// Получает по список реквизитов объекта метаданных в строку имен, разделенных запятой.
// 
// Параметры:
//  РеквизитыОбъекта - Массив Из ОбъектМетаданных
// 
// Возвращаемое значение:
//  Строка
//
Функция ПреобразоватьРеквизитыВСтроку(РеквизитыОбъекта) Экспорт
	
	Строка = "";
	
	Для Каждого РеквизитОбъекта Из РеквизитыОбъекта Цикл
		
		Если РеквизитОбъекта.Тип.СодержитТип(Тип("ХранилищеЗначения")) Тогда
			Продолжить;
		КонецЕсли;
		
		Если РеквизитОбъекта.Пароль Тогда
			Продолжить;
		КонецЕсли;
		
		Строка = Строка + СокрЛП(РеквизитОбъекта.Имя) + ";";
		
	КонецЦикла;
	
	Возврат Строка;
	
КонецФункции

// Проверить существование свойства у объекта метаданных.
// 
// Параметры:
//  ИмяСвойства - Строка
//  ОбъектМетаданных - ОбъектМетаданных
// 
// Возвращаемое значение:
//  Булево
//
Функция ПроверитьСуществованиеСвойства(ИмяСвойства, ОбъектМетаданных) Экспорт
	
	СтруктураСвойств = Новый Структура();
	СтруктураСвойств.Вставить(ИмяСвойства, Неопределено);
	ЗаполнитьЗначенияСвойств(СтруктураСвойств, ОбъектМетаданных);
	
	Если СтруктураСвойств[ИмяСвойства] = Неопределено Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
	
КонецФункции

// Генерирует сообщение пользователю.
// 
// Параметры:
//  Текст - Строка - Текст сообщения
//  КлючДанных - ЛюбаяСсылка
//             - Неопределено
//
Процедура СообщитьПользователю(Текст, КлючДанных = Неопределено) Экспорт
	
	Если ВыведенныеСообщения = Неопределено Тогда
		ВыведенныеСообщения = Новый Соответствие;
	КонецЕсли;
	
	Хеш = Новый ХешированиеДанных(ХешФункция.MD5);
	Хеш.Добавить(Текст);
	
	Если ВыведенныеСообщения.Получить(Хеш.ХешСумма) = Неопределено Тогда
		ВыведенныеСообщения.Вставить(Хеш.ХешСумма, Текст);
	Иначе
		Возврат;
	КонецЕсли;
	
	СообщениеПользователю = Новый СообщениеПользователю;
	СообщениеПользователю.Текст = Текст;
	Если ЗначениеЗаполнено(КлючДанных) Тогда
		СообщениеПользователю.КлючДанных = КлючДанных;
	КонецЕсли;
	СообщениеПользователю.Сообщить();
	
КонецПроцедуры

#КонецОбласти

#КонецОбласти

#Область СлужебныеПроцедурыИФункции

#Область РаботаСМетаданными

Функция ИнициализироватьРаздел(ИмяРаздела, Общий, Детализировать, ЭтоОсновнаяТаблица = Ложь, ЭтаТаблицаСодержитТип = Ложь)
	
	Раздел = Новый Структура;
	Раздел.Вставить("ИмяРаздела",            ИмяРаздела);
	Раздел.Вставить("Общий",                 Общий);
	Раздел.Вставить("Детализировать",        Детализировать);
	Раздел.Вставить("ЭтоОсновнаяТаблица",    ЭтоОсновнаяТаблица);
	Раздел.Вставить("ЭтаТаблицаСодержитТип", ЭтаТаблицаСодержитТип);
	
	Возврат Раздел;
	
КонецФункции

Функция ПолучитьИмяТаблицыПоРазделу(Раздел)
	
	Если Раздел = "Константы" Тогда
		Возврат "Константа";
	ИначеЕсли Раздел = "ПланыОбмена" Тогда
		Возврат "ПланОбмена";
	ИначеЕсли Раздел = "Справочники" Тогда
		Возврат "Справочник";
	ИначеЕсли Раздел = "Документы" Тогда
		Возврат "Документ";
	ИначеЕсли Раздел = "Перечисления" Тогда
		Возврат "Перечисление";
	ИначеЕсли Раздел = "ПланыВидовХарактеристик" Тогда
		Возврат "ПланВидовХарактеристик";
	ИначеЕсли Раздел = "ПланыСчетов" Тогда
		Возврат "ПланСчетов";
	ИначеЕсли Раздел = "ПланыВидовРасчета" Тогда
		Возврат "ПланВидовРасчета";
	ИначеЕсли Раздел = "РегистрыСведений" Тогда
		Возврат "РегистрСведений";
	ИначеЕсли Раздел = "РегистрыНакопления" Тогда
		Возврат "РегистрНакопления";
	ИначеЕсли Раздел = "РегистрыБухгалтерии" Тогда
		Возврат "РегистрБухгалтерии";
	ИначеЕсли Раздел = "РегистрыРасчета" Тогда
		Возврат "РегистрРасчета";
	ИначеЕсли Раздел = "БизнесПроцессы" Тогда
		Возврат "БизнесПроцесс";
	ИначеЕсли Раздел = "Задачи" Тогда
		Возврат "Задача";
	Иначе
		Возврат Раздел;
	КонецЕсли;
	
КонецФункции

Функция ПолучитьСписокОбъектныхМетаданных()
	
	СписокМетаданных = Новый Соответствие;
	СписокМетаданных.Вставить("ПланыОбмена",             Метаданные.ПланыОбмена);
	СписокМетаданных.Вставить("Справочники",             Метаданные.Справочники);
	СписокМетаданных.Вставить("Документы",               Метаданные.Документы);
	СписокМетаданных.Вставить("ПланыВидовХарактеристик", Метаданные.ПланыВидовХарактеристик);
	СписокМетаданных.Вставить("ПланыСчетов",             Метаданные.ПланыСчетов);
	СписокМетаданных.Вставить("ПланыВидовРасчета",       Метаданные.ПланыВидовРасчета);
	СписокМетаданных.Вставить("БизнесПроцессы",          Метаданные.БизнесПроцессы);
	СписокМетаданных.Вставить("Задачи",                  Метаданные.Задачи);
	
	Возврат СписокМетаданных;
	
КонецФункции

Функция ПолучитьСписокНеОбъектныхМетаданных()
	
	СписокМетаданных = Новый Соответствие;
	СписокМетаданных.Вставить("РегистрыСведений",    Метаданные.РегистрыСведений);
	СписокМетаданных.Вставить("РегистрыНакопления",  Метаданные.РегистрыНакопления);
	СписокМетаданных.Вставить("РегистрыБухгалтерии", Метаданные.РегистрыБухгалтерии);
	СписокМетаданных.Вставить("РегистрыРасчета",     Метаданные.РегистрыРасчета);
	
	Возврат СписокМетаданных;
	
КонецФункции

Функция ЭтоОсновнаяТаблица(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Если Лев(ИмяТаблицы, 9)       = "Константа" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 10) = "ПланОбмена" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 10) = "Справочник" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 8)  = "Документ" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 5)  = "Отчет" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 9)  = "Обработка" Тогда
		Возврат Истина;
	ИначеЕсли ЭтоПланВидовХарактеристик(ИмяТаблицы) Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 10) = "ПланСчетов" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 16) = "ПланВидовРасчета" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 15) = "РегистрСведений" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 17) = "РегистрНакопления" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 18) = "РегистрБухгалтерии" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 14) = "РегистрРасчета" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 13) = "БизнесПроцесс" Тогда
		Возврат Истина;
	ИначеЕсли Лев(ИмяТаблицы, 6) = "Задача" Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
	
КонецФункции

Функция ЭтоКонстанта(ПолноеИмя)
	
	Возврат
		Лев(ПолноеИмя, 9) = "Константа";
	
КонецФункции

Функция ЭтоПланОбмена(ПолноеИмя)
	
	Возврат
		Лев(ПолноеИмя, 10) = "ПланОбмена";
	
КонецФункции

Функция ЭтоТаблицаСсылочныхДанных(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 10) = "ПланОбмена"
	ИЛИ Лев(ИмяТаблицы, 10) = "Справочник"
	ИЛИ Лев(ИмяТаблицы, 8)  = "Документ"
	ИЛИ Лев(ИмяТаблицы, 12) = "Перечисление"
	ИЛИ ЭтоПланВидовХарактеристик(ИмяТаблицы)
	ИЛИ Лев(ИмяТаблицы, 10) = "ПланСчетов"
	ИЛИ Лев(ИмяТаблицы, 16) = "ПланВидовРасчета"
	ИЛИ Лев(ИмяТаблицы, 13) = "БизнесПроцесс"
	ИЛИ Лев(ИмяТаблицы, 6)  = "Задача";
	
КонецФункции

Функция ЭтоТаблицаДокумент(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 8)  = "Документ";
	
КонецФункции

Функция ЭтоТаблицаПредопределенныхДанных(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 10) = "Справочник"
	ИЛИ ЭтоПланВидовХарактеристик(ИмяТаблицы)
	ИЛИ Лев(ИмяТаблицы, 10) = "ПланСчетов"
	ИЛИ Лев(ИмяТаблицы, 16) = "ПланВидовРасчета";
	
КонецФункции

Функция ЭтоТаблицаНабораДанных(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 15) = "РегистрСведений"
	ИЛИ ЭтоРегистрНакопления(ИмяТаблицы)
	ИЛИ ЭтоРегистрБухгалтерии(ИмяТаблицы)
	ИЛИ ЭтоРегистрРасчета(ИмяТаблицы);
	
КонецФункции

Функция ЭтоПланВидовХарактеристик(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 22) = "ПланВидовХарактеристик";
	
КонецФункции

Функция ЭтоРегистрБухгалтерии(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 18) = "РегистрБухгалтерии";
	
КонецФункции

Функция ЭтоРегистрНакопления(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 17) = "РегистрНакопления";
	
КонецФункции

Функция ЭтоРегистрРасчета(ПолноеИмя)
	
	ИмяТаблицы = ПолучитьИмяТаблицыПоРазделу(ПолноеИмя);
	
	Возврат
		Лев(ИмяТаблицы, 14) = "РегистрРасчета";
	
КонецФункции

//@skip-check module-unused-method
функция ИнициализироватьПредопределенныеНаименованияНаРусском()
	
	ПредопределенныеНаименованияНаЛатинском = ИнициализироватьПредопределенныеНаименованияЛатиница();
	
	ПредопределенныеНаименования = Новый Соответствие;
	Для Каждого Название Из ПредопределенныеНаименованияНаЛатинском Цикл
		ПредопределенныеНаименования.Вставить(Название.Значение, Название.Ключ);
	КонецЦикла;
	ПредопределенныеНаименования.Вставить("Предопределенное", "Predefined");
	
	Возврат ПредопределенныеНаименования;
	
КонецФункции

функция ИнициализироватьПредопределенныеНаименованияЛатиница()
	
	ПредопределенныеНаименования = Новый Соответствие;
	ПредопределенныеНаименования.Вставить("ExchangePlan", "ПланОбмена");
	ПредопределенныеНаименования.Вставить("Constant", "Константа");
	ПредопределенныеНаименования.Вставить("Catalog", "Справочник");
	ПредопределенныеНаименования.Вставить("Document", "Документ");
	ПредопределенныеНаименования.Вставить("Enum", "Перечисление");
	ПредопределенныеНаименования.Вставить("ChartOfCharacteristicTypes", "ПланВидовХарактеристик");
	ПредопределенныеНаименования.Вставить("ChartOfAccounts", "ПланСчетов");
	ПредопределенныеНаименования.Вставить("ChartOfCalculationTypes", "ПланВидовРасчета");
	ПредопределенныеНаименования.Вставить("BusinessProcess", "БизнесПроцесс");
	ПредопределенныеНаименования.Вставить("Task", "Задача");
	ПредопределенныеНаименования.Вставить("InformationRegister", "РегистрСведений");
	ПредопределенныеНаименования.Вставить("AccumulationRegister", "РегистрНакопления");
	ПредопределенныеНаименования.Вставить("AccountingRegister", "РегистрБухгалтерии");
	ПредопределенныеНаименования.Вставить("CalculationRegister", "РегистрРасчета");
	ПредопределенныеНаименования.Вставить("AccountingFlag", "ПризнакУчетаПланаСчетов");
	ПредопределенныеНаименования.Вставить("ExtDimensionAccountingFlag", "ПризнакУчетаСубконтоПланаСчетов");
	
	ПредопределенныеНаименования.Вставить("TabularSection", "ТабличнаяЧасть");
	ПредопределенныеНаименования.Вставить("Attribute", "Реквизит");
	ПредопределенныеНаименования.Вставить("Dimension", "Измерение");
	ПредопределенныеНаименования.Вставить("AddressingAttribute", "РеквизитАдресации");
	ПредопределенныеНаименования.Вставить("Resource", "Ресурс");
	
	ПредопределенныеНаименования.Вставить("Ref", "Ссылка");
	ПредопределенныеНаименования.Вставить("DataVersion", "ВерсияДанных");
	
	ПредопределенныеНаименования.Вставить("Date", "Дата");
	ПредопределенныеНаименования.Вставить("Number", "Номер");
	ПредопределенныеНаименования.Вставить("DeletionMark", "ПометкаУдаления");
	ПредопределенныеНаименования.Вставить("Posted", "Проведен");
	
	ПредопределенныеНаименования.Вставить("Description", "Наименование");
	ПредопределенныеНаименования.Вставить("Code", "Код");
	ПредопределенныеНаименования.Вставить("PredefinedDataName", "ИмяПредопределенныхДанных");
	ПредопределенныеНаименования.Вставить("Predefined", "Предопределенный");
	ПредопределенныеНаименования.Вставить("Parent", "Родитель");
	ПредопределенныеНаименования.Вставить("IsFolder", "ЭтоГруппа");
	ПредопределенныеНаименования.Вставить("Owner", "Владелец");
	
	ПредопределенныеНаименования.Вставить("Started", "Стартован");
	ПредопределенныеНаименования.Вставить("Completed", "Завершен");
	ПредопределенныеНаименования.Вставить("HeadTask", "ВедущаяЗадача");

	ПредопределенныеНаименования.Вставить("BusinessProcess", "БизнесПроцесс");
	ПредопределенныеНаименования.Вставить("Executed", "Выполнена");
	ПредопределенныеНаименования.Вставить("RoutePoint", "ТочкаМаршрута");
	
	ПредопределенныеНаименования.Вставить("BaseCalculationTypes", "БазовыеВидыРасчета");
	ПредопределенныеНаименования.Вставить("CalculationType", "ВидРасчета");
	ПредопределенныеНаименования.Вставить("LeadingCalculationTypes", "ВедущиеВидыРасчета");
	ПредопределенныеНаименования.Вставить("DisplacingCalculationTypes", "ВытесняющиеВидыРасчета");
	ПредопределенныеНаименования.Вставить("ActionPeriodIsBasic", "ПериодДействияБазовый");
	
	ПредопределенныеНаименования.Вставить("ValueType", "ТипЗначения");
	
	ПредопределенныеНаименования.Вставить("ExchangeDate", "ДатаОбмена");
	ПредопределенныеНаименования.Вставить("SentNo", "НомерОтправленного");
	ПредопределенныеНаименования.Вставить("ReceivedNo", "НомерПринятого");
	ПредопределенныеНаименования.Вставить("ThisNode", "ЭтотУзел");
	
	ПредопределенныеНаименования.Вставить("Type", "Вид");
	ПредопределенныеНаименования.Вставить("ExtDimensionTypes", "ВидыСубконто");
	ПредопределенныеНаименования.Вставить("ExtDimensionType", "ВидСубконто");
	ПредопределенныеНаименования.Вставить("TurnoversOnly", "ТолькоОбороты");
	ПредопределенныеНаименования.Вставить("OffBalance", "Забалансовый");
	ПредопределенныеНаименования.Вставить("Order", "Порядок");
	
	ПредопределенныеНаименования.Вставить("Recorder", "Регистратор");
	ПредопределенныеНаименования.Вставить("Active", "Активность");
	ПредопределенныеНаименования.Вставить("Period", "Период");
	ПредопределенныеНаименования.Вставить("RecordType", "ВидДвижения");
	ПредопределенныеНаименования.Вставить("LineNumber", "НомерСтроки");
	
	ПредопределенныеНаименования.Вставить("ExtDimensions", "Субконто");
	ПредопределенныеНаименования.Вставить("ExtDimensionsDr", "СубконтоДт");
	ПредопределенныеНаименования.Вставить("ExtDimensionsCr", "СубконтоКт");
	ПредопределенныеНаименования.Вставить("Account", "Счет");
	ПредопределенныеНаименования.Вставить("AccountDr", "СчетДт");
	ПредопределенныеНаименования.Вставить("AccountCr", "СчетКт");
	ПредопределенныеНаименования.Вставить("PeriodAdjustment", "УточнениеПериода");
	
	ПредопределенныеНаименования.Вставить("Name", "Имя");
	ПредопределенныеНаименования.Вставить("Usage", "Использование");
	ПредопределенныеНаименования.Вставить("ComparisonType", "ВидСравнения");
	ПредопределенныеНаименования.Вставить("Value", "Значение");
	ПредопределенныеНаименования.Вставить("ValueFrom", "ЗначениеС");
	ПредопределенныеНаименования.Вставить("ValueTo", "ЗначениеПО");
	
	Возврат ПредопределенныеНаименования;
	
КонецФункции

Функция СодержитСсылочныйТип(ОписаниеТипов)
	
	СодержитСсылочныйТип = Ложь;
	Для Каждого Тип Из ОписаниеТипов.Типы() Цикл
		СодержитСсылочныйТип = СодержитСсылочныйТип Или ЭтоСсылка(Тип, Истина);
		Если СодержитСсылочныйТип Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат СодержитСсылочныйТип;
	
КонецФункции

Функция СодержитТипОтображения(ОписаниеТипов)
	
	СодержитТипОтображения = 0;
	Для Каждого Тип Из ОписаниеТипов.Типы() Цикл
		СодержитТипОтображения = Макс(СодержитТипОтображения, ?(ЭтоРедактируемыйТип(Тип), 2, ?(ЭтоТипДляПросмотра(Тип), 1, 0)));
		Если СодержитТипОтображения Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат СодержитТипОтображения;
	
КонецФункции

Функция ЭтоПеречисление(Тип)
	
	Возврат Тип <> Тип("Неопределено")
		И Перечисления.ТипВсеСсылки().СодержитТип(Тип);
	
КонецФункции

Функция ЭтоПростойТип(Тип)
	
	Возврат
		Тип = Тип("Null")
		ИЛИ Тип = Тип("Неопределено")
		ИЛИ Тип = Тип("Число")
		ИЛИ Тип = Тип("Строка")
		ИЛИ Тип = Тип("Дата")
		ИЛИ Тип = Тип("Булево")
		ИЛИ Тип = Тип("ОписаниеТипов")
		ИЛИ Тип = Тип("УникальныйИдентификатор")
		ИЛИ Тип = Тип("ХранилищеЗначения");
	
КонецФункции

Функция ЭтоТипДляПросмотра(Тип)
	
	Возврат 
		(
			ЭтоПростойТип(Тип)
			Или ЭтоСсылка(Тип)
		)
		И НЕ Тип = Тип("ХранилищеЗначения");
	
КонецФункции

Функция ЭтоРедактируемыйТип(Тип)
	
	Возврат
		ЭтоТипДляПросмотра(Тип)
		И НЕ Тип = Тип("УникальныйИдентификатор");
	
КонецФункции

Функция ЭтоИндексируемыйТип(Тип, КвалификаторыСтроки = Неопределено)
	
	Возврат Тип = Тип("Число")
		ИЛИ Тип = Тип("Строка") И КвалификаторыСтроки.Длина <= 1024
		ИЛИ Тип = Тип("Дата")
		ИЛИ Тип = Тип("Булево")
		ИЛИ Тип = Тип("УникальныйИдентификатор")
		ИЛИ ЭтоСсылка(Тип, Истина);
	
КонецФункции

Функция ЭтоИндексируемыйРеквизит(Реквизит)
	
	ИндексируемыйРеквизит = Истина;
	
	Для Каждого Тип Из Реквизит.ТипЗначения.Типы() Цикл
		
		ИндексируемыйРеквизит =
			ИндексируемыйРеквизит
			И ЭтоИндексируемыйТип(Тип, Реквизит.ТипЗначения.КвалификаторыСтроки);
			
		Если НЕ ИндексируемыйРеквизит Тогда
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ИндексируемыйРеквизит;
	
КонецФункции

//@skip-check module-unused-method
Функция ЭтоКонстантныйОбъект(ОбъектМетаданных)
	
	Возврат ОбъектМетаданных <> Неопределено
		И(Метаданные.Константы.Содержит(ОбъектМетаданных));
	
КонецФункции

Функция ЭтоСсылочныйОбъект(ОбъектМетаданных)
	
	Возврат ОбъектМетаданных <> Неопределено
		И(Метаданные.Справочники.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.Документы.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.ПланыСчетов.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.ПланыВидовРасчета.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.ПланыОбмена.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.БизнесПроцессы.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.Задачи.Содержит(ОбъектМетаданных));
	
КонецФункции

Функция ЭтоНаборЗаписей(ОбъектМетаданных)
	
	Возврат ОбъектМетаданных <> Неопределено
		И(Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.РегистрыНакопления.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.РегистрыБухгалтерии.Содержит(ОбъектМетаданных)
		ИЛИ Метаданные.РегистрыРасчета.Содержит(ОбъектМетаданных));
	
КонецФункции

Процедура ПодчиненныеМетаданныеОбъекта(СписокМетаданных)
	
	СписокМетаданных.Добавить("СтандартныеРеквизиты");
	СписокМетаданных.Добавить("Реквизиты");
	СписокМетаданных.Добавить("ТабличныеЧасти");
	
КонецПроцедуры

Процедура ПодчиненныеМетаданныеРегистра(СписокМетаданных)
	
	СписокМетаданных.Добавить("СтандартныеРеквизиты");
	СписокМетаданных.Добавить("Измерения");
	СписокМетаданных.Добавить("Ресурсы");
	СписокМетаданных.Добавить("Реквизиты");
	
КонецПроцедуры

Процедура ДополнительныеПодчиненныеМетаданныеРаздела(Раздел, СписокМетаданных)
	
	Если Раздел = "Константы" Тогда
		СписокМетаданных.Добавить("РегистрацияИзмененийКонстанты");
	ИначеЕсли Раздел = "Справочники" Тогда
		СписокМетаданных.Вставить(0, "ПредопределенныеДанные");
		СписокМетаданных.Вставить(1, "Характеристики");
		СписокМетаданных.Вставить(2, "РегистрацияИзменений");
	ИначеЕсли Раздел = "Документы" Тогда
		СписокМетаданных.Вставить(0, "Движения");
		СписокМетаданных.Вставить(0, "Характеристики");
		СписокМетаданных.Вставить(1, "РегистрацияИзменений");
	ИначеЕсли Раздел = "ПланыВидовХарактеристик" Тогда
		СписокМетаданных.Вставить(0, "ПредопределенныеДанные");
		СписокМетаданных.Вставить(1, "Характеристики");
		СписокМетаданных.Вставить(2, "РегистрацияИзменений");
	ИначеЕсли Раздел = "ПланыОбмена" Тогда
		СписокМетаданных.Вставить(0, "Характеристики");
		СписокМетаданных.Вставить(1, "РегистрацияИзменений");
	ИначеЕсли Раздел = "ПланыСчетов" Тогда
		СписокМетаданных.Вставить(0, "ПредопределенныеДанные");
		СписокМетаданных.Вставить(1, "Характеристики");
		СписокМетаданных.Вставить(2, "РегистрацияИзменений");
	ИначеЕсли Раздел = "ПланыВидовРасчета" Тогда
		СписокМетаданных.Вставить(0, "СистемныеТаблицы");
		СписокМетаданных.Вставить(1, "ПредопределенныеДанные");
		СписокМетаданных.Вставить(2, "Характеристики");
		СписокМетаданных.Вставить(3, "РегистрацияИзменений");
	ИначеЕсли Раздел = "РегистрыСведений" Тогда
		СписокМетаданных.Вставить(0, "СистемныеТаблицы");
		СписокМетаданных.Вставить(1, "РегистрацияИзменений");
	ИначеЕсли Раздел = "РегистрыНакопления" Тогда
		СписокМетаданных.Вставить(0, "СистемныеТаблицы");
		СписокМетаданных.Вставить(1, "РегистрацияИзменений");
	ИначеЕсли Раздел = "РегистрыБухгалтерии" Тогда
		СписокМетаданных.Вставить(0, "СистемныеТаблицы");
		СписокМетаданных.Вставить(1, "РегистрацияИзменений");
		СписокМетаданных.Вставить(6, "ЗначенияСубконто");
	ИначеЕсли Раздел = "РегистрыРасчета" Тогда
		СписокМетаданных.Вставить(0, "СистемныеТаблицы");
		СписокМетаданных.Вставить(1, "РегистрацияИзменений");
		СписокМетаданных.Вставить(6, "Перерасчеты");
	ИначеЕсли Раздел = "БизнесПроцессы" Тогда
		СписокМетаданных.Вставить(2, "Характеристики");
		СписокМетаданных.Вставить(3, "ТочкиМаршрута");
	ИначеЕсли Раздел = "Задачи" Тогда
		СписокМетаданных.Вставить(0, "Характеристики");
		СписокМетаданных.Вставить(1, "РегистрацияИзменений");
	КонецЕсли;
	
	Если НЕ Раздел = "Константы"
		И НЕ Раздел = "Подсистемы"
		И НЕ Раздел = "ПараметрыСеанса"
		И НЕ Раздел = "КритерииОтбора"
		И НЕ Раздел = "ФункциональныеОпции"
		И НЕ Раздел = "ХранилищаНастроек"
		И НЕ Раздел = "ОбщиеФормы"
		И НЕ Раздел = "ОбщиеКоманды"
		И НЕ Раздел = "WebСервисы" Тогда
		СписокМетаданных.Добавить("Прочие");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Раздел)
		И НЕ Раздел = "Подсистемы"
		И НЕ Раздел = "ФункциональныеОпции" Тогда
		СписокМетаданных.Вставить(0, "Подсистемы");
		СписокМетаданных.Вставить(1, "ФункциональныеОпции");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Раздел)
		И Раздел = "ФункциональныеОпции" Тогда
		СписокМетаданных.Вставить(0, "Подсистемы");
	КонецЕсли;
	
КонецПроцедуры

Функция ШаблонПроверкиНаПустоеЗначение(ИмяТаблицы, Реквизит, КэшСсылочныеТипыТаблицы)
	
	Результат = Новый Структура("ДобавленоУсловиеПустойСсылки, УсловиеПроверки", Ложь, "");
	
	Если ЭтоТаблицаСсылочныхДанных(ИмяТаблицы) И Реквизит.Имя = "Ссылка" Тогда
		Результат.УсловиеПроверки = "0";
		Возврат Результат;
	КонецЕсли;
	
	УсловиеПроверки =
	"	ВЫБОР
	|		КОГДА ТаблицаДанных.%ИмяПоля% ЕСТЬ NULL
	|			ТОГДА 0";
	
	Для Каждого Тип Из Реквизит.Тип.Типы() Цикл
		
		Если Тип = Тип("Число") Тогда
			УсловиеПроверки = УсловиеПроверки + "
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(ЧИСЛО)
			|				И ТаблицаДанных.%ИмяПоля% = 0
			|			ТОГДА 1
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(ЧИСЛО)
			|			ТОГДА 0";
		ИначеЕсли Тип = Тип("Строка") Тогда
			УсловиеПроверки = УсловиеПроверки + "
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(СТРОКА)
			|				И ТаблицаДанных.%ИмяПоля% = """"
			|			ТОГДА 1
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(СТРОКА)
			|			ТОГДА 0";
		ИначеЕсли Тип = Тип("Дата") Тогда
			УсловиеПроверки = УсловиеПроверки + "
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(ДАТА)
			|				И ТаблицаДанных.%ИмяПоля% = ДАТАВРЕМЯ(1,1,1,0,0,0)
			|			ТОГДА 1
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(ДАТА)
			|			ТОГДА 0";
		ИначеЕсли Тип = Тип("Булево") Тогда
			УсловиеПроверки = УсловиеПроверки + "
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(БУЛЕВО)
			|			ТОГДА 0";
		ИначеЕсли Тип = Тип("ХранилищеЗначения") Тогда
			УсловиеПроверки = УсловиеПроверки + "
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(ХранилищеЗначений)
			|			ТОГДА 0";
		ИначеЕсли Тип = Тип("УникальныйИдентификатор") Тогда
			УсловиеПроверки = УсловиеПроверки + "
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = &ТипУникальногоИдентификатора
			|				И ТаблицаДанных.%ИмяПоля% = &ПустойУникальныйИдентификатор
			|			ТОГДА 1
			|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = &ТипУникальногоИдентификатора
			|			ТОГДА 0";
		ИначеЕсли ЭтоСсылка(Тип, Истина) Тогда
			Если НЕ Результат.ДобавленоУсловиеПустойСсылки Тогда
				УсловиеПроверки = УсловиеПроверки + "
				|		КОГДА НЕ %ИмяПоля%ПустыеСсылки.ПустаяСсылка ЕСТЬ NULL
				|			ТОГДА 1";
				Результат.ДобавленоУсловиеПустойСсылки = Истина;
			КонецЕсли;
			КэшСсылочныеТипыТаблицы.Вставить(Тип);
		Иначе
			// Проверка на битую ссылку добавляется один раз
		КонецЕсли;
		
	КонецЦикла;
	
	УсловиеПроверки = УсловиеПроверки + "
	|		ИНАЧЕ 0
	|	КОНЕЦ";
	
	Результат.УсловиеПроверки = УсловиеПроверки;
	
	Возврат Результат;
	
КонецФункции

Процедура ПолучитьПодсистемыОбъектаРекурсивно(ОбъектМетаданных, МетаданныеПодсистемы, Подсистемы)
	
	Для Каждого Подсистема Из МетаданныеПодсистемы Цикл
		
		Если Подсистема.Состав.Содержит(ОбъектМетаданных) Тогда
			Подсистемы.Добавить(Подсистема);
		КонецЕсли;
		
		ПолучитьПодсистемыОбъектаРекурсивно(ОбъектМетаданных, Подсистема.Подсистемы, Подсистемы);
		
	КонецЦикла;
	
КонецПроцедуры

Функция ПолучитьИнтерфейсныеОбъекты()
	
	ИнтерфейсныеОбъекты = Новый Соответствие;
	
	ПолучитьИнтерфейсныеОбъектыРекурсивно(Метаданные.Подсистемы, ИнтерфейсныеОбъекты);
	
	Возврат ИнтерфейсныеОбъекты;
	
КонецФункции

// Инициализирует описание объекта.
// 
// Возвращаемое значение:
//  Структура - Инициализировать описание объекта:
//   * ПолноеИмя - Строка
//   * Имя - Строка
//   * Путь - Строка
//   * ПутьВЗапросе - Строка
//   * Раздел - Строка
//   * ПутьКСтандартномуРеквизиту - Строка
//   * ЭтоСсылка - Булево
//   * ЭтоНаборЗаписей - Булево
//   * ЭтоОсновнаяТаблица - Булево
//   * ЭтоПодсистема - Булево
//   * ЭтоФункциональнаяОпция - Булево
//   * ВключенВИнтерфейс - Булево
//   * Объекты - Массив Из См. ИнициализироватьОписаниеОбъекта()
//   * ИдентификаторСтроки - Число
//   * ОбъектМетаданных - ОбъектМетаданных
//                      - Неопределено
//   * ОбъектМетаданныхРодитель - ОбъектМетаданных
//                              - Неопределено
//
Функция ИнициализироватьОписаниеОбъекта()
	
	ОписаниеОбъекта = Новый Структура;
	ОписаниеОбъекта.Вставить("ПолноеИмя",                   "");
	ОписаниеОбъекта.Вставить("Имя",                         "");
	ОписаниеОбъекта.Вставить("Путь",                        "");
	ОписаниеОбъекта.Вставить("ПутьВЗапросе",                "");
	ОписаниеОбъекта.Вставить("Раздел",                      "");
	ОписаниеОбъекта.Вставить("ПутьКСтандартномуРеквизиту",  "");
	ОписаниеОбъекта.Вставить("ЭтоРедактируемыйТип",         Ложь);
	ОписаниеОбъекта.Вставить("ЭтоТипДляПросмотра",          Ложь);
	ОписаниеОбъекта.Вставить("ЭтоСсылка",                   Ложь);
	ОписаниеОбъекта.Вставить("ЭтоНаборЗаписей",             Ложь);
	ОписаниеОбъекта.Вставить("ЭтоОсновнаяТаблица",          Ложь);
	ОписаниеОбъекта.Вставить("ЭтоПодсистема",               Ложь);
	ОписаниеОбъекта.Вставить("ЭтоФункциональнаяОпция",      Ложь);
	ОписаниеОбъекта.Вставить("ЭтоТаблица",                  Ложь);
	ОписаниеОбъекта.Вставить("ЭтоПолеДанных",               Ложь);
	ОписаниеОбъекта.Вставить("ЭтоСтандартныйРеквизит",      Ложь);
	ОписаниеОбъекта.Вставить("ЭтоИзмерение",                Ложь);
	ОписаниеОбъекта.Вставить("ВключенВИнтерфейс",           Ложь);
	ОписаниеОбъекта.Вставить("Объекты",                     Новый Массив);
	ОписаниеОбъекта.Вставить("ИдентификаторСтроки",         0);
	ОписаниеОбъекта.Вставить("ОбъектМетаданных",            Неопределено);
	ОписаниеОбъекта.Вставить("ОбъектМетаданныхРодитель",    Неопределено);
	
	Возврат ОписаниеОбъекта;
	
КонецФункции

Функция ПолучитьСписокПолейТаблицы(ЭлементМетаданных, Раздел, ИмяПоля = "")
	
	ОтборПоПолю = ЗначениеЗаполнено(ИмяПоля);
	
	СписокГруппМетаданных = ПодчиненныеМетаданныеРаздела(Раздел);
	
	СписокПолей    = Новый Соответствие;
	КлючиСтруктуры = "Тип, КоличествоСсылочныхТипов, ГруппаМетаданных";
	
	Если (Не ОтборПоПолю ИЛИ ИмяПоля = "СчетДт") И ЭтоРегистрБухгалтерии(Раздел) И ЭлементМетаданных.Корреспонденция Тогда
		СписокПолей.Вставить("СчетДт", Новый Структура(КлючиСтруктуры, Новый ОписаниеТипов("ПланСчетовСсылка." + ЭлементМетаданных.ПланСчетов.Имя), 1, "Измерения"));
	ИначеЕсли (Не ОтборПоПолю ИЛИ ИмяПоля = "СчетКт") И ЭтоРегистрБухгалтерии(Раздел) И ЭлементМетаданных.Корреспонденция Тогда
		СписокПолей.Вставить("СчетКт", Новый Структура(КлючиСтруктуры, Новый ОписаниеТипов("ПланСчетовСсылка." + ЭлементМетаданных.ПланСчетов.Имя), 1, "Измерения"));
	ИначеЕсли (Не ОтборПоПолю ИЛИ ИмяПоля = "Счет") И ЭтоРегистрБухгалтерии(Раздел) И НЕ ЭлементМетаданных.Корреспонденция Тогда
		СписокПолей.Вставить("Счет", Новый Структура(КлючиСтруктуры, Новый ОписаниеТипов("ПланСчетовСсылка." + ЭлементМетаданных.ПланСчетов.Имя), 1, "Измерения"));
	КонецЕсли;
	
	Если ОтборПоПолю И СписокПолей.Количество() > 0 тогда
		Возврат СписокПолей;
	КонецЕсли;
	
	Для Каждого ГруппаМетаданныхРаздела Из СписокГруппМетаданных Цикл
		
		Если НЕ ЭтоПолеДанных(ГруппаМетаданныхРаздела) Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого Поле Из ЭлементМетаданных[ГруппаМетаданныхРаздела] Цикл
			
			Если ОтборПоПолю И Поле.Имя <> ИмяПоля Тогда
				Продолжить;
			КонецЕсли;
			
			КоличествоСсылочныхТипов = КоличествоСсылочныхТиповВОписанииТипа(Поле.Тип);
			
			СвойстваПоля = Новый Структура(КлючиСтруктуры, Поле.Тип, КоличествоСсылочныхТипов, ГруппаМетаданныхРаздела);
			
			Приставки = Новый Массив;
			Если ЭтоРегистрБухгалтерии(Раздел) И ЭлементМетаданных.Корреспонденция Тогда
				Приставки.Добавить("Дт");
				Приставки.Добавить("Кт");
			Иначе
				Приставки.Добавить("");
			КонецЕсли;
			
			Для Каждого Приставка Из Приставки Цикл
				ПолноеИмяПоля = ПолучитьИмяЭлементаОбъектаМетаданных(Поле, Приставка);
				СписокПолей.Вставить(ПолноеИмяПоля, СвойстваПоля);
			КонецЦикла;
			
			Если ОтборПоПолю И СписокПолей.Количество() > 0 тогда
				Возврат СписокПолей;
			КонецЕсли;
			
		КонецЦикла;
		
	КонецЦикла;
	
	Возврат СписокПолей;
	
КонецФункции

Процедура ПолучитьИнтерфейсныеОбъектыРекурсивно(МетаданныеПодсистемы, ИнтерфейсныеОбъекты)
	
	Для Каждого Подсистема Из МетаданныеПодсистемы Цикл
		
		Если Подсистема.ВключатьВКомандныйИнтерфейс Тогда
			Для Каждого ОбъектСостава Из Подсистема.Состав Цикл
				ПолноеИмя = ОбъектСостава.ПолноеИмя();
				ИнтерфейсныеОбъекты.Вставить(ПолноеИмя, ПолноеИмя);
			КонецЦикла;
		КонецЕсли;
		
		ПолучитьИнтерфейсныеОбъектыРекурсивно(Подсистема.Подсистемы, ИнтерфейсныеОбъекты);
		
	КонецЦикла;
	
КонецПроцедуры

#КонецОбласти

#Область РаботаСПравамиДоступа

Процедура ПраваСправочника(СписокПравРаздела)
	
	СписокПравРаздела.Добавить("Чтение");
	СписокПравРаздела.Добавить("Добавление");
	СписокПравРаздела.Добавить("Изменение");
	СписокПравРаздела.Добавить("Удаление");
	СписокПравРаздела.Добавить("Просмотр");
	СписокПравРаздела.Добавить("ИнтерактивноеДобавление");
	СписокПравРаздела.Добавить("Редактирование");
	СписокПравРаздела.Добавить("ИнтерактивноеУдаление");
	СписокПравРаздела.Добавить("ИнтерактивнаяПометкаУдаления");
	СписокПравРаздела.Добавить("ИнтерактивноеСнятиеПометкиУдаления");
	СписокПравРаздела.Добавить("ИнтерактивноеУдалениеПомеченных");
	СписокПравРаздела.Добавить("ВводПоСтроке");
	
КонецПроцедуры

Процедура ПраваДокумента(СписокПравРаздела)
	
	СписокПравРаздела.Добавить("Проведение");
	СписокПравРаздела.Добавить("ОтменаПроведения");
	СписокПравРаздела.Добавить("ИнтерактивноеПроведение");
	СписокПравРаздела.Добавить("ИнтерактивноеПроведениеНеОперативное");
	СписокПравРаздела.Добавить("ИнтерактивнаяОтменаПроведения");
	СписокПравРаздела.Добавить("ИнтерактивноеИзменениеПроведенных");
	
КонецПроцедуры

Процедура ПраваРегистра(СписокПравРаздела)
	
	СписокПравРаздела.Добавить("Чтение");
	СписокПравРаздела.Добавить("Изменение");
	СписокПравРаздела.Добавить("Просмотр");
	СписокПравРаздела.Добавить("Редактирование");
	
КонецПроцедуры

#КонецОбласти

#Область РаботаСоСтатистикойДанных

Функция ПроверитьЗаполненностьРеквизита(ИмяПоля, ИмяТаблицы, ОписаниеТипа = Неопределено, ЗаполненныеРеквизиты = Неопределено)
	
	Если ЗаполненныеРеквизиты = Неопределено Тогда
		ЗаполненныеРеквизиты = Новый Соответствие;
	КонецЕсли;
	
	РеквизитЗаполнен = ЗаполненныеРеквизиты.Получить(ИмяТаблицы + ИмяПоля);
	
	Если Не РеквизитЗаполнен = Неопределено Тогда
		Возврат РеквизитЗаполнен;
	КонецЕсли;
	
	УсловиеПустойСсылки = ?(
		ОписаниеТипа = Неопределено,
		"",
		"ГДЕ " + ПолучитьУсловиеПустойСсылки(ИмяПоля, ОписаниеТипа)
	);
	
	//@skip-check bsl-ql-hub
	ТекстЗапроса =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	" + ИмяПоля + "
	|ИЗ " + ИмяТаблицы + " КАК ТаблицаДанных
	|" + УсловиеПустойСсылки;
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Результат = Запрос.Выполнить();
	
	РеквизитЗаполнен = Результат.Пустой();
	
	ЗаполненныеРеквизиты.Вставить(ИмяТаблицы + ИмяПоля, РеквизитЗаполнен);
	
	Возврат РеквизитЗаполнен;
	
КонецФункции

Функция ПолучитьПакетСсылочныхРеквизитов(ОписаниеОбъекта, НеВыгружаемыеПоля = Неопределено)
	
	ПакетСсылочныхРеквизитов = Новый Соответствие;
	СсылочныеРеквизиты       = Новый Массив;
	
	Для Каждого Реквизит Из ОписаниеОбъекта.Объекты Цикл
		
		Если Реквизит.ЭтоСтандартныйРеквизит И Реквизит.Имя = "Ссылка" Тогда
			Продолжить;
		Иначе
			// Поля отбора формируются по ссылке или измерениям
		КонецЕсли;
		
		Если ЗначениеЗаполнено(НеВыгружаемыеПоля) И НеВыгружаемыеПоля.Получить(Реквизит.ПолноеИмя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если Реквизит.ЭтоСсылка Тогда
			СсылочныеРеквизиты.Добавить(Реквизит.ПутьВЗапросе);
		ИначеЕсли Реквизит.ЭтоТаблица Тогда
			
			СсылочныеРеквизитыТЧ = Новый Массив;
			
			Для Каждого РеквизитТЧ Из Реквизит.Объекты Цикл
				Если РеквизитТЧ.ЭтоСсылка Тогда
					СсылочныеРеквизитыТЧ.Добавить(РеквизитТЧ.ПутьВЗапросе);
				КонецЕсли;
			КонецЦикла;
			
			Если СсылочныеРеквизитыТЧ.Количество() > 0 Тогда
				ПакетСсылочныхРеквизитов.Вставить(Реквизит.ПутьВЗапросе, СсылочныеРеквизитыТЧ);
			КонецЕсли;
			
		Иначе
			// Остальные реквизиты не учитываются
		КонецЕсли;
		
	КонецЦикла;
	
	Если СсылочныеРеквизиты.Количество() > 0 Тогда
		ПакетСсылочныхРеквизитов.Вставить(ОписаниеОбъекта.ПутьВЗапросе, СсылочныеРеквизиты);
	КонецЕсли;
	
	Возврат ПакетСсылочныхРеквизитов;
	
КонецФункции

Процедура НайтиСсылкиПоОбъектуМетаданных(ОписаниеОбъекта, ЗначенияОбъекта, КлючиЗаписей, ОписанияОбъектов, НеВыгружаемыеПоля = Неопределено)
	
	ПакетСсылочныхРеквизитов = ПолучитьПакетСсылочныхРеквизитов(ОписаниеОбъекта, НеВыгружаемыеПоля);
	
	НайденныеСсылки = Новый Соответствие;
	
	Для Каждого НаборСсылочныхРеквизитов Из ПакетСсылочныхРеквизитов Цикл
		Запрос = ПолучитьЗапросПоПолямОбъекта(НаборСсылочныхРеквизитов.Ключ, НаборСсылочныхРеквизитов.Значение, Истина, ЗначенияОбъекта);
		//@skip-check query-in-loop
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Для Каждого ИмяРеквизита Из НаборСсылочныхРеквизитов.Значение Цикл
				ЗначениеРеквизита = Выборка[ИмяРеквизита];
				ТипЗначения = ТипЗнч(ЗначениеРеквизита);
				Если ЗначениеЗаполнено(ЗначениеРеквизита) И ЭтоСсылка(ТипЗначения, Истина) Тогда
					ОписаниеОбъектаЗначения = ОписанияОбъектов.Получить(ТипЗначения);
					Если ОписаниеОбъектаЗначения = Неопределено Тогда
						ОбъектМетаданных = ЗначениеРеквизита.Метаданные();
						Раздел = ПолучитьРазделПоОбъекту(ОбъектМетаданных);
						ОписаниеОбъектаЗначения = ПолучитьОписаниеОбъекта(ОбъектМетаданных, Раздел, Новый Соответствие);
						ДобавитьОписаниеОбъектаДляКлючейЗаписи(КлючиЗаписей, ОписаниеОбъектаЗначения);
						ОписанияОбъектов.Вставить(ТипЗначения, ОписаниеОбъектаЗначения);
					КонецЕсли;
					КлючЗаписиОбъекта = Новый Структура("Ссылка, ПометкаУдаления", ЗначениеРеквизита, Ложь);
					КлючДобавлен = ДобавитьКлючЗаписи(ОписаниеОбъектаЗначения, КлючиЗаписей, КлючЗаписиОбъекта);
					Если КлючДобавлен Тогда
						НайденныеСсылкиПоТипу = НайденныеСсылки.Получить(ТипЗначения);
						Если НайденныеСсылкиПоТипу = Неопределено Тогда
							НайденныеСсылкиПоТипу = Новый Массив;
							НайденныеСсылки.Вставить(ТипЗначения, НайденныеСсылкиПоТипу);
						КонецЕсли;
						НайденныеСсылкиПоТипу.Добавить(ЗначениеРеквизита);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого НайденныеСсылкиПоТипу Из НайденныеСсылки Цикл
		ОписаниеОбъектаЗначения = ОписанияОбъектов.Получить(НайденныеСсылкиПоТипу.Ключ);
		//@skip-check query-in-loop
		НайтиСсылкиПоОбъектуМетаданных(ОписаниеОбъектаЗначения, НайденныеСсылкиПоТипу.Значение, КлючиЗаписей, ОписанияОбъектов);
	КонецЦикла;
	
КонецПроцедуры

Функция ПолучитьУсловиеПустойСсылки(ИмяПоля, ОписаниеТипа)
	
	ШаблонУсловиеЗапроса = "ТаблицаДанных.%1 В(%2)";
	ШаблонПустаяСсылка   = "ЗНАЧЕНИЕ(%1.ПустаяСсылка)";
	
	ЭтоСсылочныйРеквизит = Ложь;
	УсловиеЗапроса       = "";
	ПустыеСсылки         = Новый Массив;
	ПустыеСсылки.Добавить("НЕОПРЕДЕЛЕНО");
	
	Если ТипЗнч(ОписаниеТипа) = Тип("ОписаниеТипов") Тогда
		Типы = ОписаниеТипа.Типы();
	Иначе
		Типы = ОписаниеТипа;
	КонецЕсли;
	
	Для Каждого Тип Из Типы Цикл
		
		ТипДляУсловия = ?(ТипЗнч(Тип) = Тип("Структура"), Тип.Тип, Тип);
		
		Если ЭтоПростойТип(ТипДляУсловия) Тогда
			Продолжить;
		КонецЕсли;
		
		ЭтоСсылочныйРеквизит = Истина;
		ПолноеИмяМетаданных  = ПолучитьПолноеИмяМетаданныхПоТипу(ТипДляУсловия);
		ПустыеСсылки.Добавить(СтрШаблон(ШаблонПустаяСсылка, ПолноеИмяМетаданных));
		
	КонецЦикла;
	
	Если ЭтоСсылочныйРеквизит Тогда
		УсловиеЗапроса = СтрШаблон(ШаблонУсловиеЗапроса, ИмяПоля, СтрСоединить(ПустыеСсылки, ","));
	КонецЕсли;
	
	Возврат УсловиеЗапроса;
	
КонецФункции

Функция ПолучитьТекстЗапросаПустыеСсылки(Типы)
	
	ТекстЗапросаПустыеСсылки =
	"ВЫБРАТЬ
	|	НЕОПРЕДЕЛЕНО КАК ПустаяСсылка
	|ПОМЕСТИТЬ ВТ_ПустыеСсылки";
	
	ШаблонПустаяСсылка =
	"
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(%1.ПустаяСсылка)";
	
	Для Каждого Тип Из Типы Цикл
		
		Если ЭтоПростойТип(Тип) Тогда
			Продолжить;
		КонецЕсли;
		
		ПолноеИмяМетаданных  = ПолучитьПолноеИмяМетаданныхПоТипу(Тип);
		ТекстЗапросаПустыеСсылки = ТекстЗапросаПустыеСсылки + СтрШаблон(ШаблонПустаяСсылка, ПолноеИмяМетаданных);
		
	КонецЦикла;
	
	ТекстЗапросаПустыеСсылки = ТекстЗапросаПустыеСсылки + 
	"
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПустаяСсылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////";
	
	Возврат ТекстЗапросаПустыеСсылки;
	
КонецФункции

#КонецОбласти

#Область РаботаСТипами

Функция КоличествоСсылочныхТиповВОписанииТипа(ОписаниеТипа)
	
	КоличествоСсылочныхТипов = 0;
	
	Для Каждого Тип Из ОписаниеТипа.Типы() Цикл
		
		Если ЭтоПростойТип(Тип) Тогда
			Продолжить;
		КонецЕсли;
		
		ТипМетаданных = Метаданные.НайтиПоТипу(Тип);
		
		Если ТипМетаданных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		КоличествоСсылочныхТипов = КоличествоСсылочныхТипов + 1;
		
	КонецЦикла;
	
	Возврат КоличествоСсылочныхТипов;
	
КонецФункции

Процедура КонкретизироватьТипыУПолей(СписокПолей, ИмяТаблицы)
	
	УстановитьПривилегированныйРежим(ИСТИНА);
	
	ШаблонЗапроса =
	"ВЫБРАТЬ
	|	ТИПЗНАЧЕНИЯ(ТаблицаДанных.ИмяПоля) КАК Тип,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТаблицаДанных.ИмяПоля) КАК ТипКоличество,
	|	МАКСИМУМ(ТаблицаДанных.ИмяПоля) КАК ТипМаксимум,
	|	""ИмяПоля"" КАК Имя
	|ИЗ
	|	&ИмяТаблицы КАК ТаблицаДанных
	|ГДЕ
	|	НЕ ТаблицаДанных.ИмяПоля = НЕОПРЕДЕЛЕНО
	|
	|СГРУППИРОВАТЬ ПО
	|	ТИПЗНАЧЕНИЯ(ТаблицаДанных.ИмяПоля)";
	
	ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "&ИмяТаблицы", ИмяТаблицы);
	ШаблонОшибкаТипаДанных = НСтр("ru = 'Ошибка типа данных: таблица ""%1"", реквизит ""%2""'");
	
	ТекстЗапроса = "";
	
	Для Каждого Поле Из СписокПолей Цикл
		
		Если Поле.Значение.КоличествоСсылочныхТипов <= 1 Тогда
			Продолжить;
		КонецЕсли;
		
		ТекстЗапроса = СтрЗаменить(ШаблонЗапроса, "ИмяПоля", Поле.Ключ);
		
		Поле.Значение.Тип = Новый Массив;
		
		Запрос = Новый Запрос(ТекстЗапроса);
		
		Попытка
			//@skip-check query-in-loop
			ДанныеПоТипам = Запрос.Выполнить().Выгрузить();
		Исключение
			ОшибкаТипаДанных = СтрШаблон(ШаблонОшибкаТипаДанных, ИмяТаблицы, Поле.Ключ);
			СообщитьПользователю(ОшибкаТипаДанных);
			Продолжить;
		КонецПопытки;
		
		Для Каждого Строка Из ДанныеПоТипам Цикл
			ПустойТип = Строка.ТипКоличество = 0
				Или Строка.ТипКоличество = 1 И НЕ ЗначениеЗаполнено(Строка.ТипМаксимум);
			СвойстваПоля = СписокПолей.Получить(Строка.Имя);
			СвойстваПоля.Тип.Добавить(Новый Структура("Тип, ПустойТип", Строка.Тип, ПустойТип));
			СписокПолей.Вставить(Строка.Имя, СвойстваПоля);
		КонецЦикла;
		
	КонецЦикла;
	
КонецПроцедуры

Функция ПолучитьТипСКвалификаторами(ОписаниеТипа, ТипКвалификатора)
	
	Если ТипКвалификатора = "Строка" Тогда
		Возврат ТипКвалификатора + " (" + Строка(ОписаниеТипа.КвалификаторыСтроки.Длина) + ")";
	ИначеЕсли ТипКвалификатора = "Число" Тогда
		Возврат ТипКвалификатора
			+ " (" + Строка(ОписаниеТипа.КвалификаторыЧисла.Разрядность) + ", "
			+ Формат(ОписаниеТипа.КвалификаторыЧисла.РазрядностьДробнойЧасти, "ЧГ=0;") + ", "
			+ Формат(ОписаниеТипа.КвалификаторыЧисла.ДопустимыйЗнак, "ЧГ=0;") + ")";
	ИначеЕсли ТипКвалификатора = "Дата" Тогда
		Возврат ТипКвалификатора + " (" + Строка(ОписаниеТипа.КвалификаторыДаты.ЧастиДаты) + ")";
	КонецЕсли;
	
КонецФункции

Функция ПолучитьТипСтрокой(Тип)
	
	Если Тип = Тип("Неопределено") Тогда
		ТипСтрокой = "Неопределено";
	ИначеЕсли Тип = Тип("Число") Тогда
		ТипСтрокой = "Число";
	ИначеЕсли Тип = Тип("Строка") Тогда
		ТипСтрокой = "Строка";
	ИначеЕсли Тип = Тип("Дата") Тогда
		ТипСтрокой = "Дата";
	ИначеЕсли Тип = Тип("Булево") Тогда
		ТипСтрокой = "Булево";
	ИначеЕсли Тип = Тип("УникальныйИдентификатор") Тогда
		ТипСтрокой = "УникальныйИдентификатор";
	ИначеЕсли Тип = Тип("ХранилищеЗначения") Тогда
		ТипСтрокой = "ХранилищеЗначения";
	Иначе
		ПолноеИмяМетаданных = ПолучитьПолноеИмяМетаданныхПоТипу(Тип);
		Если ПолноеИмяМетаданных = Неопределено Тогда
			ТипСтрокой = Строка(Тип);
		Иначе
			ТипСтрокой = ПолноеИмяМетаданных;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ТипСтрокой;
	
КонецФункции

// Проверяет, что пользователю назначены основные роли конфигурации
// 
// Возвращаемое значение:
//  Булево
//
Функция ПроверитьОсновныеРоли()
	
	Если Не ОсновныеРоли = Неопределено Тогда
		Возврат ОсновныеРоли;
	КонецЕсли;
	
	ОсновныеРоли = Ложь;
	
	Для Каждого ОсновнаяРоль Из Метаданные.ОсновныеРоли Цикл
		//@skip-check using-isinrole
		Если РольДоступна(ОсновнаяРоль) Тогда
			ОсновныеРоли = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ОсновныеРоли;
	
КонецФункции

#КонецОбласти

#Область РаботаСБД

Функция ПолучитьИмяПоляПоИмениПоляХранения(ИмяТаблицы, ИмяПоляХранения)
	
	Если ИмяПоляХранения = "PredefinedID" 
		И ЭтоТаблицаПредопределенныхДанных(ИмяТаблицы) Тогда
		Возврат "ИмяПредопределенныхДанных";
	ИначеЕсли ИмяПоляХранения = "Type" И ЭтоПланВидовХарактеристик(ИмяТаблицы) Тогда
		Возврат "ТипЗначения";
	ИначеЕсли ИмяПоляХранения = "RecordKind" И ЭтоРегистрНакопления(ИмяТаблицы) Тогда
		Возврат "ВидДвижения";
	Иначе
		Возврат "";
	КонецЕсли;
		
КонецФункции

#КонецОбласти

#Область РаботаСДанными

// Удаляет ссылку в с контролем ссылочной целостности режиме обмен данными.
// 
// Параметры:
//  СсылкаНаОбъект - Массив Из ЛюбаяСсылка
//                 - ЛюбаяСсылка
//  КонтрольСсылочнойЦелостности - Булево
//  СообщитьПользователю - Булево
//
Функция УдалитьСсылкуВРежимеОбменДанными(СсылкаНаОбъект, КонтрольСсылочнойЦелостности = Истина, СообщитьПользователю = Истина)
	
	Результат = Истина;
	
	Если ТипЗнч(СсылкаНаОбъект) = Тип("Массив") Тогда
		Ссылки = СсылкаНаОбъект;
	Иначе
		Ссылки = Новый Массив;
		Ссылки.Добавить(СсылкаНаОбъект);
	КонецЕсли;
	
	Если КонтрольСсылочнойЦелостности Тогда
		НайденныеСсылки = НайтиПоСсылкам(Ссылки);
	КонецЕсли;
	
	Для Каждого Ссылка Из Ссылки Цикл
		
		УдалитьОбъектДанных   = Истина;
		КлючиЗаписейКУдалению = Новый Массив;
		
		Если КонтрольСсылочнойЦелостности Тогда
			Отбор = Новый Структура("Ссылка", Ссылка);
			СсылающиесяОбъекты = НайденныеСсылки.НайтиСтроки(Отбор);
			Для Каждого СсылающийсяОбъект Из СсылающиесяОбъекты Цикл
				Если СсылающийсяОбъект.Ссылка = СсылающийсяОбъект.Данные Тогда
					Продолжить;
				КонецЕсли;
				Если НЕ ЭтоСсылочныйОбъект(СсылающийсяОбъект.Метаданные) Тогда
					КлючиЗаписейКУдалению.Добавить(СсылающийсяОбъект.Данные);
					Продолжить;
				КонецЕсли;
				Если УдалитьОбъектДанных Тогда
					ШаблонСообщения = НСтр("ru = 'На элемент %1 ссылаются:'");
					ТекстСообщения = СтрШаблон(
						ШаблонСообщения,
						СокрЛП(СсылающийсяОбъект.Ссылка)
					);
					Если СообщитьПользователю Тогда
						СообщитьПользователю(ТекстСообщения);
					КонецЕсли;
					УдалитьОбъектДанных = Ложь;
				КонецЕсли;
				Если СообщитьПользователю Тогда
					СообщитьПользователю(Строка(СсылающийсяОбъект.Данные), СсылающийсяОбъект.Данные);
				Иначе
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
		КонецЕсли;
		
		Если Не УдалитьОбъектДанных Тогда
			Результат = Ложь;
			Продолжить;
		КонецЕсли;
		
		НачатьТранзакцию();
		
		Попытка
			Для Каждого КлючЗаписи Из КлючиЗаписейКУдалению Цикл
				ОчиститьРегистрПоОтбору(КлючЗаписи.Метаданные().ПолноеИмя(), КлючЗаписи, Истина);
			КонецЦикла;
			ОбъектДанных = Ссылка.ПолучитьОбъект();
			Если НЕ ОбъектДанных = Неопределено Тогда
				ОбъектДанных.ОбменДанными.Загрузка = Истина;
				ОбъектДанных.ЗаписьИсторииДанных.Отказ = Истина;
				ОбъектДанных.Удалить();
			КонецЕсли;
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
			ТекстОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("ru = 'Ошибка удаления элемента объекта метаданных %1:
									|%2'");
			ТекстСообщения = СтрШаблон(
				ШаблонСообщения,
				ОбъектДанных.Метаданные().ПолноеИмя(),
				ТекстОшибки);
			СообщитьПользователю(ТекстСообщения);
			Результат = Ложь;
		КонецПопытки;
		
	КонецЦикла;
	
	Возврат Результат;
	
КонецФункции

Процедура ОчиститьДубльПредопределенного(ОбъектДанных, ЭлементОбъектовМетаданных, ТранспортДанных)
	
	Если Не ОбъектДанных.Предопределенный Тогда
		Возврат;
	КонецЕсли;
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаДанных.Ссылка КАК Ссылка
	|ИЗ
	|	&ТаблицаДанных КАК ТаблицаДанных
	|ГДЕ
	|	ТаблицаДанных.ИмяПредопределенныхДанных = &ИмяПредопределенныхДанных
	|	И НЕ ТаблицаДанных.Ссылка = &Ссылка";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТаблицаДанных", ЭлементОбъектовМетаданных.ПолноеИмя);
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ИмяПредопределенныхДанных", ОбъектДанных.ИмяПредопределенныхДанных);
	Запрос.УстановитьПараметр("Ссылка",                    ОбъектДанных.Ссылка);
	
	Результат = Запрос.Выполнить();
	
	Если Результат.Пустой() Тогда
		Возврат;
	КонецЕсли;
	
	МенеджерОбъекта = МенеджерОбъектаПоПолномуИмени(ЭлементОбъектовМетаданных.ПолноеИмя);
	СсылкаПредопределенный = МенеджерОбъекта[ОбъектДанных.ИмяПредопределенныхДанных];
	ОбъектУдален = УдалитьСсылкуВРежимеОбменДанными(СсылкаПредопределенный, Истина, Ложь);
	
	Если ОбъектУдален Тогда
		КлючЗаписиОбъекта = Новый Структура("Ссылка, ПометкаУдаления",СсылкаПредопределенный, Истина);
		ОчиститьКлючЗаписи(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей, КлючЗаписиОбъекта);
	Иначе
		ПометитьНаУдалениеСсылку(СсылкаПредопределенный, ЭлементОбъектовМетаданных, Истина, Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура УдалитьЭтотУзел(ОбъектДанных, ЭлементОбъектовМетаданных, ТранспортДанных)
	
	Если Не ОбъектДанных.ЭтотУзел Тогда
		Возврат;
	КонецЕсли;
	
	МенеджерПланаОбмена = МенеджерОбъектаПоПолномуИмени(ЭлементОбъектовМетаданных.ПолноеИмя);
	Попытка
		СсылкаЭтотУзел = МенеджерПланаОбмена.ЭтотУзел();
	Исключение
		Возврат;
	КонецПопытки;
	
	Если ОбъектДанных.Ссылка = СсылкаЭтотУзел Тогда
		Возврат;
	КонецЕсли;
	
	ОбъектУдален = УдалитьСсылкуВРежимеОбменДанными(СсылкаЭтотУзел, Истина);
	
	Если ОбъектУдален Тогда
		КлючЗаписиОбъекта = Новый Структура("Ссылка, ПометкаУдаления", СсылкаЭтотУзел, Истина);
		ОчиститьКлючЗаписи(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей, КлючЗаписиОбъекта);
	Иначе
		ПометитьНаУдалениеСсылку(СсылкаЭтотУзел, ЭлементОбъектовМетаданных, Истина, Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОчиститьРегистрПоОтбору(ПолноеИмя, Отбор = Неопределено, РежимОбменаДанными = Ложь)
	
	ЗаписатьНаборЗаписей = Ложь;
	ВсеПоляОтбора  =
		ТипЗнч(Отбор) = Тип("Структура")
		И ЗначениеЗаполнено(Отбор)
		И Отбор.Свойство("Все")
		И Отбор.Все = "*"
		ИЛИ ТипЗнч(Отбор) = Тип("Структура") И Не ЗначениеЗаполнено(Отбор)
		ИЛИ Отбор = Неопределено;
	
	НаборЗаписей = МенеджерОбъектаПоПолномуИмени(ПолноеИмя).СоздатьНаборЗаписей(); // РегистрСведенийНаборЗаписей
	Если Не ВсеПоляОтбора Тогда
		Для Каждого ЭлементОтбора Из НаборЗаписей.Отбор Цикл
			Если ТипЗнч(Отбор) = Тип("Структура") И НЕ Отбор.Свойство(ЭлементОтбора.Имя) Тогда
				Продолжить;
			КонецЕсли;
			ЭлементОтбора.Установить(Отбор[ЭлементОтбора.Имя]);
		КонецЦикла;
		НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() > 0 Тогда
			НаборЗаписей.Очистить();
			ЗаписатьНаборЗаписей = Истина;
		КонецЕсли;
	Иначе
		ЗаписатьНаборЗаписей = Истина;
	КонецЕсли;
	
	Если ЗаписатьНаборЗаписей Тогда
		Если РежимОбменаДанными Тогда
			НаборЗаписей.ОбменДанными.Загрузка = Истина;
			НаборЗаписей.ЗаписьИсторииДанных.Отказ = Истина;
		КонецЕсли;
		НаборЗаписей.Записать(Истина);
	КонецЕсли;
	
КонецПроцедуры

#КонецОбласти

#Область РаботаСБитымиСсылками

Функция ПолучитьУсловиеБитойСсылки(ИмяПоля, ОписаниеТипа)
	
	ШаблонУсловияБитойСложнойСсылки =
		"КОГДА ТИПЗНАЧЕНИЯ(ТаблицаДанных.%ИмяПоля%) = ТИП(%ТипМетаданных%) 
		|	ТОГДА ВЫРАЗИТЬ(ТаблицаДанных.%ИмяПоля% КАК %ТипМетаданных%).Ссылка ЕСТЬ NULL
		|";
	
	ШаблонУсловияБитойСсылки =
		"ВЫРАЗИТЬ(ТаблицаДанных.%ИмяПоля% КАК %ТипМетаданных%).Ссылка ЕСТЬ NULL";
	
	ЭтоСсылочныйРеквизит  = ЛОЖЬ;
	УсловиеЗапроса        = "";
	УсловиеБитойСсылки    = "";
	УсловиеНеПустойСсылки = "НЕ " + ПолучитьУсловиеПустойСсылки(ИмяПоля, ОписаниеТипа);
	
	Если ТипЗнч(ОписаниеТипа) = Тип("ОписаниеТипов") Тогда
		Типы = ОписаниеТипа.Типы();
	Иначе
		Типы = ОписаниеТипа;
	КонецЕсли;
	
	Для Каждого Тип Из Типы Цикл
		
		ТипДляУсловия = ?(ТипЗнч(Тип) = Тип("Структура"), Тип.Тип, Тип);
		ПустойТип     = ?(ТипЗнч(Тип) = Тип("Структура"), Тип.ПустойТип, Ложь);
		
		Если ЭтоПростойТип(ТипДляУсловия) Тогда
			Продолжить;
		КонецЕсли;
		
		ПолноеИмяМетаданных = ПолучитьПолноеИмяМетаданныхПоТипу(ТипДляУсловия);
		
		Если ПолноеИмяМетаданных = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ЭтоСсылочныйРеквизит = ИСТИНА;
		
		Если Типы.Количество() > 1 И НЕ ПустойТип Тогда
			УсловиеБитойСсылки = УсловиеБитойСсылки + Символы.ПС + СтрЗаменить(ШаблонУсловияБитойСложнойСсылки, "%ТипМетаданных%", ПолноеИмяМетаданных);
		ИначеЕсли Типы.Количество() = 1 Тогда
			УсловиеБитойСсылки = СтрЗаменить(ШаблонУсловияБитойСсылки, "%ТипМетаданных%", ПолноеИмяМетаданных);
		Иначе
			// Поля с пустым значением на битые ссылки не проверяем
		КонецЕсли;
		
	КонецЦикла;
	
	Если Типы.Количество() > 1 И ЗначениеЗаполнено(УсловиеБитойСсылки) Тогда
		УсловиеБитойСсылки = "ВЫБОР " + УсловиеБитойСсылки + Символы.ПС + "ИНАЧЕ ЛОЖЬ КОНЕЦ";
	КонецЕсли;
	
	Если ЭтоСсылочныйРеквизит И ЗначениеЗаполнено(УсловиеБитойСсылки) Тогда
		УсловиеЗапроса = УсловиеНеПустойСсылки + Символы.ПС
						+ "И " + УсловиеБитойСсылки;
		УсловиеЗапроса = СтрЗаменить(УсловиеЗапроса, "%ИмяПоля%", ИмяПоля);
	КонецЕсли;
	
	Возврат УсловиеЗапроса;
	
КонецФункции

Процедура ЗаполнитьТаблицуБитыхСсылокПоОбъекту(ТаблицаБитыхСсылок, ОбъектМетаданных, ЭлементМетаданных, ИмяПоля, Раздел, Порция, Очистить)
	
	ШаблонТекстаЗапроса = 
	"ВЫБРАТЬ
	|	*
	|ИЗ
	|	&ТипМетаданных КАК ТаблицаДанных
	|ГДЕ
	|	&УсловиеЗапроса";
	
	УсловиеЗапроса = "";
	
	ИмяОбъектМетаданных = ОбъектМетаданных.ПолноеИмя();
	ЭтоСсылочнаяТаблица = ЭтоТаблицаСсылочныхДанных(ИмяОбъектМетаданных);
	ИсточникПолей       = ЭлементМетаданных;
	
	Если ТипЗнч(ЭлементМетаданных) = Тип("ОписаниеСтандартнойТабличнойЧасти") Тогда
		ПолноеИмяТаблицы = ИмяОбъектМетаданных + "." + ЭлементМетаданных.Имя;
	ИначеЕсли ЭлементМетаданных.Имя = "Субконто" Тогда
		ПолноеИмяТаблицы = ИмяОбъектМетаданных + "." + ЭлементМетаданных.Имя;
		ИсточникПолей    = ОбъектМетаданных;
	Иначе
		ПолноеИмяТаблицы = СтрЗаменить(ЭлементМетаданных.ПолноеИмя(), ".ТабличнаяЧасть.", ".");
	КонецЕсли;
	
	ПодчиненРегистратору = Ложь;
	Периодический = Ложь;
	Если НЕ ЭтоСсылочнаяТаблица И (ЭтоРегистрБухгалтерии(Раздел) ИЛИ ЭтоРегистрНакопления(Раздел) ИЛИ ЭтоРегистрРасчета(Раздел)) Тогда
		ПодчиненРегистратору = Истина;
	ИначеЕсли НЕ ЭтоСсылочнаяТаблица Тогда
		ПодчиненРегистратору = ОбъектМетаданных.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору;
		Периодический = ОбъектМетаданных.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический;
	КонецЕсли;
	
	СписокПолейТаблицы = ПолучитьСписокПолейТаблицы(ИсточникПолей, Раздел, ИмяПоля);
	
	СписокПолей = Новый Соответствие;
	Для Каждого Поле Из СписокПолейТаблицы Цикл
		ТипДанных = ПроверитьТипДанных(Поле.Ключ, ПолноеИмяТаблицы);
		Если ТипДанных = 0 Тогда
			Продолжить;
		КонецЕсли;
		Если ЭлементМетаданных.Имя = "Субконто" Тогда
			Если ТипДанных = 1 Тогда
				Продолжить;
			КонецЕсли;
			Если СтрНачинаетсяС(Поле.Ключ, "Субконто") Тогда
				СписокПолей.Вставить("Значение", Поле.Значение);
			ИначеЕсли СтрНачинаетсяС(Поле.Ключ, "ВидСубконто") Тогда
				СписокПолей.Вставить("Вид", Поле.Значение);
			КонецЕсли;
		Иначе
			Если ТипДанных = 2 Тогда
				Продолжить;
			КонецЕсли;
			СписокПолей.Вставить(Поле.Ключ, Поле.Значение);
		КонецЕсли;
	КонецЦикла;
	
	КонкретизироватьТипыУПолей(СписокПолей, ПолноеИмяТаблицы);
	
	Для Каждого Поле Из СписокПолей Цикл
		
		УсловиеПустаяСсылка = ПолучитьУсловиеБитойСсылки(Поле.Ключ, Поле.Значение.Тип);
		Если ПустаяСтрока(УсловиеПустаяСсылка) Тогда
			Продолжить;
		КонецЕсли;
		
		УсловиеЗапроса = УсловиеЗапроса + Символы.ПС + Символы.Таб + ?(ПустаяСтрока(УсловиеЗапроса), "", " ИЛИ ") + УсловиеПустаяСсылка;
		
	КонецЦикла;
	
	Если ПустаяСтрока(УсловиеЗапроса) Тогда
		Возврат;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(ИСТИНА);
	
	ТекстЗапроса = СтрЗаменить(ШаблонТекстаЗапроса, "&ТипМетаданных", ПолноеИмяТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеЗапроса", УсловиеЗапроса);
	Если ЗначениеЗаполнено(Порция) Тогда
		//@skip-check bsl-ql-hub
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВЫБРАТЬ", "ВЫБРАТЬ ПЕРВЫЕ " + Формат(Порция, "ЧГ=0;"));
	КонецЕсли;
	
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Помощник для работы с конфигурацией.Поиск битых ссылок'"),
		УровеньЖурналаРегистрации.Информация,,,
		ТекстЗапроса
	);
	
	Запрос = Новый Запрос(ТекстЗапроса);
	
	ТекущееНачала = ТекущаяДатаСеанса();
	
	Результат = Запрос.Выполнить();
	
	ВремяПоиска = (ТекущаяДатаСеанса() - ТекущееНачала)/60;
	МаксимальноеВремяИсполненияЗапроса = 2; // Минуты
	
	Если ВремяПоиска > МаксимальноеВремяИсполненияЗапроса Тогда
		ШаблонСообщения = НСтр("ru = 'Время выполнения запроса по таблице %1
		|
		|%2
		|
		|составило %3 минут'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПолноеИмяТаблицы, ТекстЗапроса, Формат(ВремяПоиска, "ЧДЦ=3; ЧН=0.000; ЧГ=0"));
		СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
	ЧислоОшибочныхОпределенийБитыхСсылок = 0;
	
	Выборка = Результат.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		ОшибочноеОпределениеБитыхСсылок = Истина;
		
		Для Каждого Колонка Из Результат.Колонки Цикл
			
			Если ТипЗнч(Выборка[Колонка.Имя]) = Тип("РезультатЗапроса") Тогда
				Продолжить;
			КонецЕсли;
			
			Если НЕ Колонка.Имя = "ТочкаМаршрута" И НЕ Лев(Выборка[Колонка.Имя], 18) = "<Объект не найден>" Тогда
				Продолжить;
			КонецЕсли;
			
			Если Колонка.Имя = "ТочкаМаршрута" И ЗначениеЗаполнено(Лев(Выборка[Колонка.Имя], 18)) Тогда
				Продолжить;
			КонецЕсли;
			
			ОшибочноеОпределениеБитыхСсылок = Ложь;
			
			Строка = ТаблицаБитыхСсылок.Добавить();
			Строка.Раздел           = Раздел;
			Строка.ИмяТаблицы       = ЭлементМетаданных.Имя;
			Строка.ПолноеИмяТаблицы = ПолноеИмяТаблицы;
			Строка.ВремяПоиска      = ВремяПоиска;
			
			Если ЭтоСсылочнаяТаблица Тогда
				Строка.Ключ = Выборка.Ссылка;
			Иначе
				
				Строка.Ключ = Новый СписокЗначений;
				Если ПодчиненРегистратору Тогда
					Строка.Ключ.Добавить(Выборка.Регистратор, "Регистратор");
					Строка.Ключ.Добавить(Выборка.НомерСтроки, "НомерСтроки");
				КонецЕсли;
				
				Если Раздел = "РегистрыСведений" Тогда
					Для Каждого Поле Из ОбъектМетаданных.Измерения Цикл
						Строка.Ключ.Добавить(Выборка[Поле.Имя], Поле.Имя);
					КонецЦикла;
				КонецЕсли;
				
				Если НЕ ПодчиненРегистратору И Периодический Тогда
					Строка.Ключ.Добавить(Выборка.Период, "Период");
				КонецЕсли;
				
			КонецЕсли;
			
			Строка.ИмяПоля = Колонка.Имя;
			Строка.БитаяСсылка  = Выборка[Колонка.Имя];
			
			Если Очистить И ТипЗнч(Строка.Ключ) = Тип("СписокЗначений") Тогда
				ИсправитьБитуюСсылкуВЗаписи(Строка.Ключ, Строка.Раздел, Строка.ИмяТаблицы, Строка.ИмяПоля);
			ИначеЕсли Очистить Тогда
				ИмяТаблицы = "";
				Если СтрЧислоВхождений(Строка.ПолноеИмяТаблицы, ".") > 1 Тогда
					ИмяТаблицы = Строка.ИмяТаблицы;
				КонецЕсли;
				ИсправитьБитуюСсылкуВОбъекте(Строка.Ключ, Строка.БитаяСсылка, Строка.ИмяПоля, ИмяТаблицы);
			КонецЕсли;
			
		КонецЦикла;
		
		Если ОшибочноеОпределениеБитыхСсылок = Истина Тогда
			ЧислоОшибочныхОпределенийБитыхСсылок = ЧислоОшибочныхОпределенийБитыхСсылок + 1;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ЧислоОшибочныхОпределенийБитыхСсылок > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Ошибочное определение битых ссылок по таблице:'") + " " + ПолноеИмяТаблицы
			+ " " + Формат(ЧислоОшибочныхОпределенийБитыхСсылок, "ЧГ=0") + " " + НСтр("ru = 'раз'");
		СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
	
КонецПроцедуры

#КонецОбласти

#Область Выгрузка

Функция ПолучитьДанные(ОбъектыМетаданных, ТранспортДанных)
	
	Результат = Истина;
	
	Попытка
		ОбъектыМетаданныхКВыгрузке = ОбъектыМетаданных.ПолучитьЭлементы();
		ПолучитьРазрешенныеТипыКВыгрузке(ОбъектыМетаданных);
		Для Каждого ЭлементОбъектовМетаданных Из ОбъектыМетаданныхКВыгрузке Цикл
			Если Не ЭлементОбъектовМетаданных.Выгружать Тогда
				Продолжить;
			КонецЕсли;
			//@skip-check query-in-loop
			ПолучитьДанныеОбъектаМетаданных(ЭлементОбъектовМетаданных, ТранспортДанных);
		КонецЦикла;
		Для Каждого ЭлементОбъектовМетаданных Из ОбъектыМетаданныхКВыгрузке Цикл
			ЗаписатьКлючиЗаписейПоТипуВТранспортДанных(ТранспортДанных, ЭлементОбъектовМетаданных);
		КонецЦикла;
	Исключение
		Результат = Ложь;
		СтрокаОписанияОшибки = ОписаниеОшибки();
		ШаблонСообщения = НСтр("ru = 'При выгрузке объекта %1(%2) возникла ошибка:
				|%3'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, "При получение данных", "", СтрокаОписанияОшибки);
		СообщитьПользователю(ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
	КонецПопытки;
	
	Возврат Результат;
	
КонецФункции

// Инициализирует ключи записей.
// 
// Параметры:
//  ИнициализироватьМассив - Булево
//
// Возвращаемое значение:
//   - Соответствие Из См. ИнициализироватьКлючЗаписи()
//   - Массив Из См. ИнициализироватьКлючЗаписи()
//
Функция ИнициализироватьКлючиЗаписей(ИнициализироватьМассив = Ложь)
	
	Результат = ?(ИнициализироватьМассив, Новый Массив, Новый Соответствие);
	Возврат Результат;
	
КонецФункции

Функция ПолучитьКлючиЗаписейПоТипу(КлючиЗаписей, ЭлементОбъектовМетаданных)
	
	КлючиЗаписейПоТипу = КлючиЗаписей.Получить(ЭлементОбъектовМетаданных.ПолноеИмя);
	Если КлючиЗаписейПоТипу = Неопределено Тогда
		КлючиЗаписейПоТипу = Новый Соответствие;
		КлючиЗаписей.Вставить(ЭлементОбъектовМетаданных.ПолноеИмя, КлючиЗаписейПоТипу);
	КонецЕсли;
	
	Возврат КлючиЗаписейПоТипу;
	
КонецФункции

Функция ДобавитьКлючЗаписи(ЭлементОбъектовМетаданных, КлючиЗаписей, КлючЗаписиОбъекта)
	
	Результат = Ложь;
	
	Если ТипЗнч(КлючиЗаписей) = Тип("Массив") Тогда
		КлючЗаписи = ИнициализироватьКлючЗаписи(ЭлементОбъектовМетаданных, КлючЗаписиОбъекта, Ложь);
		КлючиЗаписей.Добавить(КлючЗаписи);
		Результат = Истина;
	Иначе
		КлючЗаписи = ИнициализироватьКлючЗаписи(ЭлементОбъектовМетаданных, КлючЗаписиОбъекта);
		КлючиЗаписейПоТипу = ПолучитьКлючиЗаписейПоТипу(КлючиЗаписей, ЭлементОбъектовМетаданных);
		Если КлючиЗаписейПоТипу.Получить(КлючЗаписи.Ключ) = Неопределено Тогда
			КлючиЗаписейПоТипу.Вставить(КлючЗаписи.Ключ, КлючЗаписи);
			Результат = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции

Функция ДобавитьОписаниеОбъектаДляКлючейЗаписи(КлючиЗаписей, ОписаниеОбъекта)
	
	Результат = Ложь;
	
	КлючиЗаписейПоТипу = ПолучитьКлючиЗаписейПоТипу(КлючиЗаписей, ОписаниеОбъекта);
	
	Если КлючиЗаписейПоТипу.Получить("ОписаниеОбъекта") = Неопределено Тогда
		КлючиЗаписейПоТипу.Вставить("ОписаниеОбъекта", ОписаниеОбъекта);
		Результат = Истина;
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции

Функция ИнициализироватьКлючЗаписи(ЭлементОбъектовМетаданных, КлючЗаписиОбъекта, ГенерироватьХеш = Истина)
	
	КлючЗаписи = Новый Структура;
	КлючЗаписи.Вставить("ПолноеИмя", ЭлементОбъектовМетаданных.ПолноеИмя);
	КлючЗаписи.Вставить("Раздел",    ЭлементОбъектовМетаданных.Раздел);
	
	Если ЭтоТаблицаСсылочныхДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
		КлючЗаписи.Вставить("Ключ",     КлючЗаписиОбъекта.Ссылка);
		КлючЗаписи.Вставить("Значение", КлючЗаписиОбъекта);
	ИначеЕсли ЭтоТаблицаНабораДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
		Если ГенерироватьХеш Тогда
			ХешСтруктуры = ПолучитьХешОбъекта(КлючЗаписиОбъекта);
		Иначе
			ХешСтруктуры = "";
		КонецЕсли;
		КлючЗаписи.Вставить("Ключ",     ХешСтруктуры);
		КлючЗаписи.Вставить("Значение", КлючЗаписиОбъекта);
	ИначеЕсли ЭтоКонстанта(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
		КлючЗаписи.Вставить("Ключ",     ЭлементОбъектовМетаданных.ПолноеИмя);
		КлючЗаписи.Вставить("Значение", КлючЗаписиОбъекта);
	КонецЕсли;
	
	Возврат КлючЗаписи;
	
КонецФункции

Процедура ОчиститьКлючЗаписи(ЭлементОбъектовМетаданных, КлючиЗаписей, КлючЗаписиОбъекта)
	
	КлючЗаписи = ИнициализироватьКлючЗаписи(ЭлементОбъектовМетаданных, КлючЗаписиОбъекта);
	
	КлючиЗаписейПоТипу = ПолучитьКлючиЗаписейПоТипу(КлючиЗаписей, ЭлементОбъектовМетаданных);
	КлючиЗаписейПоТипу.Удалить(КлючЗаписи.Ключ);
	
КонецПроцедуры

Процедура ОчиститьКлючиЗаписейТранспортаДанных(ТранспортДанных)
	
	Для Каждого КлючиЗаписейПоТипу Из ТранспортДанных.КлючиЗаписей Цикл
		КлючиЗаписейПоТипу.Значение.Очистить();
	КонецЦикла;
	ТранспортДанных.КлючиЗаписей.Очистить();
	
КонецПроцедуры

Функция ПроверитьКлючЗаписи(ЭлементОбъектовМетаданных, КлючиЗаписей, КлючЗаписиОбъекта)
	
	КлючЗаписи = ИнициализироватьКлючЗаписи(ЭлементОбъектовМетаданных, КлючЗаписиОбъекта);
	
	КлючиЗаписейПоТипу = ПолучитьКлючиЗаписейПоТипу(КлючиЗаписей, ЭлементОбъектовМетаданных);
	
	Возврат КлючиЗаписейПоТипу.Получить(КлючЗаписи.Ключ) <> Неопределено;
	
КонецФункции

Процедура УдалитьКлючиЗаписей(ЭлементОбъектовМетаданных, КлючиЗаписей)
	
	ШаблонСообщения = НСтр("ru = 'Удалено элементов объекта метаданных %1:
							|%2'");
	КоличествоУдаленных = 0;
	
	Если ТипЗнч(КлючиЗаписей) = Тип("Массив") Тогда
		Для Каждого КлючЗаписи Из КлючиЗаписей Цикл
			УдалитьЗаписьПоКлючу(КлючЗаписи);
			КоличествоУдаленных = КоличествоУдаленных + 1;
		КонецЦикла;
	Иначе
		Для Каждого КлючиЗаписейПоТипу Из КлючиЗаписей Цикл
			Для Каждого КлючЗаписи Из КлючиЗаписейПоТипу.Значение Цикл
				Если ЗначениеЗаполнено(ЭлементОбъектовМетаданных) И КлючиЗаписейПоТипу.Ключ <> ЭлементОбъектовМетаданных.ПолноеИмя Тогда
					Продолжить;
				КонецЕсли;
				Если КлючиЗаписейПоТипу.Ключ = "ОписаниеОбъекта" Тогда
					Продолжить;
				КонецЕсли;
				Если УдалитьЗаписьПоКлючу(КлючЗаписи.Значение) Тогда
					КоличествоУдаленных = КоличествоУдаленных + 1;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	
	Если КоличествоУдаленных > 0 Тогда
		ТекстСообщения = СтрШаблон(
			ШаблонСообщения,
			?(ЭлементОбъектовМетаданных = Неопределено, "<Не указан>", ЭлементОбъектовМетаданных.ПолноеИмя),
			Формат(КоличествоУдаленных, "ЧГ=0;")
		);
		СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
КонецПроцедуры

Функция УдалитьЗаписьПоКлючу(КлючЗаписи, СообщитьОбУдалении = Ложь)
	
	КлючиЗаписиУдален  = Ложь;
	
	Если ЭтоТаблицаСсылочныхДанных(КлючЗаписи.ПолноеИмя) Тогда
		ОбъектУдален = УдалитьСсылкуВРежимеОбменДанными(КлючЗаписи.Значение.Ссылка, Истина, Ложь);
		Если НЕ ОбъектУдален И НЕ КлючЗаписи.Значение.ПометкаУдаления Тогда
			ПометитьНаУдалениеСсылку(КлючЗаписи.Значение.Ссылка, КлючЗаписи, Истина, Истина);
		КонецЕсли;
		КлючиЗаписиУдален = Истина;
	ИначеЕсли ЭтоТаблицаНабораДанных(КлючЗаписи.ПолноеИмя) Тогда
		ОчиститьРегистрПоОтбору(КлючЗаписи.ПолноеИмя, КлючЗаписи.Значение, Истина);
		КлючиЗаписиУдален = Истина;
	ИначеЕсли ЭтоКонстанта(КлючЗаписи.ПолноеИмя) Тогда
		// Константы не удаляем, оставляем настройки системы по-умолчанию,
		// так как пустое значение, тоже определяет настройки системы, которые отличны от базы источника.
		// В случае пересмотра решения, необходимо раскомментировать код ниже.
		//	МенеджерЗначения = МенеджерОбъектаПоПолномуИмени(ПолноеИмя).СоздатьМенеджерЗначения(); // КонстантаМенеджерЗначения
		//	НаборЗаписей.Прочитать();
		//	Если ЗначениеЗаполнено(МенеджерЗначения.Значение) Тогда
		//		МенеджерЗначения.Значение = Неопределено;
		//		МенеджерЗначения.Записать();
		//		КлючиЗаписиУдален = Истина;
		//	КонецЕсли;
	КонецЕсли;
	
	Возврат КлючиЗаписиУдален;
	
КонецФункции

Функция ПолучитьХешОбъекта(ОбъектДанных)
	
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();
	СериализаторXDTO.ЗаписатьXML(ЗаписьXML, ОбъектДанных);
	ТекстСтруктураXDTO = ЗаписьXML.Закрыть();
	
	ХешированиеДанных = Новый ХешированиеДанных(ХешФункция.MD5);
	ХешированиеДанных.Добавить(ТекстСтруктураXDTO);
	
	Возврат Base64Строка(ХешированиеДанных.ХешСумма);
	
КонецФункции

Процедура ОбработкаОбъектаДанныхПередЗаписью(ОбъектДанных, ЭлементОбъектовМетаданных, ТранспортДанных)
	
	Если ЭтоТаблицаПредопределенныхДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
		ОчиститьДубльПредопределенного(ОбъектДанных, ЭлементОбъектовМетаданных, ТранспортДанных);
	ИначеЕсли ЭтоПланОбмена(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
		УдалитьЭтотУзел(ОбъектДанных, ЭлементОбъектовМетаданных, ТранспортДанных);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПолучитьДанныеОбъектаМетаданных(ЭлементОбъектовМетаданных, ТранспортДанных)
	
	Если ЭлементОбъектовМетаданных.Раздел = "Константы" Тогда
		ПолучитьКонстанты(ЭлементОбъектовМетаданных, ТранспортДанных);
	ИначеЕсли ЭтоТаблицаНабораДанных(ЭлементОбъектовМетаданных.ПолноеИмя) Тогда
		ПолучитьНаборДанныхРегистра(ЭлементОбъектовМетаданных, ТранспортДанных);
	ИначеЕсли ЭлементОбъектовМетаданных.Раздел = "Последовательности"
		ИЛИ ЭлементОбъектовМетаданных.Раздел = "Перечисления" Тогда
		Возврат;
	Иначе
		ПолучитьДанныеОбъектногоТипа(ЭлементОбъектовМетаданных, ТранспортДанных);
	КонецЕсли;
	
КонецПроцедуры

Функция ИнициализироватьСериализатор(XDTOАннотацияТипов)
	
	Если XDTOАннотацияТипов Тогда
		ТипыСАннотациейСсылок = ПредопределенныеТипыПриВыгрузке();
	Иначе
		ТипыСАннотациейСсылок = Новый Массив;
	КонецЕсли;
	
	Если ТипыСАннотациейСсылок.Количество() > 0 Тогда
		Фабрика = ПолучитьФабрикуСУказаниемТипов(ТипыСАннотациейСсылок);
		Сериализатор = Новый СериализаторXDTO(Фабрика);
	Иначе
		Сериализатор = Новый СериализаторXDTO(ФабрикаXDTO);
	КонецЕсли;
	
	Возврат Сериализатор;
	
КонецФункции

// Инициализирует транспорт данных.
// 
// Параметры:
//  Тип - Строка - Тип транспорта данных: Загрузка, Выгрузка, ВыгрузкаПоСсылке
//  ПараметрыВыгрузки - См. ИнициализироватьПараметрыВыгрузки
//  АдресФайла - Строка - Адрес файла в хранилище.
//  XDTOАннотацияТипов - Булево
// 
// Возвращаемое значение:
//  Структура - Транспорт данных:
//   * Тип - Строка
//   * Сериализатор - СериализаторXDTO
//   * ПотокЗаписи  - ПотокВПамяти
//   * ЗаписьДанных - ЗаписьJSON
//                  - ЗаписьXML
//   * ЧтениеДанных - ЧтениеJSON
//                  - ЧтениеXML
//   * ПараметрыВыгрузки - См. ИнициализироватьПараметрыВыгрузки
//   * КлючиЗаписей - Соответствие Из См. ИнициализироватьКлючиЗаписей()
//   * Протокол     - См. ИнициализироватьПротокол()
//
Функция ИнициализироватьТранспортДанных(Тип, ПараметрыВыгрузки, АдресФайла = "", XDTOАннотацияТипов = Ложь)
	
	ТранспортДанных = Новый Структура;
	ТранспортДанных.Вставить("Тип",                     Тип);
	ТранспортДанных.Вставить("Сериализатор",            ИнициализироватьСериализатор(XDTOАннотацияТипов));
	ТранспортДанных.Вставить("КлючиЗаписей",            ИнициализироватьКлючиЗаписей(Тип = "Очистка"));
	ТранспортДанных.Вставить("ПараметрыВыгрузки",       ИнициализироватьПараметрыВыгрузки(Истина));
	ТранспортДанных.Вставить("МаксимальноеЧислоКлючей", 500000);
	ТранспортДанных.Вставить("Протокол",                ИнициализироватьПротокол());
	
	Если Тип = "Выгрузка" Или Тип = "ВыгрузкаПоСсылке" Тогда
		ТранспортДанных.Вставить("ПотокЗаписи", Новый ПотокВПамяти());
		Если ПараметрыВыгрузки.СтрогийФорматВыгрузки Тогда
			ЗаписьXML = Новый ЗаписьXML;
			ЗаписьXML.ОткрытьПоток(ТранспортДанных.ПотокЗаписи, "UTF-8", Ложь);
			ЗаписьXML.ЗаписатьОбъявлениеXML();
			ЗаписьXML.ЗаписатьНачалоЭлемента("Config");
			ЗаписьXML.ЗаписатьСоответствиеПространстваИмен("v8", "http://v8.1c.ru/8.1/data/enterprise/current-config");
			ЗаписьXML.ЗаписатьСоответствиеПространстваИмен("xs", "http://www.w3.org/2001/XMLSchema");
			ЗаписьXML.ЗаписатьСоответствиеПространстваИмен("xsi", "http://www.w3.org/2001/XMLSchema-instance");
			ЗаписьXML.ЗаписатьНачалоЭлемента("Settings");
			ЗаписьXML.ЗаписатьНачалоЭлемента("ОчищатьНеСовпадающиеЭлементыПриЗагрузке");
			ЗаписьXML.ЗаписатьТекст(XMLСтрока(ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке));
			ЗаписьXML.ЗаписатьКонецЭлемента();
			ЗаписьXML.ЗаписатьКонецЭлемента();
			ЗаписьXML.ЗаписатьНачалоЭлемента("Data");
			ТранспортДанных.Вставить("ЗаписьДанных", ЗаписьXML);
		Иначе
			ЗаписьJSON = Новый ЗаписьJSON;
			ЗаписьJSON.ПроверятьСтруктуру = Ложь;
			ЗаписьJSON.ОткрытьПоток(ТранспортДанных.ПотокЗаписи, "UTF-8", Ложь);
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
			ЗаписьJSON.ЗаписатьИмяСвойства("Config");
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
			ЗаписьJSON.ЗаписатьИмяСвойства("Settings");
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
			ЗаписьJSON.ЗаписатьИмяСвойства("ОчищатьНеСовпадающиеЭлементыПриЗагрузке");
			ЗаписьJSON.ЗаписатьЗначение(ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке);
			ЗаписьJSON.ЗаписатьКонецОбъекта();
			ЗаписьJSON.ЗаписатьИмяСвойства("Data");
			ЗаписьJSON.ЗаписатьНачалоМассива();
			ТранспортДанных.Вставить("ЗаписьДанных", ЗаписьJSON);
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(АдресФайла) Тогда
		
		Если Тип = "Загрузка" И ПараметрыВыгрузки.СтрогийФорматВыгрузки Тогда
			ЧтениеДанных = Новый ЧтениеXML;
		ИначеЕсли Тип = "Загрузка" Тогда
			ЧтениеДанных = Новый ЧтениеJSON;
		КонецЕсли;
		
		ДвоичныеДанные = ПолучитьИзВременногоХранилища(АдресФайла);
		ЧтениеДанных.ОткрытьПоток(ДвоичныеДанные.ОткрытьПотокДляЧтения(), "UTF-8");
		
		Если Не ПрочитатьБлокConfig(ЧтениеДанных) Тогда
			СообщитьПользователю(НСтр("ru = 'Неверный формат файла выгрузки'"));
			Возврат Неопределено;
		КонецЕсли;
		
		Если ЭтоБлокSettings(ЧтениеДанных) Тогда
			ПрочитатьБлокSettings(ЧтениеДанных, ТранспортДанных);
		КонецЕсли;
		
		Если ЭтоБлокData(ЧтениеДанных) Тогда
			ПрочитатьБлокData(ЧтениеДанных);
		Иначе
			СообщитьПользователю(НСтр("ru = 'Неверный формат файла выгрузки'"));
			Возврат Неопределено;
		КонецЕсли;
		
		ТранспортДанных.Вставить("ЧтениеДанных", ЧтениеДанных);
		
	КонецЕсли;
	
	Для Каждого КлючИЗначение Из ПараметрыВыгрузки Цикл
		Если НЕ КлючИЗначение.Значение = Неопределено Тогда
			ТранспортДанных.ПараметрыВыгрузки[КлючИЗначение.Ключ] = КлючИЗначение.Значение;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ТранспортДанных;
	
КонецФункции

Функция ПрочитатьБлокConfig(ЧтениеДанных)
	
	Результат = Истина;
	
	ЧтениеДанных.Прочитать();
	Если ТипЗнч(ЧтениеДанных) = Тип("ЧтениеJSON") Тогда
		ЧтениеДанных.Прочитать();
	КонецЕсли;
	
	Если ЭтоБлокConfig(ЧтениеДанных) Тогда
		ЧтениеДанных.Прочитать();
		Если ТипЗнч(ЧтениеДанных) = Тип("ЧтениеJSON") Тогда
			ЧтениеДанных.Прочитать();
		КонецЕсли;
	Иначе
		Результат = Ложь;
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции

Процедура ПрочитатьБлокSettings(ЧтениеДанных, ТранспортДанных)
	
	ЧтениеДанных.Прочитать();
	
	Если ТипЗнч(ЧтениеДанных) = Тип("ЧтениеJSON") Тогда
		ЧтениеДанных.Прочитать();
		Пока ЧтениеДанных.ТипТекущегоЗначения = ТипЗначенияJSON.ИмяСвойства Цикл
			ИмяСвойства = ЧтениеДанных.ТекущееЗначение;
			ЧтениеДанных.Прочитать();
			Если ИмяСвойства = "ОчищатьНеСовпадающиеЭлементыПриЗагрузке" Тогда
				ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке = ЧтениеДанных.ТекущееЗначение;
			КонецЕсли;
			ЧтениеДанных.Прочитать();
		КонецЦикла;
	Иначе
		Пока ЧтениеДанных.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
			ИмяЭлемента = ЧтениеДанных.Имя;
			ЧтениеДанных.Прочитать();
			Если ИмяЭлемента = "ОчищатьНеСовпадающиеЭлементыПриЗагрузке" Тогда
				ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеСовпадающиеЭлементыПриЗагрузке = XMLЗначение(Тип("Булево"), ЧтениеДанных.Значение);
			КонецЕсли;
			ЧтениеДанных.Прочитать();
			ЧтениеДанных.Прочитать();
		КонецЦикла;
	КонецЕсли;
	
	ЧтениеДанных.Прочитать();
	
КонецПроцедуры

Процедура ПрочитатьБлокData(ЧтениеДанных)
	
	ЧтениеДанных.Прочитать();
	
КонецПроцедуры

Функция ЭтоБлокConfig(ЧтениеДанных)
	
	Если ТипЗнч(ЧтениеДанных) = Тип("ЧтениеJSON") Тогда
		Результат = ЧтениеДанных.ТекущееЗначение = "Config";
	Иначе
		Результат = ЧтениеДанных.Имя = "Config";
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции

Функция ЭтоБлокSettings(ЧтениеДанных)
	
	Если ТипЗнч(ЧтениеДанных) = Тип("ЧтениеJSON") Тогда
		Результат = ЧтениеДанных.ТекущееЗначение = "Settings";
	Иначе
		Результат = ЧтениеДанных.Имя = "Settings";
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции

Функция ЭтоБлокData(ЧтениеДанных)
	
	Если ТипЗнч(ЧтениеДанных) = Тип("ЧтениеJSON") Тогда
		Результат = ЧтениеДанных.ТекущееЗначение = "Data";
	Иначе
		Результат = ЧтениеДанных.Имя = "Data";
	КонецЕсли;
	
	Возврат Результат;
	
КонецФункции

Процедура ЗаписатьДанныеВТранспортДанных(ТранспортДанных, Данные)
	
	Если ТранспортДанных.ПараметрыВыгрузки.СтрогийФорматВыгрузки Тогда
		ТранспортДанных.Сериализатор.ЗаписатьXML(ТранспортДанных.ЗаписьДанных, Данные);
	Иначе
		ТранспортДанных.Сериализатор.ЗаписатьJSON(ТранспортДанных.ЗаписьДанных, Данные, НазначениеТипаXML.Явное);
	КонецЕсли;
	
КонецПроцедуры

Функция ЗафиксироватьТранспортДанных(ТранспортДанных, ЗакончитьЗаписьДанных = Истина)
	
	Если ЗакончитьЗаписьДанных И ТранспортДанных.ПараметрыВыгрузки.СтрогийФорматВыгрузки Тогда
		ТранспортДанных.ЗаписьДанных.ЗаписатьКонецЭлемента();
		ТранспортДанных.ЗаписьДанных.ЗаписатьКонецЭлемента();
	ИначеЕсли ЗакончитьЗаписьДанных Тогда
		ТранспортДанных.ЗаписьДанных.ЗаписатьКонецМассива();
		ТранспортДанных.ЗаписьДанных.ЗаписатьКонецОбъекта();
		ТранспортДанных.ЗаписьДанных.ЗаписатьКонецОбъекта();
	КонецЕсли;
	ТранспортДанных.ЗаписьДанных.Закрыть();
	ДвоичныеДанные = ТранспортДанных.ПотокЗаписи.ЗакрытьИПолучитьДвоичныеДанные();
	
	Возврат ДвоичныеДанные;
	
КонецФункции

Функция ПрочитатьОбъектИзТранспортДанных(ТранспортДанных)
	
	Если ТранспортДанных.ПараметрыВыгрузки.СтрогийФорматВыгрузки Тогда
		ОбъектДанных = ПрочитатьОбъектИзЧтениеXML(ТранспортДанных);
	Иначе
		ОбъектДанных = ПрочитатьОбъектИзЧтениеJSON(ТранспортДанных);
	КонецЕсли;
	
	Возврат ОбъектДанных;
	
КонецФункции

Функция ЗначениеJSONТипОбъекта(ЗначениеJSON)
	
	Возврат ТипЗнч(ЗначениеJSON) = Тип("Соответствие")
			И ЗначениеJSON.Количество() = 2
			И ЗначениеJSON.Получить("#type") <> Неопределено
			И ЗначениеJSON.Получить("#value") <> Неопределено;
	
КонецФункции

Процедура ЗаписатьЗначениеВJSON(ЗаписьJSON, ЗначениеJSON, СвойствоЗначения, ЗаписыватьИмяСвойства = Истина)
	
	ПреобразованноеЗначениеJSON = ЗначениеJSON;
	ТипЗаписиЗначенияJSON       = 0;
	Если ЗначениеJSONТипОбъекта(ЗначениеJSON) Тогда
		ТипЗаписиЗначенияJSON = 1;
	ИначеЕсли ТипЗнч(ЗначениеJSON) = Тип("Соответствие") Тогда
		ТипЗаписиЗначенияJSON = 2;
	ИначеЕсли ТипЗнч(ЗначениеJSON) = Тип("Массив") Тогда
		ТипЗаписиЗначенияJSON = 3;
	Иначе
		ПреобразованноеЗначениеJSON = ПреобразоватьЗначениеПоТипуXDTO(ЗначениеJSON, СвойствоЗначения);
	КонецЕсли;
	
	Если ПреобразованноеЗначениеJSON = Null Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗаписыватьИмяСвойства Тогда
		ЗаписьJSON.ЗаписатьИмяСвойства(СвойствоЗначения.Имя);
	КонецЕсли;
	
	Если ТипЗаписиЗначенияJSON = 1 Тогда
		ЗаписьJSON.ЗаписатьНачалоОбъекта();
		ЗаписьJSON.ЗаписатьИмяСвойства("#type");
		ЗаписьJSON.ЗаписатьЗначение(ЗначениеJSON.Получить("#type"));
		ЗаписьJSON.ЗаписатьИмяСвойства("#value");
		ЗаписьJSON.ЗаписатьЗначение(ЗначениеJSON.Получить("#value"));
		ЗаписьJSON.ЗаписатьКонецОбъекта();
	ИначеЕсли ТипЗаписиЗначенияJSON = 2 Тогда
		ЗаписатьСоответствиеВJSON(ЗаписьJSON, ЗначениеJSON, СвойствоЗначения);
	ИначеЕсли ТипЗаписиЗначенияJSON = 3 Тогда
		ЗаписатьМассивВJSON(ЗаписьJSON, ЗначениеJSON, СвойствоЗначения);
	Иначе
		ЗаписьJSON.ЗаписатьЗначение(ПреобразованноеЗначениеJSON);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаписатьСоответствиеВJSON(ЗаписьJSON, ЗначениеJSON, СвойствоЗначения)
	
	ИнициализироватьТипыXDTO();
	СтруктураJSON = ПреобразоватьСоответствиеВСтруктуру(ЗначениеJSON);
	
	ОбъектXDTOСвойства = ФабрикаXDTO.Создать(СвойствоЗначения.Тип);
	ЗаписьJSON.ЗаписатьНачалоОбъекта();
	Для Каждого Свойство Из ОбъектXDTOСвойства.Свойства() Цикл
		Если Свойство.Тип = ТипыXDTO.FilterItem Тогда
			ОбъектXDTOFilterItem = ФабрикаXDTO.Создать(Свойство.Тип);
			Для Каждого СвойствоFilterItem Из ОбъектXDTOFilterItem.Свойства() Цикл
				ЗначениеСвойстваJSON = ПолучитьЗначениеСтруктурыJSON(СтруктураJSON, СвойствоFilterItem.Имя);
				ЗаписатьЗначениеВJSON(ЗаписьJSON, ЗначениеСвойстваJSON, СвойствоFilterItem);
			КонецЦикла;
		Иначе
			ЗначениеСвойстваJSON = ПолучитьЗначениеСтруктурыJSON(СтруктураJSON, Свойство.Имя);
			ЗаписатьЗначениеВJSON(ЗаписьJSON, ЗначениеСвойстваJSON, Свойство);
		КонецЕсли;
	КонецЦикла;
	ЗаписьJSON.ЗаписатьКонецОбъекта();
	
КонецПроцедуры

Процедура ЗаписатьМассивВJSON(ЗаписьJSON, ЗначениеJSON, СвойствоЗначения)
	
	ЗаписьJSON.ЗаписатьНачалоМассива();
	Для Каждого ЗначениеСвойстваJSON Из ЗначениеJSON Цикл
		ЗаписатьЗначениеВJSON(ЗаписьJSON, ЗначениеСвойстваJSON, СвойствоЗначения, Ложь);
	КонецЦикла;
	ЗаписьJSON.ЗаписатьКонецМассива();
	
КонецПроцедуры

Функция ПрочитатьОбъектИзЧтениеJSON(ТранспортДанных)
	
	ОбъектПочитан = Ложь;
	ОбъектДанных  = Неопределено;
	
	Пока Не ОбъектПочитан Цикл
		
		ЭтоГруппа = Ложь;
		
		Попытка
			
			ОбъектJSON = ПрочитатьJSON(ТранспортДанных.ЧтениеДанных, Истина);
			
			Если ОбъектJSON = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			ТипОбъектаJSON = СтрЗаменить(ОбъектJSON.Получить("#type"), "jcfg:", "");
			ТипОбъектаXDTO = ФабрикаXDTO.Тип("http://v8.1c.ru/8.1/data/enterprise/current-config", ТипОбъектаJSON);
			
			Если ТипОбъектаXDTO = Неопределено Тогда
				ШаблонСообщения = НСтр("ru = 'Ошибка поиска метаданных %1 в информационной базе'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ТипОбъектаJSON);
				СообщитьПользователю(ТекстСообщения);
				Продолжить;
			КонецЕсли;
			
			ЗаписьОбъектаJSON = Новый ЗаписьJSON;
			ЗаписьОбъектаJSON.УстановитьСтроку();
			ЗаписьОбъектаJSON.ЗаписатьНачалоМассива();
			ЗаписьОбъектаJSON.ЗаписатьНачалоОбъекта();
			ЗаписьОбъектаJSON.ЗаписатьИмяСвойства("#type");
			ЗаписьОбъектаJSON.ЗаписатьЗначение(ОбъектJSON.Получить("#type"));
			ЗаписьОбъектаJSON.ЗаписатьИмяСвойства("#value");
			ЗаписьОбъектаJSON.ЗаписатьНачалоОбъекта();
			
			СтруктураJSON = ПреобразоватьСоответствиеВСтруктуру(ОбъектJSON.Получить("#value"));
			ЭтоГруппа = СтруктураJSON.Свойство("IsFolder") И СтруктураJSON.IsFolder;
			ПустойОбъектXDTO = СоздатьПустойОбъектXDTO(ТранспортДанных.Сериализатор, ТипОбъектаXDTO, ЭтоГруппа);
			СвойстваXDTO = ПолучитьСвойстваОбъектаXDTO(ПустойОбъектXDTO.ОбъектXDTO);
			ПредопределенныеНаименования = ИнициализироватьПредопределенныеНаименованияЛатиница();
			
			Для Каждого СвойствоXDTO Из СвойстваXDTO Цикл
				
				ЗначениеJSON = ПолучитьЗначениеСтруктурыJSON(СтруктураJSON, СвойствоXDTO.Имя);
				Если Не ПроверитьСвойствоОбъекта(ПустойОбъектXDTO.ОбъектИБ, СвойствоXDTO.Имя)
					Или ЗначениеJSON = Null
						И (
							ТипЗнч(ПустойОбъектXDTO.ОбъектXDTO[СвойствоXDTO.Имя]) = Тип("СписокXDTO")
							Или ПредопределенныеНаименования.Получить(СвойствоXDTO.Имя) <> Неопределено
						) Тогда
					Продолжить;
				КонецЕсли;
				
				ЗаписатьЗначениеВJSON(ЗаписьОбъектаJSON, ЗначениеJSON, СвойствоXDTO);
				
			КонецЦикла;
			
			ЗаписьОбъектаJSON.ЗаписатьКонецОбъекта();
			ЗаписьОбъектаJSON.ЗаписатьКонецОбъекта();
			ЗаписьОбъектаJSON.ЗаписатьКонецМассива();
			СтрокаJSON = ЗаписьОбъектаJSON.Закрыть();
			
			ЧтениеОбъектаJSON = Новый ЧтениеJSON();
			ЧтениеОбъектаJSON.УстановитьСтроку(СтрокаJSON);
			ЧтениеОбъектаJSON.Прочитать();
			
			ОбъектДанных  = ТранспортДанных.Сериализатор.ПрочитатьJSON(ЧтениеОбъектаJSON);
			ОбъектПочитан = Истина;
			
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ИмяЭлемента = "";
			ШаблонСообщения = НСтр("ru = 'Ошибка чтения элемента %1 объекта метаданных %2 в информационной базе:
									|
									| %3'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ТипОбъектаJSON, ИмяЭлемента, ОписаниеОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, "", 2, ТекстСообщения, Новый Структура);
		КонецПопытки;
		
	КонецЦикла;
	
	Возврат ОбъектДанных;
	
КонецФункции

Процедура ИнициализироватьТипыXDTO()
	
	Если НЕ ТипыXDTO = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТипыXDTO = Новый Структура;
	ТипыXDTO.Вставить("anyType",         ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema",    "anyType"));
	ТипыXDTO.Вставить("FilterItem",      ФабрикаXDTO.Тип("http://v8.1c.ru/8.1/data/enterprise", "FilterItem"));
	ТипыXDTO.Вставить("string",          ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema",    "string"));
	ТипыXDTO.Вставить("boolean",         ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema",    "boolean"));
	ТипыXDTO.Вставить("decimal",         ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema",    "decimal"));
	ТипыXDTO.Вставить("dateTime",        ФабрикаXDTO.Тип("http://www.w3.org/2001/XMLSchema",    "dateTime"));
	ТипыXDTO.Вставить("UUID",            ФабрикаXDTO.Тип("http://v8.1c.ru/8.1/data/core",       "UUID"));
	ТипыXDTO.Вставить("ValueStorage",    ФабрикаXDTO.Тип("http://v8.1c.ru/8.1/data/core",       "ValueStorage"));
	ТипыXDTO.Вставить("AnyDBRef",        ФабрикаXDTO.Тип("http://v8.1c.ru/8.1/data/enterprise", "AnyDBRef"));
	
КонецПроцедуры

Функция ПолучитьТипЗначенияПоТипуXDTO(ТипXDTO)
	
	ИнициализироватьТипыXDTO();
	
	Если ТипXDTO = ТипыXDTO.boolean Тогда
		ТипЗначения = Тип("Булево");
	ИначеЕсли ТипXDTO = ТипыXDTO.decimal Тогда
		ТипЗначения = Тип("Число");
	ИначеЕсли ТипXDTO = ТипыXDTO.string Тогда
		ТипЗначения = Тип("Строка");
	ИначеЕсли ТипXDTO = ТипыXDTO.dateTime Тогда
		ТипЗначения = Тип("Дата");
	ИначеЕсли ТипXDTO = ТипыXDTO.UUID Тогда
		ТипЗначения = Тип("УникальныйИдентификатор");
	ИначеЕсли ТипXDTO = ТипыXDTO.ValueStorage Тогда
		ТипЗначения = Тип("ХранилищеЗначения");
	ИначеЕсли ТипXDTO.БазовыйТип = ТипыXDTO.AnyDBRef Тогда
		ТипЗначения = Тип(ТипXDTO.Имя);
	Иначе
		ТипЗначения = Неопределено;
	КонецЕсли;
	
	Возврат ТипЗначения;
	
КонецФункции

Функция ПреобразоватьЗначениеПоТипуXDTO(Значение, СвойствоЗначения)
	
	ТипЗначения         = ТипЗнч(Значение);
	ТипСвойстваЗначения = ПолучитьТипЗначенияПоТипуXDTO(СвойствоЗначения.Тип);
	
	Если Значение = Null
		И (
			СвойствоЗначения.Имя = "TypeSet"
			ИЛИ СвойствоЗначения.Имя = "TypeId"
			ИЛИ СвойствоЗначения.Имя = "BinaryDataQualifiers"
			ИЛИ СвойствоЗначения.Имя = "NumberQualifiers"
			ИЛИ СвойствоЗначения.Имя = "StringQualifiers"
			ИЛИ СвойствоЗначения.Имя = "DateQualifiers"
		) Тогда
		ПреобразованноеЗначение = Значение;
	ИначеЕсли НЕ ТипСвойстваЗначения = Неопределено И ЭтоПростойТип(ТипСвойстваЗначения) И НЕ ТипЗначения = ТипСвойстваЗначения Тогда
		ТипыДляОписания = Новый Массив;
		ТипыДляОписания.Добавить(ТипСвойстваЗначения);
		ОписаниеТипов = Новый ОписаниеТипов(ТипыДляОписания);
		ПреобразованноеЗначение = ОписаниеТипов.ПривестиЗначение(Значение);
		Если ТипЗнч(ПреобразованноеЗначение) = Тип("ХранилищеЗначения") Тогда
			ПреобразованноеЗначение = "AQEIAAAAAAAAAO+7v3siVSJ9";
		ИначеЕсли ТипЗнч(ПреобразованноеЗначение) = Тип("УникальныйИдентификатор") Тогда
			ПреобразованноеЗначение = Строка(ПреобразованноеЗначение);
		ИначеЕсли ТипЗнч(ПреобразованноеЗначение) = Тип("Дата") Тогда
			ПреобразованноеЗначение = ЗаписатьДатуJSON(ПреобразованноеЗначение, ФорматДатыJSON.ISO);
		КонецЕсли;
	ИначеЕсли НЕ ТипСвойстваЗначения = Неопределено И ЭтоСсылка(ТипСвойстваЗначения, Истина) И НЕ ЗначениеЗаполнено(Значение) Тогда
		Если СтрНачинаетсяС(СвойствоЗначения.Тип.Имя, "EnumRef") Тогда
			ПреобразованноеЗначение = "";
		Иначе
			ПреобразованноеЗначение = "00000000-0000-0000-0000-000000000000";
		КонецЕсли;
	ИначеЕсли СвойствоЗначения.Тип = ТипыXDTO.anyType И Не ЗначениеЗаполнено(Значение) Тогда
		ПреобразованноеЗначение = Неопределено;
	ИначеЕсли Значение = Null Тогда
		ПреобразованноеЗначение = Неопределено;
	Иначе
		ПреобразованноеЗначение = Значение;
	КонецЕсли;
	
	Возврат ПреобразованноеЗначение;
	
КонецФункции

Функция СоздатьПустойОбъектXDTO(Сериализатор, ТипОбъектаXDTO, ЭтоГруппа)
	
	ПустойОбъектXDTO = Новый Структура;
	
	ЧастиИмени = РазложитьСтрокуВМассивПодстрок(ТипОбъектаXDTO.Имя);
	
	Если ЧастиИмени.Количество() >= 2 Тогда
		КлассОМ = ЧастиИмени[0];
		ИмяОМ = ЧастиИмени[1];
	КонецЕсли;
	
	Если КлассОМ = "ExchangePlanObject" Тогда
		ОбъектИБ = ПланыОбмена[ИмяОМ].СоздатьУзел();
	ИначеЕсли КлассОМ = "CatalogObject" И НЕ ЭтоГруппа Тогда
		ОбъектИБ = Справочники[ИмяОМ].СоздатьЭлемент();
	ИначеЕсли КлассОМ = "CatalogObject" И ЭтоГруппа Тогда
		ОбъектИБ = Справочники[ИмяОМ].СоздатьГруппу();
	ИначеЕсли КлассОМ = "DocumentObject" Тогда
		ОбъектИБ = Документы[ИмяОМ].СоздатьДокумент();
	ИначеЕсли КлассОМ = "ChartOfCharacteristicTypesObject" Тогда
		ОбъектИБ = ПланыВидовХарактеристик[ИмяОМ].СоздатьЭлемент();
	ИначеЕсли КлассОМ = "ChartOfAccountsObject" Тогда
		ОбъектИБ = ПланыСчетов[ИмяОМ].СоздатьСчет();
	ИначеЕсли КлассОМ = "ChartOfCalculationTypesObject" Тогда
		ОбъектИБ = ПланыВидовРасчета[ИмяОМ].СоздатьВидРасчета();
	ИначеЕсли КлассОМ = "InformationRegisterRecordSet" Тогда
		ОбъектИБ = РегистрыСведений[ИмяОМ].СоздатьНаборЗаписей();
	ИначеЕсли КлассОМ = "AccumulationRegisterRecordSet" Тогда
		ОбъектИБ = РегистрыНакопления[ИмяОМ].СоздатьНаборЗаписей();
	ИначеЕсли КлассОМ = "AccountingRegisterRecordSet" Тогда
		ОбъектИБ = РегистрыБухгалтерии[ИмяОМ].СоздатьНаборЗаписей();
	ИначеЕсли КлассОМ = "CalculationRegisterRecordSet" Тогда
		ОбъектИБ = РегистрыРасчета[ИмяОМ].СоздатьНаборЗаписей();
	ИначеЕсли КлассОМ = "BusinessProcessObject" Тогда
		ОбъектИБ = БизнесПроцессы[ИмяОМ].СоздатьБизнесПроцесс();
	ИначеЕсли КлассОМ = "TaskObject" Тогда
		ОбъектИБ = Задачи[ИмяОМ].СоздатьЗадачу();
	Иначе
		ОбъектИБ = Неопределено;
	КонецЕсли;
	
	Если ОбъектИБ = Неопределено Тогда
		ОбъектXDTO = ФабрикаXDTO.Создать(ТипОбъектаXDTO);
	Иначе
		ОбъектXDTO = Сериализатор.ЗаписатьXDTO(ОбъектИБ);
	КонецЕсли;
	
	ПустойОбъектXDTO.Вставить("ОбъектИБ",   ОбъектИБ);
	ПустойОбъектXDTO.Вставить("ОбъектXDTO", ОбъектXDTO);
	
	Возврат ПустойОбъектXDTO;
	
КонецФункции

Функция ПроверитьСвойствоОбъекта(Объект, Свойство)
	
	ПустоеЗначение = Null;
	
	ПроверкаСвойства = Новый Структура(Свойство, ПустоеЗначение);
	ЗаполнитьЗначенияСвойств(ПроверкаСвойства, Объект, Свойство);
	
	Возврат ПроверкаСвойства[Свойство] <> ПустоеЗначение;
	
КонецФункции

// Свойства при записи должны идти в следующем порядке:
// - Стандартные реквизиты (по алфавиту).
// - Пользовательские реквизиты (по алфавиту).
// - Стандартные табличные части (по алфавиту).
// - Пользовательские табличные части (по алфавиту).
//
Функция ПолучитьСвойстваОбъектаXDTO(ОбъектXDTO)
	
	ПредопределенныеНаименования = ИнициализироватьПредопределенныеНаименованияЛатиница();
	
	Свойства = Новый ТаблицаЗначений;
	Свойства.Колонки.Добавить("Свойство");
	Свойства.Колонки.Добавить("Индекс",  Новый ОписаниеТипов("Число"));
	Свойства.Колонки.Добавить("Порядок", Новый ОписаниеТипов("Число"));
	
	Индекс = 0;
	Для Каждого Свойство Из ОбъектXDTO.Свойства() Цикл
		Стандартный = ПредопределенныеНаименования.Получить(Свойство.Имя) <> Неопределено;
		Таблица     = ТипЗнч(ОбъектXDTO[Свойство.Имя]) = Тип("СписокXDTO");
		НоваяСтрока = Свойства.Добавить();
		НоваяСтрока.Свойство = Свойство;
		НоваяСтрока.Индекс   = Индекс;
		Если Не Стандартный И Не Таблица Тогда
			НоваяСтрока.Порядок = 1;
		ИначеЕсли Стандартный И Таблица Тогда
			НоваяСтрока.Порядок = 2;
		ИначеЕсли Не Стандартный И Таблица Тогда
			НоваяСтрока.Порядок = 3;
		Иначе
			НоваяСтрока.Порядок = 0;
		КонецЕсли;
		Индекс = Индекс + 1;
	КонецЦикла;
	
	Свойства.Сортировать("Порядок Возр, Индекс Возр");
	
	Возврат Свойства.ВыгрузитьКолонку("Свойство");
	
КонецФункции

Функция ПрочитатьОбъектИзЧтениеXML(ТранспортДанных)
	
	ОбъектПочитан = Ложь;
	ОбъектДанных  = Неопределено;
	
	Пока Не ОбъектПочитан Цикл
		
		ВозможностьЧтения = ТранспортДанных.Сериализатор.ВозможностьЧтенияXML(ТранспортДанных.ЧтениеДанных);
		ИмяОбъектаМетаданных = ТранспортДанных.ЧтениеДанных.ЛокальноеИмя;
		
		Если Не ВозможностьЧтения Тогда
			
			Если ТранспортДанных.ЧтениеДанных.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
				Прервать;
			КонецЕсли;
			
			ШаблонСообщения = НСтр("ru = 'Ошибка разбора метаданных %1 в информационной базе'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ИмяОбъектаМетаданных);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ИмяОбъектаМетаданных, 2, ТекстСообщения, Новый Структура);
			
			Если Не ПрочитатьXMLДоСледующегоОбъекта(ТранспортДанных.ЧтениеДанных) Тогда
				Прервать;
			КонецЕсли;
			
			Продолжить;
			
		КонецЕсли;
		
		Попытка
			ОбъектДанных  = ТранспортДанных.Сериализатор.ПрочитатьXML(ТранспортДанных.ЧтениеДанных);
			ОбъектПочитан = Истина;
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ИмяЭлемента = ТранспортДанных.ЧтениеДанных.ЛокальноеИмя;
			ШаблонСообщения = НСтр("ru = 'Ошибка чтения элемента %1 объекта метаданных %2 в информационной базе:
									|
									| %3'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ИмяОбъектаМетаданных, ИмяЭлемента, ОписаниеОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ИмяОбъектаМетаданных, 2, ТекстСообщения, Новый Структура);
		КонецПопытки;
		
		Если Не ОбъектПочитан Тогда
			Если Не ПрочитатьXMLДоСледующегоОбъекта(ТранспортДанных.ЧтениеДанных) Тогда
				Прервать;
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат ОбъектДанных;
	
КонецФункции

Функция ПрочитатьXMLДоСледующегоОбъекта(ЧтениеXML)
	
	ПрочитанСледующий = Истина;
	Если ЭтоНачалоЭлементаXMLОбъекта(ЧтениеXML) Тогда
		ЧтениеXML.Пропустить();
		Если Не ЧтениеXML.Прочитать() Тогда
			ПрочитанСледующий = Ложь;
		КонецЕсли;
	Иначе
		Пока НЕ ЭтоНачалоЭлементаXMLОбъекта(ЧтениеXML) Цикл
			Если Не ЧтениеXML.Прочитать() Тогда
				ПрочитанСледующий = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Возврат ПрочитанСледующий;
	
КонецФункции

Функция ЭтоНачалоЭлементаXMLОбъекта(ЧтениеXML)
	
	Возврат (ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента
		И СтрПодобнаПоРегулярномуВыражению(ЧтениеXML.Имя, "v8:[A-Z,a-z]*[.].*"));
	
КонецФункции

Процедура ЗаписатьКлючиЗаписейПоТипуВТранспортДанных(ТранспортДанных, ЭлементОбъектовМетаданных)
	
	КлючиЗаписейПоТипу = ТранспортДанных.КлючиЗаписей.Получить(ЭлементОбъектовМетаданных.ПолноеИмя);
	Если КлючиЗаписейПоТипу = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ШаблонСообщенияВыгрузки = НСтр("ru = 'Выгружено элементов объекта метаданных %1:
								|%2'");
	ШаблонСообщенияНеВыгрузки = НСтр("ru = 'Не выгружено элементов объекта метаданных %1:
								|%2'");
	КоличествоКлючей = 0;
	КоличествоНеВыгруженныхКлючей = 0;
	
	ОписаниеОбъекта = КлючиЗаписейПоТипу.Получить("ОписаниеОбъекта");
	ПараметрыВыгрузки = ПреобразоватьСтрокуВПараметрыВыгрузки(ЭлементОбъектовМетаданных.ПараметрыВыгрузки);
	
	Для Каждого КлючЗаписи Из КлючиЗаписейПоТипу Цикл
		Если КлючЗаписи.Ключ = "ОписаниеОбъекта" Тогда
			Продолжить;
		КонецЕсли;
		КлючЗаписан = ЗаписатьКлючЗаписиВТранспортДанных(ТранспортДанных, КлючЗаписи.Значение.Значение, ОписаниеОбъекта, ПараметрыВыгрузки.НеВыгружаемыеПоля);
		Если КлючЗаписан Тогда
			КоличествоКлючей = КоличествоКлючей + 1;
		Иначе
			КоличествоНеВыгруженныхКлючей = КоличествоНеВыгруженныхКлючей + 1;
		КонецЕсли;
	КонецЦикла;
	
	Если КоличествоКлючей > 0 Тогда
		ТекстСообщения = СтрШаблон(
			ШаблонСообщенияВыгрузки,
			ЭлементОбъектовМетаданных.ПолноеИмя,
			Формат(КоличествоКлючей, "ЧН=0; ЧГ=0")
		);
		СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
	Если КоличествоНеВыгруженныхКлючей > 0 Тогда
		ТекстСообщения = СтрШаблон(
			ШаблонСообщенияНеВыгрузки,
			ЭлементОбъектовМетаданных.ПолноеИмя,
			Формат(КоличествоНеВыгруженныхКлючей, "ЧН=0; ЧГ=0")
		);
		СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	
КонецПроцедуры

Функция ЗаписатьКлючЗаписиВТранспортДанных(ТранспортДанных, КлючЗаписиОбъекта, ОписаниеОбъекта, НеВыгружаемыеПоля)
	
	КлючЗаписан = Ложь;
	
	Если ЭтоТаблицаНабораДанных(ОписаниеОбъекта.ПолноеИмя) Тогда
		Попытка
			НаборЗаписей = МенеджерОбъектаПоПолномуИмени(ОписаниеОбъекта.ПолноеИмя).СоздатьНаборЗаписей();
			Отбор        = НаборЗаписей.Отбор;
			Для Каждого КлючОтбора Из КлючЗаписиОбъекта Цикл
				Отбор[КлючОтбора.Ключ].Значение      = КлючОтбора.Значение;
				Отбор[КлючОтбора.Ключ].ВидСравнения  = ВидСравнения.Равно;
				Отбор[КлючОтбора.Ключ].Использование = Истина;
			КонецЦикла;
			НаборЗаписей.Прочитать();
			Если ПроверитьСсылочнуюЦелостностьВыгружаемыхДанных(ТранспортДанных, НаборЗаписей, ОписаниеОбъекта, НеВыгружаемыеПоля) Тогда
				ЗаписатьДанныеВТранспортДанных(ТранспортДанных, НаборЗаписей); 
				КлючЗаписан = Истина;
			КонецЕсли;
		Исключение
			СтрокаОписанияОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("ru = 'При выгрузке регистра %1 возникла ошибка:
				|%3'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОбъекта.ПолноеИмя, СтрокаОписанияОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения, КлючЗаписиОбъекта, ОписаниеОбъекта);
			ВызватьИсключение ТекстСообщения;
		КонецПопытки;
	ИначеЕсли ЭтоТаблицаСсылочныхДанных(ОписаниеОбъекта.ПолноеИмя) Тогда
		ОбъектДанных = КлючЗаписиОбъекта.Ссылка.ПолучитьОбъект();
		Попытка
			//@skip-check query-in-loop
			Если ПроверитьСсылочнуюЦелостностьВыгружаемыхДанных(ТранспортДанных, ОбъектДанных, ОписаниеОбъекта, НеВыгружаемыеПоля) Тогда
				ЗаписатьДанныеВТранспортДанных(ТранспортДанных, ОбъектДанных);
				Если ЭтоТаблицаДокумент(ОписаниеОбъекта.ПолноеИмя) Тогда
					Для Каждого Движение Из ОбъектДанных.Движения Цикл
						Движение.Прочитать();
						ЗаписатьДанныеВТранспортДанных(ТранспортДанных, Движение);
					КонецЦикла;
				КонецЕсли;
				КлючЗаписан = Истина;
			КонецЕсли;
		Исключение
			СтрокаОписанияОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("ru = 'При выгрузке объекта %1(%2) возникла ошибка:
				|%3'");
			СтрокаСообщения = СтрШаблон(ШаблонСообщения, ОбъектДанных, ТипЗнч(ОбъектДанных), СтрокаОписанияОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения, КлючЗаписиОбъекта.Ссылка, ОписаниеОбъекта);
			ВызватьИсключение СтрокаСообщения;
		КонецПопытки;
	ИначеЕсли ЭтоКонстанта(ОписаниеОбъекта.ПолноеИмя) Тогда
		Попытка
			МенеджерЗначения = МенеджерОбъектаПоПолномуИмени(КлючЗаписиОбъекта).СоздатьМенеджерЗначения();
			МенеджерЗначения.Прочитать();
			ЗаписатьДанныеВТранспортДанных(ТранспортДанных, МенеджерЗначения);
			КлючЗаписан = Истина;
		Исключение
			СтрокаОписанияОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("ru = 'При выгрузке константы %1 возникла ошибка:
				|%2'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОбъекта.ПолноеИмя, СтрокаОписанияОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения);
			ВызватьИсключение ТекстСообщения;
		КонецПопытки;
	КонецЕсли;
	
	Возврат КлючЗаписан;
	
КонецФункции

Функция СравнитьОбъектДанныхСДаннымиВБазе(ОбъектДанных, ОписаниеОбъекта, ТранспортДанных)
	
	Если ЭтоТаблицаНабораДанных(ОписаниеОбъекта.ПолноеИмя) Тогда
		Попытка
			ОбъектДанныхВБазе = МенеджерОбъектаПоПолномуИмени(ОписаниеОбъекта.ПолноеИмя).СоздатьНаборЗаписей();
			Отбор             = ОбъектДанныхВБазе.Отбор;
			Для Каждого ЭлементОтбора Из ОбъектДанных.Отбор Цикл
				Отбор[ЭлементОтбора.Имя].Значение      = ЭлементОтбора.Значение;
				Отбор[ЭлементОтбора.Имя].ВидСравнения  = ВидСравнения.Равно;
				Отбор[ЭлементОтбора.Имя].Использование = Истина;
			КонецЦикла;
			ОбъектДанныхВБазе.Прочитать();
			Если ОбъектДанныхВБазе.Количество() = 0 Тогда
				Возврат Ложь;
			КонецЕсли;
		Исключение
			СтрокаОписанияОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("ru = 'При анализе регистра %1 возникла ошибка:
				|%3'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОбъекта.ПолноеИмя, СтрокаОписанияОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения, ОбъектДанных.Отбор, ОписаниеОбъекта);
			ВызватьИсключение ТекстСообщения;
		КонецПопытки;
	ИначеЕсли ЭтоТаблицаСсылочныхДанных(ОписаниеОбъекта.ПолноеИмя) Тогда
		Если Не ЗначениеЗаполнено(ОбъектДанных.Ссылка) Тогда
			Возврат Ложь;
		КонецЕсли;
		Попытка
			ОбъектДанныхВБазе = ОбъектДанных.Ссылка.ПолучитьОбъект();
		Исключение
			СтрокаОписанияОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("ru = 'При выгрузке объекта %1(%2) возникла ошибка:
				|%3'");
			СтрокаСообщения = СтрШаблон(ШаблонСообщения, ОбъектДанных, ТипЗнч(ОбъектДанных), СтрокаОписанияОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения, ОбъектДанных.Ссылка, ОписаниеОбъекта);
			ВызватьИсключение СтрокаСообщения;
		КонецПопытки;
	ИначеЕсли ЭтоКонстанта(ОписаниеОбъекта.ПолноеИмя) Тогда
		Попытка
			ОбъектДанныхВБазе = МенеджерОбъектаПоПолномуИмени(ОписаниеОбъекта.ПолноеИмя).СоздатьМенеджерЗначения();
			ОбъектДанныхВБазе.Прочитать();
		Исключение
			СтрокаОписанияОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("ru = 'При выгрузке константы %1 возникла ошибка:
				|%2'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОбъекта.ПолноеИмя, СтрокаОписанияОшибки);
			ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения);
			ВызватьИсключение ТекстСообщения;
		КонецПопытки;
	КонецЕсли;
	
	ХешОбъекта      = ПолучитьХешОбъекта(ОбъектДанных);
	ХешОбъектаВБазе = ПолучитьХешОбъекта(ОбъектДанныхВБазе); 
	
	Возврат ХешОбъекта = ХешОбъектаВБазе;
	
КонецФункции

Процедура ПолучитьКонстанты(ЭлементОбъектовМетаданных, ТранспортДанных)
	
	ОбъектМетаданных  = Метаданные.НайтиПоПолномуИмени(ЭлементОбъектовМетаданных.ПолноеИмя);
	ОписаниеОбъекта   = ПолучитьОписаниеОбъекта(ОбъектМетаданных, ЭлементОбъектовМетаданных.Раздел, Новый Соответствие);
	
	ДобавитьКлючЗаписи(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей, ОписаниеОбъекта.ПолноеИмя);
	ДобавитьОписаниеОбъектаДляКлючейЗаписи(ТранспортДанных.КлючиЗаписей, ОписаниеОбъекта);
	
КонецПроцедуры

Процедура ПолучитьНаборДанныхРегистра(ЭлементОбъектовМетаданных, ТранспортДанных)
	
	НаборЗаписей   = МенеджерОбъектаПоПолномуИмени(ЭлементОбъектовМетаданных.ПолноеИмя).СоздатьНаборЗаписей();
	Отбор          = НаборЗаписей.Отбор;
	ВсеПоляОтбора  =
		НЕ ЭлементОбъектовМетаданных.Свойство("Отбор")
		ИЛИ Не ЗначениеЗаполнено(ЭлементОбъектовМетаданных.Отбор)
		ИЛИ ЭлементОбъектовМетаданных.Отбор.Свойство("Все")
			И ЭлементОбъектовМетаданных.Отбор.Все = "*";
	ПоляВыборки    = Новый Массив;
	
	ЗначенияОбъекта   = Новый ТаблицаЗначений;
	Для Каждого ЭлементОтбора Из Отбор Цикл
		Если НЕ ВсеПоляОтбора И НЕ ЭлементОбъектовМетаданных.Отбор.Свойство(ЭлементОтбора.Имя) Тогда
			Продолжить;
		КонецЕсли;
		ЗначенияОбъекта.Колонки.Добавить(ЭлементОтбора.Имя, ЭлементОтбора.ТипЗначения);
		ПоляВыборки.Добавить(ЭлементОтбора.Имя);
	КонецЦикла;
	
	ПараметрыВыгрузки = ПреобразоватьСтрокуВПараметрыВыгрузки(ЭлементОбъектовМетаданных.ПараметрыВыгрузки);
	Если Не ПараметрыВыгрузки.НастройкиВыгрузки = Неопределено Тогда
		ПолучитьПоляВыборкиРекурсивно( ПараметрыВыгрузки.НастройкиВыгрузки.Отбор.Элементы, ПоляВыборки);
	КонецЕсли;
	
	Если ПоляВыборки.Количество() > 0 Тогда
		СтрокаПолейДляВыборки = СтрШаблон(
			"ПЕРВЫЕ %1 %2",
			Формат(ТранспортДанных.МаксимальноеЧислоКлючей, "ЧГ=0"),
			СтрСоединить(ПоляВыборки, ",")
		);
	Иначе
		СтрокаПолейДляВыборки = "*";
	КонецЕсли;
	
	НастройкиКомпоновкиДанных = Новый КомпоновщикНастроекКомпоновкиДанных;
	СхемаКомпоновкиДанных = ПодготовитьСхемуКомпоновкиДанных(ЭлементОбъектовМетаданных, СтрокаПолейДляВыборки, НЕ ВсеПоляОтбора);
	ЗагрузитьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, ПараметрыВыгрузки.НастройкиВыгрузки, Новый УникальныйИдентификатор);
	Данные = ВывестиСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных);
	
	ОписаниеОбъекта   = Неопределено;
	ОписанияОбъектов  = Новый Соответствие;
	ИспользуетсяОтбор = ИспользуетсяОтборСКД(ПараметрыВыгрузки.НастройкиВыгрузки); 
	
	Для Каждого Строка Из Данные Цикл
		
		Если ОписаниеОбъекта = Неопределено И (ТранспортДанных.Тип = "Выгрузка" ИЛИ ТранспортДанных.Тип = "ВыгрузкаПоСсылке") Тогда
			ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ЭлементОбъектовМетаданных.ПолноеИмя);
			ОписаниеОбъекта = ПолучитьОписаниеОбъекта(ОбъектМетаданных, ЭлементОбъектовМетаданных.Раздел, Новый Соответствие);
			ДобавитьОписаниеОбъектаДляКлючейЗаписи(ТранспортДанных.КлючиЗаписей, ОписаниеОбъекта);
		КонецЕсли;
		
		// TO DO: Проверить корректность отбора на регистр расчета
		КлючЗаписиОбъекта = Новый Структура;
		Для Каждого ЭлементОтбора Из НаборЗаписей.Отбор Цикл
			Если НЕ ВсеПоляОтбора
				И НЕ ЭлементОбъектовМетаданных.Отбор.Свойство(ЭлементОтбора.Имя) Тогда
				Продолжить;
			КонецЕсли;
			КлючЗаписиОбъекта.Вставить(ЭлементОтбора.Имя, Строка[ЭлементОтбора.Имя]);
		КонецЦикла;
		
		Если НаборЗаписей.Отбор.Количество() > 0
			И ИспользуетсяОтбор Тогда
			НоваяСтрока = ЗначенияОбъекта.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
		КонецЕсли;
		
		ДобавитьКлючЗаписи(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей, КлючЗаписиОбъекта);
		
	КонецЦикла;
	
	Данные.Очистить();
	
	Если ТранспортДанных.Тип = "ВыгрузкаПоСсылке" И Не ОписаниеОбъекта = Неопределено Тогда
		НайтиСсылкиПоОбъектуМетаданных(ОписаниеОбъекта, ЗначенияОбъекта, ТранспортДанных.КлючиЗаписей, ОписанияОбъектов, ПараметрыВыгрузки.НеВыгружаемыеПоля);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПолучитьДанныеОбъектногоТипа(ЭлементОбъектовМетаданных, ТранспортДанных)
	
	ПоляВыборки = Новый Массив;
	ПоляВыборки.Добавить("Ссылка");
	ПоляВыборки.Добавить("ПометкаУдаления");
	
	ПараметрыВыгрузки = ПреобразоватьСтрокуВПараметрыВыгрузки(ЭлементОбъектовМетаданных.ПараметрыВыгрузки);
	Если Не ПараметрыВыгрузки.НастройкиВыгрузки = Неопределено Тогда
		ПолучитьПоляВыборкиРекурсивно(ПараметрыВыгрузки.НастройкиВыгрузки.Отбор.Элементы, ПоляВыборки);
	КонецЕсли;
	
	СтрокаПолейДляВыборки = СтрШаблон(
		"ПЕРВЫЕ %1 %2",
		Формат(ТранспортДанных.МаксимальноеЧислоКлючей, "ЧГ=0"),
		СтрСоединить(ПоляВыборки, ",")
	);
	НастройкиКомпоновкиДанных = Новый КомпоновщикНастроекКомпоновкиДанных;
	СхемаКомпоновкиДанных = ПодготовитьСхемуКомпоновкиДанных(ЭлементОбъектовМетаданных, СтрокаПолейДляВыборки, Ложь, ТранспортДанных.Тип = "Очистка");
	ЗагрузитьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, ПараметрыВыгрузки.НастройкиВыгрузки, Новый УникальныйИдентификатор);
	Если НЕ ТранспортДанных.ПараметрыВыгрузки.ВыгружатьПомеченныеНаУдаление Тогда
		ДобавитьУсловиеПометкаУдаления(НастройкиКомпоновкиДанных);
	КонецЕсли;
	Данные = ВывестиСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных);
	
	ОписаниеОбъекта   = Неопределено;
	ОписанияОбъектов  = Новый Соответствие;
	ЗначенияОбъекта   = Новый Массив;
	ИспользуетсяОтбор = ИспользуетсяОтборСКД(ПараметрыВыгрузки.НастройкиВыгрузки);
	
	Для Каждого Строка Из Данные Цикл
		
		Если ОписаниеОбъекта = Неопределено И (ТранспортДанных.Тип = "Выгрузка" ИЛИ ТранспортДанных.Тип = "ВыгрузкаПоСсылке") Тогда
			ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ЭлементОбъектовМетаданных.ПолноеИмя);
			ОписаниеОбъекта = ПолучитьОписаниеОбъекта(ОбъектМетаданных, ЭлементОбъектовМетаданных.Раздел, Новый Соответствие);
			ДобавитьОписаниеОбъектаДляКлючейЗаписи(ТранспортДанных.КлючиЗаписей, ОписаниеОбъекта);
			ОписанияОбъектов.Вставить(ТипЗнч(Строка.Ссылка), ОписаниеОбъекта)
		КонецЕсли;
		
		КлючЗаписиОбъекта = Новый Структура("Ссылка, ПометкаУдаления", Строка.Ссылка, Строка.ПометкаУдаления);
		КлючДобавлен = ДобавитьКлючЗаписи(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей, КлючЗаписиОбъекта);
		Если ИспользуетсяОтбор И КлючДобавлен Тогда
			ЗначенияОбъекта.Добавить(Строка.Ссылка);
		КонецЕсли;
		
	КонецЦикла;
	
	Данные.Очистить();
	
	Если ТранспортДанных.Тип = "ВыгрузкаПоСсылке" И Не ОписаниеОбъекта = Неопределено Тогда
		НайтиСсылкиПоОбъектуМетаданных(ОписаниеОбъекта, ЗначенияОбъекта, ТранспортДанных.КлючиЗаписей, ОписанияОбъектов, ПараметрыВыгрузки.НеВыгружаемыеПоля);
	КонецЕсли;
	
КонецПроцедуры

Функция ИспользуетсяОтборСКД(НастройкиВыгрузки)
	
	Результат = Ложь;
	
	Если НастройкиВыгрузки = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	Для Каждого ЭлементОтбора Из НастройкиВыгрузки.Отбор.Элементы Цикл
		Если ЭлементОтбора.Использование Тогда
			Результат = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат Результат;
	
КонецФункции

Процедура ДобавитьУсловиеПометкаУдаления(НастройкиКомпоновкиДанных)
	
	ПолеПометкаУдаления = Новый ПолеКомпоновкиДанных("ПометкаУдаления");
	Для Каждого Элемент Из НастройкиКомпоновкиДанных.Настройки.Отбор.Элементы Цикл
		Если Элемент.ЛевоеЗначение = ПолеПометкаУдаления И НЕ Элемент.Использование Тогда
			Элемент.Использование  = Истина;
			Элемент.ПравоеЗначение = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
		
КонецПроцедуры

Функция ПроверитьСсылочнуюЦелостностьВыгружаемыхДанных(ТранспортДанных, ОбъектДанных, ОписаниеОбъекта, НеВыгружаемыеПоля)
	
	СтатусПроверки = Истина;
	КлючОбъекта  = ?(ОписаниеОбъекта.ЭтоСсылка, ОбъектДанных.Ссылка, ?(ОписаниеОбъекта.ЭтоНаборЗаписей, ОбъектДанных.Отбор, Неопределено));
	
	Если Не ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеВыгружаемыеСсылки И Не ЗначениеЗаполнено(НеВыгружаемыеПоля) Тогда
		Возврат СтатусПроверки;
	КонецЕсли;
	
	Для Каждого Реквизит Из ОписаниеОбъекта.Объекты Цикл
		Если Реквизит.Раздел = "Команды"
			ИЛИ (Реквизит.Имя = "Ссылка" И ОписаниеОбъекта.ЭтоСсылка)
			ИЛИ Реквизит.Имя = "Родитель"
			ИЛИ Реквизит.Раздел = "ПризнакиУчетаСубконто" Тогда
			Продолжить;
		КонецЕсли;
		ИмяПоля = НеВыгружаемыеПоля.Получить(Реквизит.ПолноеИмя);
		Если Реквизит.ЭтоТаблица И ЗначениеЗаполнено(ИмяПоля) И ЗначениеЗаполнено(ОбъектДанных[ИмяПоля]) Тогда
			ОбъектДанных[ИмяПоля].Очистить();
		ИначеЕсли Реквизит.ЭтоТаблица Тогда
			РеквизитОчищен = 0;
			Для Каждого РеквизитТЧ Из Реквизит.Объекты Цикл
				Если ОбъектДанных[Реквизит.Имя].Количество() = 0 Тогда
					Прервать;
				КонецЕсли;
				ИмяПоля = НеВыгружаемыеПоля.Получить(РеквизитТЧ.ПолноеИмя);
				Если Не ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеВыгружаемыеСсылки И Не ЗначениеЗаполнено(ИмяПоля) Тогда
					Продолжить;
				КонецЕсли;
				РеквизитОчищен = ОчиститьРеквизитОбъектаДанных(ТранспортДанных, ОбъектДанных[Реквизит.Имя], РеквизитТЧ, Реквизит, ЗначениеЗаполнено(ИмяПоля));
				Если Не ЗначениеЗаполнено(ИмяПоля) И РеквизитОчищен > 0 Тогда
					ШаблонСообщения = НСтр("ru = 'При выгрузке ""%1"" очищен реквизит %2 %3 таблицы ""%4"".'");
					ТекстСообщения = СтрШаблон(
						ШаблонСообщения,
						ОписаниеОбъекта.Имя,
						РеквизитТЧ.Имя,
						СтрокаСЧислом(";в %1 строке;; в %1 строках; в %1 строках; в %1 строках", РеквизитОчищен, ВидЧисловогоЗначения.Количественное),
						Реквизит.Имя
					);
					ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 1, ТекстСообщения, КлючОбъекта, ОписаниеОбъекта);
				КонецЕсли;
			КонецЦикла;
			ОчищенаТалица = ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеВыгружаемыеСсылки И РеквизитОчищен И ОбъектДанных[Реквизит.Имя].Количество() = 0;
			//@skip-check query-in-loop
			Если ОчищенаТалица
				И Реквизит.ОбъектМетаданных.ПроверкаЗаполнения = ПроверкаЗаполнения.ВыдаватьОшибку
				И ПроверитьЗаполненностьРеквизита("*", Реквизит.ПутьВЗапросе,, ЗаполненныеРеквизиты) Тогда
				ШаблонСообщения = НСтр("ru = 'При выгрузке ""%1"" для ""%2"" возникла ошибка:
					|таблицу ""%3"" нельзя очищать, она является обязательной для заполнения.'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ОбъектДанных, ОписаниеОбъекта.Имя, Реквизит.Имя);
				ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения, КлючОбъекта, ОписаниеОбъекта);
				СтатусПроверки = Ложь;
				Прервать;
			ИначеЕсли ОчищенаТалица Тогда
				ШаблонСообщения = НСтр("ru = 'При выгрузке ""%1"" очищена таблица ""%2"".'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОбъекта.Имя, Реквизит.Имя);
				ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 1, ТекстСообщения, КлючОбъекта, ОписаниеОбъекта);
			КонецЕсли;
		ИначеЕсли Не ТранспортДанных.ПараметрыВыгрузки.ОчищатьНеВыгружаемыеСсылки И Не ЗначениеЗаполнено(ИмяПоля) Тогда
			Продолжить;
		Иначе
			РеквизитОчищен = ОчиститьРеквизитОбъектаДанных(ТранспортДанных, ОбъектДанных, Реквизит, ОписаниеОбъекта, ЗначениеЗаполнено(ИмяПоля));
			//@skip-check query-in-loop
			Если ЗначениеЗаполнено(ИмяПоля) Тогда
				// Безусловная очистка реквизита
			ИначеЕсли РеквизитОчищен > 0
				И Реквизит.ОбъектМетаданных.ПроверкаЗаполнения = ПроверкаЗаполнения.ВыдаватьОшибку
				И ПроверитьЗаполненностьРеквизита(Реквизит.Имя, ОписаниеОбъекта.ПолноеИмя, Реквизит.ОбъектМетаданных.Тип, ЗаполненныеРеквизиты) Тогда
				ШаблонСообщения = НСтр("ru = 'При выгрузке ""%1"" для ""%2"" возникла ошибка:
					|реквизит ""%3"" нельзя очищать, он является обязательным для заполнения.'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ОбъектДанных, ОписаниеОбъекта.Имя, Реквизит.Имя);
				ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения, КлючОбъекта, ОписаниеОбъекта);
				СтатусПроверки = Ложь;
				Прервать;
			ИначеЕсли РеквизитОчищен > 0
				И Реквизит.ЭтоИзмерение Тогда
				ШаблонСообщения = НСтр("ru = 'При выгрузке ""%1"" для ""%2"" возникла ошибка:
					|измерение ""%3"" нельзя очищать, оно является ключевой сущностью.'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ОбъектДанных, ОписаниеОбъекта.Имя, Реквизит.Имя);
				ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 2, ТекстСообщения, КлючОбъекта, ОписаниеОбъекта);
				СтатусПроверки = Ложь;
				Прервать;
			ИначеЕсли РеквизитОчищен > 0 Тогда
				ШаблонСообщения = НСтр("ru = 'При выгрузке ""%1"" очищен реквизит ""%2"".'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОбъекта.Имя, Реквизит.Имя);
				ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 1, ТекстСообщения, КлючОбъекта, ОписаниеОбъекта);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Возврат СтатусПроверки;
	
КонецФункции

Функция ОчиститьРеквизитОбъектаДанных(ТранспортДанных, ОбъектДанных, Реквизит, ОписаниеОбъекта, ОчиститьБезусловно = Ложь)
	
	РеквизитОчищен = 0;
	
	Если ОписаниеОбъекта.ЭтоСсылка Тогда
		Данные = Новый Массив;
		Данные.Добавить(ОбъектДанных);
	ИначеЕсли ОписаниеОбъекта.ЭтоНаборЗаписей Или ОписаниеОбъекта.ЭтоТаблица Тогда
		Данные = ОбъектДанных;
	Иначе
		Возврат РеквизитОчищен;
	КонецЕсли;
	
	СтрокиКУдалению = Новый Массив;
	
	Для Каждого СтрокаДанных Из Данные Цикл
		
		ЗначениеРеквизита = СтрокаДанных[Реквизит.Имя];
		
		Если ЗначениеРеквизита = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если ОчиститьБезусловно И ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
			СтрокаДанных[Реквизит.Имя] = Неопределено;
			РеквизитОчищен = РеквизитОчищен + 1;
			Продолжить;
		КонецЕсли;
		
		ТипРеквизита = ТипЗнч(ЗначениеРеквизита);
		
		ЭтоОчищаемаяСсылка = ЭтоСсылка(ТипРеквизита, Истина);
		
		Если Не ЭтоОчищаемаяСсылка Тогда
			Продолжить;
		КонецЕсли;
		
		ЭлементОбъектовМетаданных = РазрешенныеТипыКВыгрузке.Получить(ТипЗнч(ЗначениеРеквизита));
		
		Если ЭлементОбъектовМетаданных = Неопределено
			И ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
			СтрокаДанных[Реквизит.Имя] = Неопределено;
			РеквизитОчищен = РеквизитОчищен + 1;
		ИначеЕсли НЕ ЭлементОбъектовМетаданных = Неопределено
			И ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
			КлючЗаписиОбъекта = Новый Структура("Ссылка, ПометкаУдаления", ЗначениеРеквизита);
			Если НЕ ПроверитьКлючЗаписи(ЭлементОбъектовМетаданных, ТранспортДанных.КлючиЗаписей, КлючЗаписиОбъекта)Тогда
				СтрокаДанных[Реквизит.Имя] = Неопределено;
				РеквизитОчищен = РеквизитОчищен + 1;
			КонецЕсли;
		КонецЕсли;
		
		Если РеквизитОчищен > 0 И ОписаниеОбъекта.ЭтоТаблица Тогда
			УдалитьСтроку = Истина;
			Для Каждого РеквизитТЧ Из ОписаниеОбъекта.Объекты Цикл
				Если РеквизитТЧ.Имя = "НомерСтроки" Тогда
					Продолжить;
				КонецЕсли;
				Если Не ЗначениеЗаполнено(СтрокаДанных[РеквизитТЧ.Имя])
					И РеквизитТЧ.ОбъектМетаданных.ПроверкаЗаполнения = ПроверкаЗаполнения.ВыдаватьОшибку Тогда
					УдалитьСтроку = Истина;
					Прервать;
				Иначе
					УдалитьСтроку = УдалитьСтроку И (СтрокаДанных[РеквизитТЧ.Имя] = Ложь Или Не ЗначениеЗаполнено(СтрокаДанных[РеквизитТЧ.Имя]));
				КонецЕсли;
			КонецЦикла;
			Если УдалитьСтроку Тогда
				СтрокиКУдалению.Добавить(СтрокаДанных);
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого СтрокаДанных Из СтрокиКУдалению Цикл
		Данные.Удалить(СтрокаДанных);
	КонецЦикла;
	
	Возврат РеквизитОчищен;
	
КонецФункции

Функция ПредопределенныеТипыПриВыгрузке()
	
	Типы = Новый Массив;
	
	Для Каждого ОбъектМетаданных Из Метаданные.Справочники Цикл
		Типы.Добавить(ОбъектМетаданных);
	КонецЦикла;
	
	Для Каждого ОбъектМетаданных Из Метаданные.ПланыСчетов Цикл
		Типы.Добавить(ОбъектМетаданных);
	КонецЦикла;
	
	Для Каждого ОбъектМетаданных Из Метаданные.ПланыВидовХарактеристик Цикл
		Типы.Добавить(ОбъектМетаданных);
	КонецЦикла;
	
	Для Каждого ОбъектМетаданных Из Метаданные.ПланыВидовРасчета Цикл
		Типы.Добавить(ОбъектМетаданных);
	КонецЦикла;
	
	Возврат Типы;
	
КонецФункции

Функция ПолучитьФабрикуСУказаниемТипов(Знач Типы)
	
	НаборСхем = ФабрикаXDTO.ЭкспортСхемыXML("http://v8.1c.ru/8.1/data/enterprise/current-config");
	Схема = НаборСхем[0];
	Схема.ОбновитьЭлементDOM();
	
	УказанныеТипы = Новый Соответствие;
	Для каждого Тип Из Типы Цикл
		УказанныеТипы.Вставить(XMLТипСсылки(Тип), Истина);
	КонецЦикла;
	
	ПространствоИмен = Новый Соответствие;
	ПространствоИмен.Вставить("xs", "http://www.w3.org/2001/XMLSchema");
	РазыменовательПространствИменDOM = Новый РазыменовательПространствИменDOM(ПространствоИмен);
	ТекстXPath = "/xs:schema/xs:complexType/xs:sequence/xs:element[starts-with(@type,'tns:')]";
	
	Запрос = Схема.ДокументDOM.СоздатьВыражениеXPath(ТекстXPath, РазыменовательПространствИменDOM);
	//@skip-check server-execution-safe-mode
	Результат = Запрос.Вычислить(Схема.ДокументDOM);

	Пока Истина Цикл
		
		УзелПоля = Результат.ПолучитьСледующий();
		Если УзелПоля = Неопределено Тогда
			Прервать;
		КонецЕсли;
		АтрибутТип = УзелПоля.Атрибуты.ПолучитьИменованныйЭлемент("type");
		ТипБезNSПрефикса = Сред(АтрибутТип.ТекстовоеСодержимое, СтрДлина("tns:") + 1);
		
		Если УказанныеТипы.Получить(ТипБезNSПрефикса) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		УзелПоля.УстановитьАтрибут("nillable", "true");
		УзелПоля.УдалитьАтрибут("type");
	КонецЦикла;
	
	ЗаписьXML = Новый ЗаписьXML;
	ИмяФайлаСхемы = ПолучитьИмяВременногоФайла("xsd");
	ЗаписьXML.ОткрытьФайл(ИмяФайлаСхемы);
	ЗаписьDOM = Новый ЗаписьDOM;
	ЗаписьDOM.Записать(Схема.ДокументDOM, ЗаписьXML);
	ЗаписьXML.Закрыть();
	
	Фабрика = СоздатьФабрикуXDTO(ИмяФайлаСхемы);
	
	Попытка
		УдалитьФайлы(ИмяФайлаСхемы);
	Исключение
		СтрокаОписанияОшибки = ОписаниеОшибки();
		СообщитьПользователю(СтрокаОписанияОшибки);
	КонецПопытки;
	
	Возврат Фабрика;
	
КонецФункции

Процедура ПолучитьРазрешенныеТипыКВыгрузке(ОбъектыМетаданных)
	
	ОбъектыМетаданныхКВыгрузке = ОбъектыМетаданных.ПолучитьЭлементы();
	
	РазрешенныеТипыКВыгрузке = Новый Соответствие;
	
	Для Каждого Элемент Из ОбъектыМетаданныхКВыгрузке Цикл
		Если Не Элемент.Выгружать Тогда
			Продолжить;
		КонецЕсли;
		Если ЭтоТаблицаСсылочныхДанных(Элемент.ПолноеИмя) Тогда
			РазрешенныеТипыКВыгрузке.Вставить(Тип(СтрЗаменить(Элемент.ПолноеИмя, ".", "Ссылка.")), Элемент);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Функция ПодготовитьСхемуКомпоновкиДанных(ЭлементОбъектовМетаданных, СтрокаПолейДляВыборки = "", ВыбратьРазличные = Ложь, НеПомеченныеНаУдаление = Ложь)
	
	ТекстЗапроса = ПолучитьТекстЗапросаПоСтроке(ЭлементОбъектовМетаданных, СтрокаПолейДляВыборки, ВыбратьРазличные, НеПомеченныеНаУдаление);
	
	СхемаЗапроса = Новый СхемаЗапроса;
	СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
	Для Каждого Запрос Из СхемаЗапроса.ПакетЗапросов Цикл
		Для Каждого Оператор Из Запрос.Операторы Цикл
			Для Каждого Поле Из Оператор.ВыбираемыеПоля Цикл
				СоставПоля = СтрРазделить(Поле, ".");
				Если СоставПоля.Количество() > 2 Тогда
					СоставПоля.Удалить(СоставПоля.ВГраница());
					Оператор.ВыраженияОтбораКомпоновкиДанных.Добавить(СтрСоединить(СоставПоля, "."));
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
	ТекстЗапроса = СхемаЗапроса.ПолучитьТекстЗапроса();
	
	СхемаКомпоновкиДанных = Новый СхемаКомпоновкиДанных;
	
	ИсточникДанных = СхемаКомпоновкиДанных.ИсточникиДанных.Добавить();
	ИсточникДанных.Имя = "ИсточникДанных";
	ИсточникДанных.ТипИсточникаДанных = "Local";
	
	НаборыДанных = СхемаКомпоновкиДанных.НаборыДанных.Добавить(Тип("НаборДанныхЗапросСхемыКомпоновкиДанных"));
	НаборыДанных.Имя                          = "НаборыДанных";
	НаборыДанных.ИсточникДанных               = "ИсточникДанных";
	НаборыДанных.АвтоЗаполнениеДоступныхПолей = Истина;
	НаборыДанных.Запрос                       = ТекстЗапроса;
	
	ЭтоТаблицаСсылочныхДанных = ЭтоТаблицаСсылочныхДанных(ЭлементОбъектовМетаданных.ПолноеИмя);
	ЭтоНаборДанных            = ЭтоТаблицаНабораДанных(ЭлементОбъектовМетаданных.ПолноеИмя);
	ЭтоКонстанта              = ЭтоКонстанта(ЭлементОбъектовМетаданных.ПолноеИмя);
	
	ГруппировкаДетальныеЗаписи = СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
	ГруппировкаДетальныеЗаписи.Порядок.Элементы.Добавить(Тип("АвтоЭлементПорядкаКомпоновкиДанных"));
	
	ТипТаблицаЗначений = Тип("ТаблицаЗначений");
	
	Для Каждого ПолеОтбора Из СхемаЗапроса.ПакетЗапросов[0].Колонки Цикл
		
		Если ТипЗнч(ПолеОтбора) = Тип("КолонкаВложеннаяТаблицаСхемыЗапроса") Тогда
			Продолжить;
		КонецЕсли;
		
		Если ПолеОтбора.Псевдоним = "ИмяПредопределенныхДанных"
			ИЛИ ПолеОтбора.Псевдоним = "ВерсияДанных"
			ИЛИ ПолеОтбора.ТипЗначения.СодержитТип(ТипТаблицаЗначений) Тогда
			Продолжить;
		КонецЕсли;
		
		ПолеНабора = НаборыДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
		ПолеНабора.Заголовок   = СокрЛП(ПолеОтбора.Псевдоним);
		ПолеНабора.Поле        = ПолеОтбора.Псевдоним;
		ПолеНабора.ПутьКДанным = ПолеОтбора.Псевдоним;
		
		ЭлементОтбораКомпоновкиДанных = СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбораКомпоновкиДанных.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ПолеНабора.ПутьКДанным);
		ЭлементОтбораКомпоновкиДанных.ВидСравнения  = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбораКомпоновкиДанных.ИдентификаторПользовательскойНастройки = Новый УникальныйИдентификатор;
		ЭлементОтбораКомпоновкиДанных.Использование = Ложь;
		ЭлементОтбораКомпоновкиДанных.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
		
		ДобавитьПолеВыбора =
			ЗначениеЗаполнено(СтрокаПолейДляВыборки)
			Или (ПолеОтбора.Псевдоним = "Ссылка" Или ПолеОтбора.Псевдоним = "ПометкаУдаления") И ЭтоТаблицаСсылочныхДанных
			Или ЭтоНаборДанных И ЭтоИндексируемыйРеквизит(ПолеОтбора)
			Или ЭтоКонстанта;
		
		Если ДобавитьПолеВыбора Тогда
			ВыбранноеПолеКомпоновкиДанных = ГруппировкаДетальныеЗаписи.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
			ВыбранноеПолеКомпоновкиДанных.Заголовок = СокрЛП(ПолеОтбора.Псевдоним);
			ВыбранноеПолеКомпоновкиДанных.Использование = Истина;
			ВыбранноеПолеКомпоновкиДанных.Поле = Новый ПолеКомпоновкиДанных(ПолеОтбора.Псевдоним);
		КонецЕсли;
		
	КонецЦикла;
	
	СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Отбор.ИдентификаторПользовательскойНастройки = Новый УникальныйИдентификатор;
	
	Возврат СхемаКомпоновкиДанных;
	
КонецФункции

Функция ВывестиСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, ПроверятьДоступностьПолей = Ложь)
	
	ТаблицаДанных     = Новый ТаблицаЗначений;
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(
		СхемаКомпоновкиДанных,
		НастройкиКомпоновкиДанных.ПолучитьНастройки(), , ,
		Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"),
		ПроверятьДоступностьПолей
	);

	ПроцессорКомпоновки = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновки.Инициализировать(МакетКомпоновки);
	
	ПроцессорВывода = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений();
	ПроцессорВывода.УстановитьОбъект(ТаблицаДанных);
	ПроцессорВывода.Вывести(ПроцессорКомпоновки, Истина);
	
	КомпоновщикМакета   = Неопределено;
	МакетКомпоновки     = Неопределено;
	ПроцессорКомпоновки = Неопределено;
	ПроцессорВывода     = Неопределено;
	
	Возврат ТаблицаДанных;
	
КонецФункции

Процедура ЗагрузитьНастройкиКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиКомпоновкиДанных, НастройкиВыгрузки, УникальныйИдентификатор)
	
	ИсточникДоступныхНастроек = Новый ИсточникДоступныхНастроекКомпоновкиДанных(ПоместитьВоВременноеХранилище(СхемаКомпоновкиДанных, УникальныйИдентификатор));
	НастройкиКомпоновкиДанных.Инициализировать(ИсточникДоступныхНастроек);
	
	НовыеНастройкиВыгрузки = СхемаКомпоновкиДанных.НастройкиПоУмолчанию;
	Если Не НастройкиВыгрузки = Неопределено Тогда
		НовыеНастройкиВыгрузки.Отбор.Элементы.Очистить();
		СкопироватьЭлементыОтбораРекурсивно(НовыеНастройкиВыгрузки.Отбор.Элементы, НастройкиВыгрузки.Отбор.Элементы);
	КонецЕсли;
	
	НастройкиКомпоновкиДанных.ЗагрузитьНастройки(НовыеНастройкиВыгрузки);
	
КонецПроцедуры

Процедура СкопироватьЭлементыОтбораРекурсивно(ЭлементыОтбораПриемник, ЭлементыОтбораИсточник)
	
	Для Каждого ЭлементОтбора Из ЭлементыОтбораИсточник Цикл
		НовыйЭлементОтбора = ЭлементыОтбораПриемник.Добавить(Тип(ЭлементОтбора));
		ЗаполнитьЗначенияСвойств(НовыйЭлементОтбора, ЭлементОтбора);
		Если Тип(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			СкопироватьЭлементыОтбораРекурсивно(НовыйЭлементОтбора.Элементы, ЭлементОтбора.Элементы);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура ПолучитьПоляВыборкиРекурсивно(ЭлементыОтбора, ПоляВыборки)
	
	Для Каждого ЭлементОтбора Из ЭлементыОтбора Цикл
		Если Тип(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			ПолучитьПоляВыборкиРекурсивно(ЭлементОтбора.Элементы, ПоляВыборки);
		Иначе
			ИмяПоля = Строка(ЭлементОтбора.ЛевоеЗначение);
			Если ЭлементОтбора.Использование И ПоляВыборки.Найти(ИмяПоля) = Неопределено Тогда
				ПоляВыборки.Добавить(ИмяПоля);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Функция XMLТипСсылки(Знач Значение)
	
	Если ТипЗнч(Значение) = Тип("ОбъектМетаданных") Тогда
		ОбъектМетаданных = Значение;
		МенеджерОбъекта = МенеджерОбъектаПоПолномуИмени(ОбъектМетаданных.ПолноеИмя());
		Ссылка = МенеджерОбъекта.ПолучитьСсылку();
	Иначе
		ОбъектМетаданных = Значение.Метаданные();
		Ссылка = Значение;
	КонецЕсли;
	
	Если ЭтоСсылочныйОбъект(ОбъектМетаданных) Тогда
		XMLТип = СериализаторXDTO.XMLТипЗнч(Ссылка);
	Иначе
		ТекстИсключения = НСтр("ru = 'Ошибка при определении XMLТипа ссылки для объекта %1: объект не является ссылочным!'");
		ТекстИсключения = СтрЗаменить(ТекстИсключения, "%1", ОбъектМетаданных.ПолноеИмя());
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	Возврат XMLТип.ИмяТипа;
	
КонецФункции

Функция ПолучитьТекстЗапросаПоСтроке(ЭлементОбъектовМетаданных, Знач СтрокаПолейДляВыборки = "", ВыбратьРазличные = Ложь, НеПомеченныеНаУдаление = Ложь)
	
	Если НЕ ЗначениеЗаполнено(СтрокаПолейДляВыборки) Тогда
		СтрокаПолейДляВыборки = "ТаблицаДанных.*";
	КонецЕсли;
	
	//@skip-check bsl-ql-hub
	ШаблонТексЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ %1 %2 ИЗ %3 КАК ТаблицаДанных";
	ИмяТаблицаДанных = ЭлементОбъектовМетаданных.ПолноеИмя;
	
	ТекстЗапроса = СтрШаблон(ШаблонТексЗапроса, ?(ВыбратьРазличные, "РАЗЛИЧНЫЕ", ""), СтрокаПолейДляВыборки, ИмяТаблицаДанных);
	Если НеПомеченныеНаУдаление Тогда
		ТекстЗапроса = ТекстЗапроса + " ГДЕ НЕ ТаблицаДанных.ПометкаУдаления";
	КонецЕсли;
	
	Возврат ТекстЗапроса;
	
КонецФункции

#КонецОбласти

#Область Протокол

// Инициализирует протокол сообщений.
// 
// Возвращаемое значение:
//  Массив Из Структура:
//   * Текст    - Строка - Текст сообщения.
//   * Важность - Число - Важность сообщения: 0 - информация, 1 - предупреждение, 2 - ошибка.
//   * Группа   - Строка - Группировка протокола.
//              - ЛюбаяСсылка
//
Функция ИнициализироватьПротокол()
	
	Протокол = Новый Массив;
	
	Возврат Протокол;
	
КонецФункции

Функция ДобавитьСообщениеВПротокол(Протокол, Группа, Важность, Текст, Ключ = Неопределено, ОписаниеОбъекта = Неопределено)
	
	Если ТипЗнч(Ключ) = Тип("Структура") Тогда
		КлючПротокола = Новый СписокЗначений;
		Для Каждого КлючИЗначение Из Ключ Цикл
			КлючПротокола.Добавить(КлючИЗначение.Значение, КлючИЗначение.Ключ);
		КонецЦикла;
	ИначеЕсли ТипЗнч(Ключ) = Тип("Отбор") Тогда
		КлючПротокола = Новый СписокЗначений;
		Для Каждого ЭлементОтбора Из Ключ Цикл
			КлючПротокола.Добавить(ЭлементОтбора.Значение, ЭлементОтбора.Имя);
		КонецЦикла;
	Иначе
		КлючПротокола = Ключ;
	КонецЕсли;
	
	Если ОписаниеОбъекта = Неопределено Тогда
		ОписаниеОбъекта = Новый Структура("Раздел, Имя, ПолноеИмя");
	КонецЕсли;
	
	СтрокаПротокола = Новый Структура;
	СтрокаПротокола.Вставить("Группа",           Группа);
	СтрокаПротокола.Вставить("Важность",         Важность);
	СтрокаПротокола.Вставить("Текст",            Текст);
	СтрокаПротокола.Вставить("Ключ",             КлючПротокола);
	СтрокаПротокола.Вставить("Раздел",           ОписаниеОбъекта.Раздел);
	СтрокаПротокола.Вставить("ИмяТаблицы",       ОписаниеОбъекта.Имя);
	СтрокаПротокола.Вставить("ПолноеИмяТаблицы", ОписаниеОбъекта.ПолноеИмя);
	
	Протокол.Добавить(СтрокаПротокола);
	
	Возврат СтрокаПротокола;
	
КонецФункции

#КонецОбласти

#Область Прочее

Функция ПреобразоватьСоответствиеВСтруктуру(Соответствие)
	
	Структура = Новый Структура;
	Для Каждого КлючИЗначение Из Соответствие Цикл
		Структура.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
	
	Возврат Структура;
	
КонецФункции

Функция ПолучитьЗначениеСтруктурыJSON(СтруктураJSON, КлючJSON)
	
	СвойствоНайдено = СтруктураJSON.Свойство(КлючJSON);
	Если СвойствоНайдено Тогда
		ЗначениеJSON = СтруктураJSON[КлючJSON];
		Если ЗначениеJSON = Null Тогда
			ЗначениеJSON = Неопределено;
		КонецЕсли;
	Иначе
		ЗначениеJSON = Null;
	КонецЕсли;
	
	Возврат ЗначениеJSON;
	
КонецФункции

Функция ПолучитьМенеджерИлиМодуль(Имя)
	
	УстановитьБезопасныйРежим(Истина);
	Модуль = Вычислить(Имя);
	
	Возврат Модуль;
	
КонецФункции

Функция РазложитьСтрокуВМассивПодстрок(Знач Строка, Разделитель = ".")
	
	МассивСтрок = Новый Массив();
	ДлинаРазделителя = СтрДлина(Разделитель);
	Пока Истина Цикл
		Поз = СтрНайти(Строка, Разделитель);
		Если Поз = 0 Тогда
			Если (СокрЛП(Строка) <> "") Тогда
				МассивСтрок.Добавить(Строка);
			КонецЕсли;
			Возврат МассивСтрок;
		КонецЕсли;
		МассивСтрок.Добавить(Лев(Строка, Поз - 1));
		Строка = Сред(Строка, Поз + ДлинаРазделителя);
	КонецЦикла;
	
КонецФункции

#КонецОбласти

#КонецОбласти

#Иначе
ВызватьИсключение НСтр("ru = 'Недопустимый вызов объекта на клиенте'");
#КонецЕсли