...
Функция НайтиСсылкиПоЗначениюРеквизита(ЗначениеРеквизита, ИмяТаблицы, ИмяРеквизита) Экспорт
	...
	СсылкиТекущейТаблицы = Новый Массив;
...
//   * ОчищенныеРеквизиты - Соответствие Из КлючИЗначение
...
Процедура ЗаписатьКлючиЗаписейПоТипуВТранспортДанных(ТранспортДанных, ЭлементОбъектовМетаданных)
	...
	Для Каждого КлючЗаписи Из КлючиЗаписейПоТипу Цикл
		Если КлючЗаписи.Ключ = "ОписаниеОбъекта" Тогда
			Продолжить;
		КонецЕсли;
		ЗаписатьКлючЗаписиВТранспортДанных(ТранспортДанных, КлючЗаписи.Значение.Значение, ОписаниеОбъекта, ПараметрыВыгрузки.НеВыгружаемыеПоля);
		...
		ИначеЕсли Реквизит.Раздел = "ТабличныеЧасти" Тогда
			...
			Для Каждого РеквизитТЧ Из Реквизит.Объекты Цикл
				...
				Если Не ЗначениеЗаполнено(ИмяПоля) И РеквизитОчищен И ОписаниеОбъекта.ОчищенныеРеквизиты.Получить(РеквизитТЧ.ПолноеИмя) = Неопределено Тогда
					...
					ДобавитьСообщениеВПротокол(ТранспортДанных.Протокол, ОписаниеОбъекта.ПолноеИмя, 1, ТекстСообщения);
...
Функция ДобавитьСообщениеВПротокол(Протокол, Группа, Важность, Текст)
