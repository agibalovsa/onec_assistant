...
Процедура ЗаполнитьМетаданные(Форма, ЗаполнитьСтруктуруХранения = Ложь) Экспорт
	...
	Длительность = (ДатаОкончания - ДатаНачала)/1000;
	Форма.ВремяЗаполнения = Длительность;
...
Процедура ЗаполнитьСтатистикуТаблицы(ЭлементМетаданных, РеквизитыОбъекта = Неопределено, СистемнаяТаблица = Неопределено)
	...
	Иначе
		СтруктураПоСтатистике = ОбъектОбработки.ПолучитьСтатистикуЗаполненностиПолейВВСистемнойТаблице(ЭлементМетаданных.ПолноеИмя, СистемнаяТаблица);
	КонецЕсли;
...
&НаКлиенте
Функция ВыгрузитьДанныеОбъектов(Форма, РежимВыгрузкиПоСсылке = Ложь) Экспорт
	
	ПолучитьОбъектыМетаданныхПоПризнаку(Форма.ОбъектыМетаданных, "ЭтоОсновнаяТаблица", Истина);
	АдресФайлаВоВременномХранилище = ВыгрузитьДанныеОбъектовНаСервере(РежимВыгрузкиПоСсылке);
...
Процедура ЗагрузитьДанныеОбъектов(Адрес, ОчищатьНеСовпадающиеЭлементы) Экспорт
	...
	ЗагрузитьДанныеОбъектовНаСервере(Адрес, ОчищатьНеСовпадающиеЭлементы);
...
&НаСервере
Функция ВыгрузитьДанныеОбъектовНаСервере(РежимВыгрузкиПоСсылке = Ложь)
	
	ИнициализироватьОбъектОбработки();
	
	Возврат ОбъектОбработки.ВыгрузитьДанныеОбъектов(ОбъектыМетаданных, УникальныйИдентификатор, РежимВыгрузкиПоСсылке);
	
КонецФункции

&НаСервере
Процедура ЗагрузитьДанныеОбъектовНаСервере(Адрес, ОчищатьНеСовпадающиеЭлементы)
	
	ИнициализироватьОбъектОбработки();
	
	ОбъектОбработки. ЗагрузитьДанныеОбъектов(Адрес, ОчищатьНеСовпадающиеЭлементы);
