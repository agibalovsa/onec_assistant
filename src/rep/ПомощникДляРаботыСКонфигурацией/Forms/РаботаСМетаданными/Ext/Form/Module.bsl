...
Процедура ЗаполнитьМетаданные(Форма, ЗаполнитьСтруктуруХранения = Ложь) Экспорт
	...
	Если ЗаполнитьСтруктуруХранения Тогда
		ПоказатьПредупреждение(
			...
			НСтр("ru = 'Внимание. В режиме ""Анализ таблиц"", заполнение дерева метаданных происходит в несколько раз дольше.'")
		);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьДочерниеМетаданные(Форма, ТекущиеДанные, ПолучитьСтатистику = Ложь) Экспорт
	...
	ЗаполнитьДочерниеМетаданныеНаСервере(ЭлементОбъектовМетаданных.ПолучитьИдентификатор(), ПолучитьСтатистику);
...
&НаКлиенте
Процедура ПеренестиОбъектыМетаданныхПоПризнакуВИсточник(ОтбираемыеОбъектыМетаданных, СписокСвойств = "")
	...
	Для Каждого ЭлементыОбъектовМетаданныхПоПризнаку Из ОбъектыМетаданныхПоПризнаку Цикл
		...
		ЗаполнитьЗначенияСвойств(ЭлементОбъектаМетаданных, ЭлементыОбъектовМетаданныхПоПризнаку, СписокСвойств);
	КонецЦикла;
...
&НаКлиенте
Процедура ОткрытьФормуОбъектаПоКлючу(Данные, ВыбраннаяСтрока) Экспорт
	
	ТекущиеДанные = Данные.НайтиПоИдентификатору(ВыбраннаяСтрока);
	...
	ИначеЕсли Не ТекущиеДанные.Ключ = Неопределено Тогда
...
Функция ПолучитьКартинкуПоРазделу(ИмяПодраздела)
	...
	ИначеЕсли ИмяПодраздела = "ДокументыПроведение" Тогда
		Возврат КартинкиМетаданных.Области.ДокументПроведение.Картинка;
	...
	ИначеЕсли ИмяПодраздела = "Движения" Тогда
		Возврат ЛокальнаяБиблиотекаКартинок.РегистрНакопления;
	...
	ИначеЕсли ИмяПодраздела = "ПолучитьСтатистику" Тогда
		Возврат БиблиотекаКартинок.Обновить;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьМетаданныеПодраздела(ИмяРаздела, Владелец, ИмяПодраздела, МетаданныеЭлемента, МетаданныеОбъект, РеквизитыОбъекта, ИдентификаторОбъектаСОборотамиОстатками, ПолучитьСтатистику = Ложь)
	...
	ИначеЕсли ИмяПодраздела = "ФункциональныеОпции" И НЕ Метаданные.ФункциональныеОпции.Содержит(МетаданныеОбъект) Тогда
		...
		Для Каждого ФункциональнаяОпция Из ФункциональныеОпции Цикл
			...
			ЭлементМетаданных.Владелец         = Владелец;
	...
	ИначеЕсли ИмяПодраздела = "РегистрацияИзмененийКонстанты" Тогда
		...
		СтруктураПодраздела = Новый Структура("Назначение, Метаданные", ИмяПодраздела, Владелец);
	...
	ИначеЕсли ИмяПодраздела = "Прочие" Тогда
		...
		СтруктураПодраздела = Новый Структура("Прочие, Метаданные", Истина, Владелец);
		ДанныеСистемнойТаблицыПодраздела = СтруктураХранения.СтруктураХраненияСистемныхТаблиц.НайтиСтроки(СтруктураПодраздела);
		...
		Для Каждого СтрокаТаблицыЗначений Из ДанныеСистемнойТаблицыПодраздела Цикл
			...
			Подподраздел.Владелец   = Владелец;
	...
	Иначе
		...
		Подраздел.Имя      = ИмяПодраздела;
		Подраздел.Картинка = ПолучитьКартинкуПоРазделу(ИмяПодраздела);
		Если ИмяПодраздела = "ПредопределенныеДанные" Тогда
			Подраздел.ПолноеИмя  = Владелец;
		ИначеЕсли ИмяПодраздела = "РегистрацияИзменений"
			И ОбъектОбработки.ДоступнаРегистрацияОбъекта(МетаданныеОбъект) Тогда
			Подраздел.ПолноеИмя  = Владелец + ".Изменения";
		КонецЕсли;
	...
	ИначеЕсли ИмяПодраздела = "СистемныеТаблицы" Тогда
		...
		СтруктураПодраздела = Новый Структура("Метаданные, ИмяТаблицы", Владелец, "");
		ДанныеСистемнойТаблицыПодраздела = СтруктураХранения.СтруктураХраненияСистемныхТаблиц.НайтиСтроки(СтруктураПодраздела);
		...
		Для Каждого СтрокаТаблицыЗначений Из ДанныеСистемнойТаблицыПодраздела Цикл
			...
			Если СтрНайти(СтрокаТаблицыЗначений.Назначение, "ИнициализированныеПредопределенныеДанные") = 0 Тогда
				...
				Если СтрокаТаблицыЗначений.Назначение = "Обороты" Тогда
					...
					Подраздел.ПолноеИмя = Владелец + ".Обороты";
				...
				ИначеЕсли СтрокаТаблицыЗначений.Назначение = "Итоги" Тогда
					...
					Подраздел.ПолноеИмя = Владелец + ".Остатки";
					ИдентификаторОбъектаСОборотамиОстатками = Подраздел.ПолучитьИдентификатор();
	...
	ИначеЕсли ИмяПодраздела = "ПредопределенныеДанные" Тогда
		...
		СтруктураПодраздела = Новый Структура("Метаданные", Владелец);
		ДанныеСистемнойТаблицыПодраздела = СтруктураХранения.СтруктураХраненияСистемныхТаблиц.НайтиСтроки(СтруктураПодраздела);
	...
	ИначеЕсли ИмяРаздела = "РегистрыБухгалтерии" И ИмяПодраздела = "Измерения" Тогда
		...
		Для Каждого Счет Из Счета Цикл
			...
			Подраздел.ПолноеИмяДляРоли     = "Счет";
			...
			Подраздел.ЭтоПоле    = ОбъектОбработки.ЭтоПолеДанных(ИмяПодраздела);
	...
	Для Каждого МетаданныеПодраздел Из МетаданныеОбъект[ИмяПодраздела] Цикл
		...
		Если ИмяПодраздела = "Характеристики" Тогда
			...
			Для Каждого СвойствоХарактеристик Из ОбъектОбработки.ПолучитьСписокСвойствХарактеристик() Цикл
				...
				Если Не  МетаданныеПодраздел[СвойствоХарактеристик] = Неопределено Тогда
				...
				ПодразделХарактеристик.Владелец      = Владелец;
		...
		ИначеЕсли ИмяПодраздела = "Движения" Тогда
			...
			РазделДвижения           = МетаданныеПодраздела.Добавить();
			РазделДвижения.Имя       = МетаданныеПодраздел.Имя;
			РазделДвижения.ПолноеИмя = МетаданныеПодраздел.ПолноеИмя();
			РазделДвижения.Раздел    = ИмяПодраздела;
			РазделДвижения.Владелец  = Владелец;
			ТипРегистра = ОбъектОбработки.ПолучитьРазделПоПолномуИмени(МетаданныеПодраздел.ПолноеИмя());
			Если ТипРегистра = "РегистрыНакопления" Тогда
				РазделДвижения.ЗаголовокТипа = МетаданныеПодраздел.ВидРегистра;
			ИначеЕсли ТипРегистра = "РегистрыСведений" Тогда
				РазделДвижения.ЗаголовокТипа = МетаданныеПодраздел.ПериодичностьРегистраСведений;
			ИначеЕсли ТипРегистра = "РегистрыРасчета" Тогда
				ДанныеРегистраРасчета = СтрШаблон(
					"Базовый: %1, Периодичность: %2, ПериодДействия: %3",
					МетаданныеПодраздел.БазовыйПериод,
					МетаданныеПодраздел.Периодичность,
					?(МетаданныеПодраздел.ПериодДействия, МетаданныеПодраздел.График.ПолноеИмя(), "Нет")
				);
				РазделДвижения.ЗаголовокТипа = ДанныеРегистраРасчета;
			ИначеЕсли ТипРегистра = "РегистрыБухгалтерии" Тогда
				ДанныеРегистраРасчета = СтрШаблон(
					"План счетов: %1, Длина уточнения периода: %2",
					?(Не МетаданныеПодраздел.ПланСчетов = Неопределено, МетаданныеПодраздел.ПланСчетов.ПолноеИмя(), "Нет"),
					МетаданныеПодраздел.ДлинаУточненияПериода
				);
				РазделДвижения.ЗаголовокТипа = ДанныеРегистраРасчета;
			КонецЕсли;
			РазделДвижения.Картинка = ПолучитьКартинкуПоРазделу(ТипРегистра);
			
			Если ПолучитьСтатистику Тогда
				//@skip-check query-in-loop
				ЗаполнитьСтатистикуТаблицы(РазделДвижения, , "Регистратор ССЫЛКА " + Владелец);
			КонецЕсли;
		...
		ДобавитьДтКт =
			ИмяРаздела = "РегистрыБухгалтерии" И МетаданныеОбъект.Корреспонденция
			И (
				(
					ОбъектОбработки.ПроверитьСуществованиеСвойства("Балансовый", МетаданныеПодраздел)
					И НЕ МетаданныеПодраздел.Балансовый
				)
				ИЛИ Лев(МетаданныеПодраздел.Имя, 8) = "Субконто"
				ИЛИ Лев(МетаданныеПодраздел.Имя, 11) = "ВидСубконто"
				ИЛИ МетаданныеПодраздел.Имя = "СчетДт"
				ИЛИ МетаданныеПодраздел.Имя = "СчетКт"
			);
		...
		Для Каждого Приставка Из Приставки Цикл
			...
			ИмяМетаданныхПодраздела = ОбъектОбработки.ПолучитьИмяЭлементаОбъектаМетаданных(МетаданныеПодраздел, Приставка);
			
			Если ИмяРаздела = "ПланыСчетов"
				И ИмяПодраздела = "СтандартныеТабличныеЧасти"
				И ИмяМетаданныхПодраздела = "ВидыСубконто"
				И НЕ (
					ЗначениеЗаполнено(МетаданныеОбъект.ВидыСубконто)
					И МетаданныеОбъект.МаксКоличествоСубконто > 0
				) Тогда
				Продолжить;
			КонецЕсли;
			...
			Подраздел                  = МетаданныеПодраздела.Добавить();
			Подраздел.Имя              = ИмяМетаданныхПодраздела;
			Подраздел.Раздел           = ИмяПодраздела;
			Подраздел.Владелец         = Владелец;
			...
			ИначеЕсли ИмяПодраздела = "СтандартныеРеквизиты" Тогда
				...
				Подраздел.ПолноеИмяДляРоли     = МетаданныеОбъект.ПолноеИмя() + "." + МетаданныеПодраздел.Имя;
				Подраздел.КартинкаРоль         = ПолучитьКартинкуПоРазделу("Роли");
				Подраздел.КартинкаПользователь = ПолучитьКартинкуПоРазделу("Пользователи");
				Подраздел.СтандартныйРеквизит  = ИСТИНА;
				Подраздел.ВходитВИндексСУБД    = СтруктураХранения.СтруктураХраненияИндексов.Получить(Подраздел.ПолноеИмя) <> Неопределено;
				...
				РеквизитОбъекта = Новый Структура;
				РеквизитОбъекта.Вставить("Имя",       ИмяМетаданныхПодраздела);
			...
			Иначе
				...
				Если ИмяПодраздела = "Реквизиты" ИЛИ ИмяПодраздела = "РеквизитыАдресации" ИЛИ ИмяПодраздела = "Измерения" ИЛИ ИмяПодраздела = "Ресурсы" 
					...
					Если ИмяПодраздела = "Реквизиты" ИЛИ ИмяПодраздела = "РеквизитыАдресации" ИЛИ ИмяПодраздела = "ПризнакиУчета" ИЛИ ИмяПодраздела = "Измерения" ИЛИ ИмяПодраздела = "Ресурсы" Тогда
				...
				Подраздел.ПолноеИмя            = МетаданныеПодраздел.ПолноеИмя() + Приставка;
				Подраздел.ПолноеИмяДляРоли     = МетаданныеПодраздел.ПолноеИмя();
				Подраздел.КартинкаРоль         = ПолучитьКартинкуПоРазделу("Роли");
				Подраздел.КартинкаПользователь = ПолучитьКартинкуПоРазделу("Пользователи");
				Подраздел.ВходитВИндексСУБД    = СтруктураХранения.СтруктураХраненияИндексов.Получить(Подраздел.ПолноеИмя) <> Неопределено;
...
&НаСервере
Процедура ДобавитьМетаданныеТабличнойЧасти(Владелец, ИмяПодраздела, МетаданныеЭлемента, МетаданныеОбъект, РеквизитыТабличнойЧастиОбъекта)
	...
	Для Каждого МетаданныеПодраздел Из МетаданныеОбъект[ИмяПодраздела] Цикл
		...
		Реквизит.Владелец = Владелец;
		
		Если ИмяПодраздела = "СтандартныеРеквизиты" Тогда
			Реквизит.ПолноеИмя    = Владелец + "." + Реквизит.Имя;
		...
		Реквизит.ЭтоПоле              = ОбъектОбработки.ЭтоПолеДанных(ИмяПодраздела);
...
&НаСервере
Процедура ЗаполнитьОбъектМетаданныхПоРазделу(ЭлементМетаданных, ИмяРаздела, МетаданныеОбъект = Неопределено, ПолучитьСтатистику = Ложь)
...
Процедура ДобавитьМетаданныеПоРазделу(РазделыКонфигурации, Раздел)
	...
	Если Раздел.ИмяРаздела = "Документы" Тогда
		КартинкаПроведение = ПолучитьКартинкуПоРазделу("ДокументыПроведение");
	КонецЕсли;
	...
	Для Каждого ОбъектМетаданных Из Метаданные[Раздел.ИмяРаздела] Цикл
		...
		Если Раздел.ИмяРаздела = "Документы" И ОбъектМетаданных.Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить Тогда
			ЭлементМетаданных.Картинка = КартинкаПроведение;
		Иначе
			ЭлементМетаданных.Картинка = Картинка;
		КонецЕсли;
		...
		Если ЭлементМетаданных.ЭтоОсновнаяТаблица Тогда
			...
			ЭлементМетаданных.КартинкаПолучитьСтатистику = ПолучитьКартинкуПоРазделу("ПолучитьСтатистику");
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДочерниеМетаданныеНаСервере(ИдентификаторСтроки, ПолучитьСтатистику = Ложь)
	...
	ЗаполнитьОбъектМетаданныхПоРазделу(ТекущиеДанные, ТекущиеДанные.Раздел, Неопределено, ПолучитьСтатистику);
...
Процедура ПолучитьПраваДоступа(Форма, РежимРаботы) Экспорт
	...
	Если ТекущиеДанные.СтандартныйРеквизит Тогда
		...
		Иначе
			...
			Иначе
				ПутьКСтандартномуРеквизиту = СтрЗаменить(
					?(ЗначениеЗаполнено(ТекущиеДанные.ПолноеИмяДляРоли), ТекущиеДанные.ПолноеИмяДляРоли, ТекущиеДанные.ПолноеИмя),
	...
	Иначе
		...
		ПутьКОбъектуМетаданных     = ?(ЗначениеЗаполнено(ТекущиеДанные.ПолноеИмяДляРоли), ТекущиеДанные.ПолноеИмяДляРоли, ТекущиеДанные.ПолноеИмя);
...
&НаСервере
Функция ПолучитьПраваПоРолямИПользователямСервер(Знач ИмяАналитики, Знач ИмяМакета)
...
&НаСервере
Функция СформироватьТабличныйДокументПоПравам(
		Знач ПутьКОбъектуОтбора, Знач Объект, Знач ИмяМакета,
		Знач АнализироватьТолькоИнтерфейсныеОбъекты = Неопределено
	)
...
Функция ПолучитьПраваДоступаПоОбъектам(ИмяРолиПользователя, ИмяМакета, РежимРаботы, АнализироватьТолькоИнтерфейсныеОбъекты = Ложь)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Для Каждого ОбъектМетаданных Из Метаданные[Раздел.ИмяРаздела] Цикл
			...
			Если НЕ Раздел.ИмяРаздела = "Константы" Тогда
...
&НаСервере
Процедура ПолучитьПраваДоступаПоПодчиненнымОбъектам(
		ПодчиненныеОбъекты, ОписаниеОбъекта, ПраваОбъектовМетаданных,
...
&НаКлиенте
Процедура ВыполнитьОчисткуОбъекта(ЭлементОбъектовМетаданных) Экспорт
	
	ОчиститьОбъект(ЭлементОбъектовМетаданных);
...
&НаСервере
Процедура ОчиститьОбъект(ЭлементОбъектовМетаданных)
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ОчиститьОбъект(ЭлементОбъектовМетаданных);
...
&НаКлиенте
Процедура НайтиБитыеСсылки(Форма, Отбор = Неопределено) Экспорт
	
	НайтиБитыеСсылкиНаСервере(Отбор);
...
&НаСервере
Процедура НайтиБитыеСсылкиНаСервере(Отбор = Неопределено)
	
	ИнициализироватьОбъектОбработки();
	
	ТаблицаБитыхСсылок = ОбъектОбработки.ПолучитьТаблицуБитыхСсылок(Отбор);
...
Процедура ЗаполнитьСтатистику(Форма) Экспорт
	...
	ПеренестиОбъектыМетаданныхПоПризнакуВИсточник(Форма.ОбъектыМетаданных, "КоличествоЭлементов, КоличествоПредопределенных, Выключен");
...
Процедура ЗаполнитьСтатистикуРеквизитаПоТипам(Форма, ИмяПоля, ИмяТаблицы) Экспорт
	...
	Для Каждого Строка Из Форма.Типы Цикл
		...
		Статистика = СтатистикаРеквизитаПоТипам.Получить(СокрЛП(ЧастиТипа[0]));
		Строка.Количество = ?(Статистика = Неопределено, 0, Статистика.Количество);
		Строка.Размер     = ?(Статистика = Неопределено, 0, Статистика.Размер);
	КонецЦикла;
...
&НаСервере
Функция ПолучитьСтатистикуРеквизитаПоТипам(ИмяПоля, ИмяТаблицы)
	
	ИнициализироватьОбъектОбработки();
	
	Возврат ОбъектОбработки.ПолучитьСтатистикуРеквизитаПоТипам(ИмяПоля, ИмяТаблицы);
...
Процедура ЗаполнитьСтатистикуТаблицы(ЭлементМетаданных, РеквизитыОбъекта = Неопределено, СистемнаяТаблица = Неопределено)
	...
	Для Каждого Подраздел_МетаданныеЭлемента Из МетаданныеЭлемента Цикл
		...
		Для Каждого Подраздел Из МетаданныеПодраздела Цикл
			...
			Если ОбъектОбработки.ПроверитьТипДанных(Подраздел.Имя, ЭлементМетаданных.ПолноеИмя) = 0 Тогда
				Продолжить;
			КонецЕсли;
...
функция ИнициализироватьПараметрыВыгрузки(Инициализировать = Ложь) Экспорт
	...
	Возврат ИнициализироватьПараметрыВыгрузкиНаСервере(Инициализировать);
	
КонецФункции

&НаКлиенте
Функция ВыгрузитьДанныеОбъектов(Форма, ПараметрыВыгрузки) Экспорт
	
	ПолучитьОбъектыМетаданныхПоПризнаку(Форма.ОбъектыМетаданных, "ЭтоОсновнаяТаблица", Истина);
	ДанныеВыгрузки = ВыгрузитьДанныеОбъектовНаСервере(ПараметрыВыгрузки);
	ОбъектыМетаданных.ПолучитьЭлементы().Очистить();
...
Функция ЗагрузитьДанныеОбъектов(Адрес, ПараметрыВыгрузки) Экспорт
	...
	возврат ЗагрузитьДанныеОбъектовНаСервере(Адрес, ПараметрыВыгрузки);
...
&НаСервере
Функция ИнициализироватьПараметрыВыгрузкиНаСервере(Инициализировать)
	
	ИнициализироватьОбъектОбработки();
	
	Возврат ОбъектОбработки.ИнициализироватьПараметрыВыгрузки(Инициализировать);
	
КонецФункции

&НаСервере
Функция ВыгрузитьДанныеОбъектовНаСервере(ПараметрыВыгрузки)
	
	ИнициализироватьОбъектОбработки();
	
	Возврат ОбъектОбработки.ВыгрузитьДанныеОбъектов(ОбъектыМетаданных, УникальныйИдентификатор, ПараметрыВыгрузки);
	
КонецФункции

&НаСервере
Функция ЗагрузитьДанныеОбъектовНаСервере(Адрес, ПараметрыВыгрузки)
	
	ИнициализироватьОбъектОбработки();
	
	Возврат ОбъектОбработки.ЗагрузитьДанныеОбъектов(Адрес, ПараметрыВыгрузки);
