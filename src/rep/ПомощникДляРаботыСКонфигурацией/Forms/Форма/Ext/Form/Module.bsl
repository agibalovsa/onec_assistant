...
Процедура ВыгрузитьДанныеОбъектов()
	...
	РежимВыгрузкиПоСсылке = Элементы.ОбъектыМетаданныхРежимВыгрузкиПоСсылке.Пометка;
	СтрогийФорматВыгрузки = Элементы.ОбъектыМетаданныхСтрогийФорматВыгрузки.Пометка;
	
	АдресФайлаВоВременномХранилище = РаботаСМетаданными.ВыгрузитьДанныеОбъектов(
		ЭтотОбъект,
		РежимВыгрузкиПоСсылке,
		СтрогийФорматВыгрузки);
	
	ДатаОкончания = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ДлительностьОперации = (ДатаОкончания - ДатаНачала)/1000;
	
	Если Не ПустаяСтрока(АдресФайлаВоВременномХранилище) Тогда
		ИмяФайла = НСтр("ru = 'ФайлДанных'") + ?(СтрогийФорматВыгрузки, ".xml", ".json");
...
Процедура ЗагрузитьДанныеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ДатаНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Расширение = НРег(СтрЗаменить(Результат.СсылкаНаФайл.Расширение, ".", ""));
	
	РаботаСМетаданными.ЗагрузитьДанныеОбъектов(Результат.Адрес, Расширение, ДополнительныеПараметры.ОчищатьНеСовпадающиеЭлементы);
...
Процедура ЗапуститьУдалениеПомеченныхНаСервере()
	...
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ЗапуститьУдалениеПомеченных("", Истина);
