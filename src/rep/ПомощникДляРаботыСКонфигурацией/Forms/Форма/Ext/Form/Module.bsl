...
Асинх Процедура ВыполнитьКомандуПакетногоРежима()
	...
	ПараметрыЗапускаПомощника = ИнициализироватьПараметрыЗапускаПомощника();
	ШаблонСообщенияОтсутствуетФайл = НСтр("ru = 'Файл %1 не существует'");
	
	Если ЗначениеЗаполнено(ПараметрыЗапускаПомощника.Файл) Тогда
	...
	Если ПараметрыЗапускаПомощника.Режим = "ВыгрузитьДанные"
	...
	ИначеЕсли ПараметрЗапуска = "ЗагрузитьДанные" И ЗначениеЗаполнено(ПараметрыЗапускаПомощника.Файл) Тогда
		ОписаниеПомещенногоФайла = ПоместитьФайлНаСерверАсинх(Неопределено, Неопределено, "", ПараметрыЗапускаПомощника.Файл, УникальныйИдентификатор);
		Результат = Ждать ОписаниеПомещенногоФайла;
...
&НаКлиенте
Функция ИнициализироватьПараметрыЗапускаПомощника()
	...
	КлючиЗапуска = СтрРазделить(ПараметрЗапуска, ";");
