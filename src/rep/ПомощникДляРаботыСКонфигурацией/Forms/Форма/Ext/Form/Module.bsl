...
//
&НаКлиенте
Процедура ДобавитьТекстСообщения(ТекстСообщения, Важность, Группа) Экспорт
	
	ГруппаПротокола = ПолучитьГруппуПротокола(Группа);
	СтрокаПроткола = ГруппаПротокола.Добавить();
	Если Важность = 1 Тогда
		СтрокаПроткола.Важность = БиблиотекаКартинок.ДиалогВосклицание;
	ИначеЕсли Важность = 2 Тогда
		СтрокаПроткола.Важность = БиблиотекаКартинок.ДиалогСтоп;
	Иначе
		СтрокаПроткола.Важность = БиблиотекаКартинок.ДиалогИнформация;
	КонецЕсли;
	СтрокаПроткола.Текст    = ТекстСообщения;
	СтрокаПроткола.Уровень  = 1;
